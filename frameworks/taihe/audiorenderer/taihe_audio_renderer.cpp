...
int64_t AudioRendererImpl::GetImplPtr()
{
...
void AudioRendererImpl::StartSync()
{
    ...
    int32_t ret = audioRenderer_->StartWithError();
    if (ret != OHOS::AudioStandard::SUCCESS) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_SYSTEM, OHOS::AudioStandard::ErrMessage(ret));
        return;
    }
}
...
void AudioRendererImpl::DrainSync()
{
    ...
    int32_t ret = audioRenderer_->DrainWithError();
    if (ret != OHOS::AudioStandard::SUCCESS) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_SYSTEM, OHOS::AudioStandard::ErrMessage(ret));
        return;
    }
...
void AudioRendererImpl::FlushSync()
{
    ...
    int32_t ret = audioRenderer_->FlushWithError();
    if (ret != OHOS::AudioStandard::SUCCESS) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_SYSTEM, OHOS::AudioStandard::ErrMessage(ret));
        return;
    }
...
void AudioRendererImpl::PauseSync()
{
    ...
    int32_t ret = audioRenderer_->PauseWithError();
    if (ret != OHOS::AudioStandard::SUCCESS) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_SYSTEM, OHOS::AudioStandard::ErrMessage(ret));
        return;
    }
...
void AudioRendererImpl::StopSync()
{
    ...
    int32_t ret = audioRenderer_->StopWithError();
    if (ret != OHOS::AudioStandard::SUCCESS) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_SYSTEM, OHOS::AudioStandard::ErrMessage(ret));
        return;
    }
...
void AudioRendererImpl::ReleaseSync()
{
    ...
    int32_t ret = audioRenderer_->ReleaseWithError();
    if (ret != OHOS::AudioStandard::SUCCESS) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_SYSTEM, OHOS::AudioStandard::ErrMessage(ret));
        return;
    }
...
AudioStreamInfo AudioRendererImpl::GetStreamInfoSync()
{
    AudioStreamInfo emptyStreamInfo {
        .samplingRate = AudioSamplingRateOrInt::make_type_enum(
            ohos::multimedia::audio::AudioSamplingRate::key_t::SAMPLE_RATE_48000),
...
void AudioRendererImpl::SetIndependentAudioSessionStrategy(
    const AudioSessionStrategy &strategy, const uint32_t behavior)
{
    ...
    switch (audioSessionStrategy.concurrencyMode) {
        ...
        default:
            ...
            TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_INVALID_PARAM, "concurrencyMode is invalid");
            return;
    }
    CHECK_AND_RETURN_RET_LOG(
        (behavior & ~static_cast<uint32_t>(OHOS::AudioStandard::AudioSessionBehaviorFlags::MASK_VALID_BEHAVIOR)) == 0,
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_INVALID_PARAM), "invaild behavior param");
    int32_t ret = audioRenderer_->SetIndependentAudioSessionStrategy(audioSessionStrategy, behavior);
    if (ret != OHOS::AudioStandard::SUCCESS) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_ILLEGAL_STATE, "SetIndependentAudioSessionStrategy failed");
        return;
    }
}
    ...
    const std::string &cbName, AudioRendererImpl *taiheRenderer)
{
    ...
    taiheRenderer->audioRenderer_->SetRegisterInterruptCallbackFlag(true);
}
