...
const std::vector<OHOS::AudioStandard::DeviceType> DEVICE_TYPE_SET = {
    ...
    OHOS::AudioStandard::DeviceType::DEVICE_TYPE_BT_SPP,
...
const std::unordered_map<OHOS::AudioStandard::AudioSamplingRate, AudioSamplingRate> gNativeToAniAudioSamplingRate = {
    ...
    {OHOS::AudioStandard::AudioSamplingRate::SAMPLE_RATE_384000, AudioSamplingRate::key_t::SAMPLE_RATE_384000},
};
...
{
    int32_t intValue = GetIntValueFromSamplingRateUnion(options.streamInfo.samplingRate);
    if (!IsValidAudioSamplingRate(intValue)) {
    ...
    AudioRendererOptions const &options)
{
    ...
    if (IsValidAudioSamplingRate(intValue)) {
        audioStreamInfo.samplingRate = static_cast<OHOS::AudioStandard::AudioSamplingRate>(intValue);
    } else if (IsRendererCustomSampleRateValid(intValue)) {
        // When using custom sample rate, set samplingRate to a valid enum value as placeholder.
        // The actual rate is stored in customSampleRate field.
        audioStreamInfo.samplingRate = OHOS::AudioStandard::AudioSamplingRate::SAMPLE_RATE_8000;
        audioStreamInfo.customSampleRate = static_cast<uint32_t>(intValue);
    } else {
        AUDIO_ERR_LOG("invaild samplingRate");
        return AUDIO_ERR;
    }
...
int32_t TaiheParamUtils::GetPreferredInputDevice(OHOS::AudioStandard::AudioCapturerOptions *opts,
    AudioCapturerOptions const &options)
{
    CHECK_AND_RETURN_RET(options.preferredInputDevice.has_value(), AUDIO_OK);
    auto device = std::make_shared<OHOS::AudioStandard::AudioDeviceDescriptor>();
    CHECK_AND_RETURN_RET_LOG(device != nullptr, AUDIO_ERR, "No memory");

    bool argTransFlag = false;
    int32_t status = GetAudioDeviceDescriptor(device, argTransFlag, options.preferredInputDevice.value());
    CHECK_AND_RETURN_RET(status == AUDIO_OK, status);
    CHECK_AND_RETURN_RET_LOG(argTransFlag, AUDIO_INVALID_PARAM, "PreferInputDevice argTransFlag is false");
    ...
    AudioCapturerOptions const &options)
{
    ...
    status = GetPreferredInputDevice(opts, options);
    CHECK_AND_RETURN_RET_LOG(status == AUDIO_OK, status, "Parse PreferredInputDevice failed");
    ...
    AudioRendererOptions const &options)
{
    ...
    if (options.originalAppIdInfo.has_value()) {
        status = GetAppInfo(opts->appInfo, options.originalAppIdInfo.value());
        CHECK_AND_RETURN_RET_LOG(status == AUDIO_OK, status, "Parse AppIdInfo failed");
    }
    return AUDIO_OK;
}

int32_t TaiheParamUtils::GetAppInfo(OHOS::AudioStandard::AppInfo &appInfo, AppIdInfo const &options)
{
    appInfo.appUid = options.appUid;
    appInfo.appPid = options.appPid;

    CHECK_AND_RETURN_RET_LOG(options.appTokenId >= 0, AUDIO_INVALID_PARAM, "appTokenId is invalid negative value");
    appInfo.appTokenId = static_cast<uint32_t>(options.appTokenId);

    CHECK_AND_RETURN_RET_LOG(options.appTokenId >= 0, AUDIO_INVALID_PARAM, "appFullTokenId is invalid negative value");
    appInfo.appFullTokenId = static_cast<uint64_t>(options.appFullTokenId);
...
int32_t TaiheParamUtils::GetAudioDeviceTypeVector(
    std::vector<OHOS::AudioStandard::DeviceType> &deviceTypeVector, array_view<DeviceType> in)
{
    if (in.empty()) {
        deviceTypeVector = {};
        AUDIO_ERR_LOG("DeviceTypeVector vector is empty!");
        return AUDIO_INVALID_PARAM;
    }

    for (DeviceType &element : in) {
        int32_t intValue = element.get_value();
        if (std::find(DEVICE_TYPE_SET.begin(), DEVICE_TYPE_SET.end(), intValue) == DEVICE_TYPE_SET.end()) {
            AUDIO_INFO_LOG("Undefined device type: %{public}d", intValue);
            return AUDIO_INVALID_PARAM;
        }
        deviceTypeVector.emplace_back(static_cast<OHOS::AudioStandard::DeviceType>(intValue));
    }
    return AUDIO_OK;
}
...
int32_t TaiheParamUtils::UniqueEffectPropertyData(OHOS::AudioStandard::AudioEffectPropertyArray &propertyArray)
{
...
int32_t TaiheParamUtils::GetEffectPropertyArray(OHOS::AudioStandard::AudioEffectPropertyArray &propertyArray,
    array_view<AudioEffectProperty> in)
{
    OHOS::AudioStandard::AudioEffectPropertyArray effectArray;
    OHOS::AudioStandard::AudioEffectPropertyArray enhanceArray;
    for (const auto &inProperty : in) {
        OHOS::AudioStandard::AudioEffectProperty prop = {
...
int32_t TaiheParamUtils::GetIntValueFromSamplingRateUnion(AudioSamplingRateOrInt const &samplingRate)
{
    switch (samplingRate.get_tag()) {
        case AudioSamplingRateOrInt::tag_t::type_enum:
            return samplingRate.get_type_enum_ptr()->get_value();
        case AudioSamplingRateOrInt::tag_t::type_int:
            return *samplingRate.get_type_int_ptr();
        default:
            AUDIO_ERR_LOG("GetIntValueFromSamplingRateUnion unknown tag");
            return 0;
    }
}

bool TaiheParamUtils::IsRendererCustomSampleRateValid(int32_t sampleRate)
{
    return ((sampleRate >= OHOS::AudioStandard::AudioSamplingRate::SAMPLE_RATE_8000 &&
        sampleRate <= OHOS::AudioStandard::AudioSamplingRate::SAMPLE_RATE_384000 &&
        sampleRate % OHOS::AudioStandard::SAMPLE_RATE_RESOLUTION_10 == 0) ||
        (sampleRate == static_cast<int32_t>(OHOS::AudioStandard::AudioSamplingRate::SAMPLE_RATE_11025)));
}

bool TaiheParamUtils::IsValidAudioSamplingRate(int32_t sampleRate)
{
    for (auto rate : OHOS::AudioStandard::AUDIO_SUPPORTED_SAMPLING_RATES) {
        if (static_cast<int32_t>(rate) == sampleRate) {
            return true;
        }
    }
    return false;
...
AudioSamplingRateOrInt TaiheParamUtils::ToTaiheAudioSamplingRate(
    OHOS::AudioStandard::AudioSamplingRate audioSamplingRate)
{
    ...
    if (itr == gNativeToAniAudioSamplingRate.end()) {
        ...
        return AudioSamplingRateOrInt::make_type_enum(AudioSamplingRate::key_t::SAMPLE_RATE_8000);
    }
    return AudioSamplingRateOrInt::make_type_enum(itr->second);
}
...
{
    AudioSamplingRateOrInt samplingRateUnion = AudioSamplingRateOrInt::make_type_int(0);
    if (src->customSampleRate != 0) {
        samplingRateUnion = AudioSamplingRateOrInt::make_type_int(static_cast<int32_t>(src->customSampleRate));
    } else {
        samplingRateUnion = ToTaiheAudioSamplingRate(src->samplingRate);
    }
    AudioStreamInfo streamInfo = {
        .samplingRate = samplingRateUnion,
...
taihe::array<AudioEffectProperty> TaiheParamUtils::ToTaiheEffectPropertyArray(
    const OHOS::AudioStandard::AudioEffectPropertyArray &propertyArray)
{
...
VolumeEvent TaiheParamUtils::SetValueVolumeEvent(const OHOS::AudioStandard::VolumeEvent &volumeEvent)
{
    ...
    VolumeEvent taiheVolumeEvent {
        ...
        .appUid = taihe::optional<int32_t>(std::in_place_t{}, volumeEvent.appUid),
    };
    ...
    const OHOS::AudioStandard::CurrentOutputDeviceChangedEvent &event)
{
    CurrentOutputDeviceChangedEvent currentOutputDeviceChangedEvent {
        ...
        .preDevices = TaiheParamUtils::SetDeviceDescriptors(event.preDevices),
    };
...
taihe::array<ActiveStreamVolumeInfo> TaiheParamUtils::ToTaiheActiveStreamsVolumeInfo(
    const std::vector<OHOS::AudioStandard::ActiveStreamVolumeInfo> &activeStreamsVolumeInfo)
{
    std::vector<ActiveStreamVolumeInfo> result;
    for (const auto &activeStreamVolumeInfo : activeStreamsVolumeInfo) {
        result.emplace_back(ToTaiheActiveStreamVolumeInfo(activeStreamVolumeInfo));
    }
    return taihe::array<ActiveStreamVolumeInfo>(result);
}

ActiveStreamVolumeInfo TaiheParamUtils::ToTaiheActiveStreamVolumeInfo(
    const OHOS::AudioStandard::ActiveStreamVolumeInfo &src)
{
    ActiveStreamVolumeInfo activeStreamVolumeInfo = {
        .volumeType = TaiheAudioEnum::GetJsAudioVolumeType(src.volumeType_),
        .appVolume = src.systemAppVolumePercentage_,
        .clientUid = src.clientUid_,
    };
    return activeStreamVolumeInfo;
}
} // namespace ANI::Audio
