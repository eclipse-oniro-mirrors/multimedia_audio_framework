...
void AudioCapturerImpl::StartSync()
{
    ...
    int32_t ret = audioCapturer_->StartWithError();
...
void AudioCapturerImpl::StopSync()
{
    ...
    int32_t ret = audioCapturer_->StopWithError();
    if (ret != OHOS::AudioStandard::SUCCESS) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_SYSTEM, OHOS::AudioStandard::ErrMessage(ret));
        return;
    }
...
void AudioCapturerImpl::ReleaseSync()
{
    ...
    int32_t ret = audioCapturer_->ReleaseWithError();
    if (ret != OHOS::AudioStandard::SUCCESS) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_SYSTEM, OHOS::AudioStandard::ErrMessage(ret));
        return;
    }
...
void AudioCapturerImpl::SetMuteHintSync(bool mute)
{
    if (audioCapturer_ == nullptr) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_SYSTEM, "audioCapturer_ is nullptr");
        return;
    ...
    int32_t ret = audioCapturer_->SetMuteHint(mute);
    if (ret == OHOS::AudioStandard::ERROR_ILLEGAL_STATE || ret == OHOS::AudioStandard::ERR_ILLEGAL_STATE) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_ILLEGAL_STATE, "SetMuteHint failed");
    } else if (ret != OHOS::AudioStandard::SUCCESS) {
        TaiheAudioError::ThrowErrorAndReturn(TAIHE_ERR_SYSTEM, "SetMuteHint failed");
    }
}
...
void AudioCapturerImpl::SetIndependentAudioSessionStrategy(
    const AudioSessionStrategy &strategy, const uint32_t behavior)
{
    ...
    int32_t ret = audioCapturer_->SetIndependentAudioSessionStrategy(audioSessionStrategy, behavior);
    if (ret != OHOS::AudioStandard::SUCCESS) {
