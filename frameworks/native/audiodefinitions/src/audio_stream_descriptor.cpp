...
AudioStreamDescriptor::AudioStreamDescriptor(const std::shared_ptr<AudioStreamDescriptor> streamDescriptor)
{
    ...
    stateStartTimeStamp_ = streamDescriptor->stateStartTimeStamp_;
    ...
    micInStreamInfo_ = streamDescriptor->micInStreamInfo_;
    ecStreamInfo_ = streamDescriptor->ecStreamInfo_;
...
void AudioStreamDescriptor::CopyToStruct(AudioStreamDescriptor &streamDesc)
{
    ...
    streamDesc.stateStartTimeStamp_ = stateStartTimeStamp_;
    ...
    streamDesc.micInStreamInfo_ = micInStreamInfo_;
    streamDesc.ecStreamInfo_ = ecStreamInfo_;
...
bool AudioStreamDescriptor::Marshalling(Parcel &parcel) const
{
    return streamInfo_.Marshalling(parcel) &&
        ...
        parcel.WriteInt64(stateStartTimeStamp_) &&
        ...
        micInStreamInfo_.Marshalling(parcel) &&
...
AudioStreamDescriptor *AudioStreamDescriptor::Unmarshalling(Parcel &parcel)
{
    ...
    info->stateStartTimeStamp_ = parcel.ReadInt64();
    ...
    info->micInStreamInfo_.UnmarshallingSelf(parcel);
...
void AudioStreamDescriptor::SetBundleName(const std::string &bundleName)
{
...
void AudioStreamDescriptor::DumpCommonAttrs(std::string &dumpString)
{
    ...
    AppendFormat(dumpString, "    - StateStartTimestamp: %" PRId64"\n", stateStartTimeStamp_);
}
...
void AudioStreamDescriptor::DumpRendererStreamAttrs(std::string &dumpString)
{
    ...
    if (routeFlag_ & AUDIO_OUTPUT_FLAG_FAST) {
        AppendFormat(dumpString, "    - UltraFastFlag: %u\n", ultraFastFlag_);
    }
}
