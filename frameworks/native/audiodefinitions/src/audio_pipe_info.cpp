...
AudioPipeInfo::AudioPipeInfo(const std::shared_ptr<AudioPipeInfo> pipeInfo)
{
    ...
    isRunning_ = pipeInfo->isRunning_;
}
...
void AudioPipeInfo::DumpOutputAttrs(std::string &dumpString)
{
    ...
    if (routeFlag_ & AUDIO_OUTPUT_FLAG_FAST) {
        AppendFormat(dumpString, " - UltraFastPipe: %d\n", ultraFastPipe_);
    }
}
...
bool AudioPipeInfo::IsPipeMatch(const std::string adapterName, const uint32_t targetFlag)
{
    return adapterName_ == adapterName && (routeFlag_ & targetFlag);
}
} // AudioStandard
} // namespace OHOS