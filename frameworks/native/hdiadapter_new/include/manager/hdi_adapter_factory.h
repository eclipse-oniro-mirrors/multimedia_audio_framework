...
private:
    ...
    bool IsMultiStreamInfo(const std::string &info);
...
#ifdef FEATURE_DISTRIBUTE_AUDIO
    std::shared_ptr<IAudioCaptureSource> CreateRemoteCaptureSource(const uint32_t captureId, const std::string &info);
