...
void FastAudioRenderSink::DoSetOutputRoute(std::vector<DeviceType> &outputDevices)
{
...
int32_t FastAudioRenderSink::Start(void)
{
    ...
    if (runningLock_ != nullptr) {
        ...
        HILOG_COMM_ERROR("[Start]running lock is null, playback can not work well");
    }
#endif
...
bool FastAudioRenderSink::SetPlayingAppNameParam(const std::string &param)
{
