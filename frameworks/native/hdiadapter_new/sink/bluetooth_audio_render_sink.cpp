    ...
    : isBluetoothLowLatency_(isBluetoothLowLatency), halName_(halName)
{
    if (halName_ == HDI_ID_INFO_HEARING_AID) {
        sinkType_ = ADAPTER_TYPE_HEARING_AID_NM;
    }
...
int32_t BluetoothAudioRenderSink::Start(void)
{
    ...
    while (tryCount-- > 0) {
        ...
        CHECK_AND_BREAK_LOG(audioRender_ != nullptr, "render is nullptr");
        int32_t ret = audioRender_->control.Start(reinterpret_cast<AudioHandle>(audioRender_));
        if (ret) {
            HILOG_COMM_ERROR("[Start]start fail, remain %{public}d attempt(s)", tryCount);
    ...
    HILOG_COMM_ERROR("[Start]start fail for three times, return");
#ifdef FEATURE_POWER_MANAGER
    if (runningLock_ != nullptr) {
...
void BluetoothAudioRenderSink::SetA2dpCacheParam()
{
    std::pair<AudioParamKey, std::pair<std::string, std::string>> param = {AudioParamKey::NONE, {"", ""}};
    HdiAdapterManager &manager = HdiAdapterManager::GetInstance();
    int32_t ret = manager.GetSinkPrestoreInfo(PRESTORE_INFO_AUDIO_BT_PARAM, param);
    if (ret == SUCCESS && param.first != AudioParamKey::NONE) {
        SetAudioParameterInner(param.second.second);
        AUDIO_INFO_LOG("Set a2dpParam %{public}s SUCCESS", param.second.second.c_str());
        manager.EraseSinkPrestoreInfo(PRESTORE_INFO_AUDIO_BT_PARAM);
    }
}
} // namespace AudioStandard
} // namespace OHOS
