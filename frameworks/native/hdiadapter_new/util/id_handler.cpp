    ...
    uint32_t routeFlag)
{
    ...
    if (deviceClass == "primary") {
        if (routeFlag == AUDIO_OUTPUT_FLAG_INTERPHONE) {
            return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY, HDI_ID_INFO_INTERPHONE);
        }
    ...
    } else if (deviceClass == "primary_extra") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_PRIMARY, info);
    ...
    return GetRenderIdByDeviceClassSub(deviceClass, info, routeFlag);
}

uint32_t IdHandler::GetRenderIdByDeviceClassSub(const std::string &deviceClass, const std::string &info,
    uint32_t routeFlag)
{
    if (deviceClass == "offload") {
    ...
    } else if (deviceClass == "primary_mmap" || deviceClass == "ultra_fast") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_FAST, HDI_ID_INFO_DEFAULT);
    } else if (deviceClass == "usb_arm_fast" || deviceClass == "usb_arm_ultra_fast") {
        return GetId(HDI_ID_BASE_RENDER, HDI_ID_TYPE_FAST, HDI_ID_INFO_USB);
...
uint32_t IdHandler::GetIdForSolePipeSource(SourceType sourceType, uint32_t routeFlag)
{
    ...
    } else if (routeFlag == AUDIO_INPUT_FLAG_LIVE) {
        return GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_PRIMARY, HDI_ID_INFO_LIVE);
    ...
    } else if (routeFlag == AUDIO_INPUT_FLAG_INTERPHONE) {
        return GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_PRIMARY, HDI_ID_INFO_INTERPHONE);
    } else if (routeFlag == AUDIO_INPUT_FLAG_CAMCORDER) {
        return GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_PRIMARY, HDI_ID_INFO_CAMCORDER);
    }
...
uint32_t IdHandler::GetCaptureIdByDeviceClass(const std::string &deviceClass, const SourceType sourceType,
    const std::string &info, uint32_t routeFlag)
{
    CHECK_AND_RETURN_RET_LOG(!deviceClass.empty(), HDI_INVALID_ID, "invalid device class");
    AUDIO_INFO_LOG("deviceClass: %{public}s, sourceType: %{public}d, info: %{public}s, routeFlag: %{public}u",
        deviceClass.c_str(), sourceType, info.c_str(), routeFlag);
    ...
    if (deviceClass == "primary") {
        ...
        if (SolePipe::IsSolePipeSource(sourceType)) {
            return GetIdForSolePipeSource(sourceType, routeFlag);
        }
        return GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_PRIMARY, HDI_ID_INFO_DEFAULT);
        ...
        return GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_FAST, HDI_ID_INFO_VOIP);
    } else if (deviceClass == "primary_mmap") {
        return GetId(HDI_ID_BASE_CAPTURE, HDI_ID_TYPE_FAST, HDI_ID_INFO_DEFAULT);
    }
