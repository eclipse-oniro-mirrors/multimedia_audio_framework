...
static constexpr uint32_t RAW_CHANNEL_COUNT = 1;
static constexpr uint32_t MICIN_CHANNEL_COUNT = 4;
static constexpr uint32_t CAMCORDER_RAW_CHANNEL_COUNT = 2;
static constexpr uint32_t CAMCORDER_MICIN_CHANNEL_COUNT = 4;
static constexpr uint32_t AUDIO_D2D = 12;
static constexpr uint32_t AUDIO_INPUT_INTERPHON_TYPE = 1 << 16;
AudioInputType hdiAudioInterphoneInputType = static_cast<AudioInputType>(AUDIO_INPUT_INTERPHON_TYPE);
...
static bool ResolveVoiceRecognitionLayout(uint64_t frameLen, size_t bytesPerSample, size_t &frameCount)
{
    if (frameLen == 0 || bytesPerSample == 0) {
        return false;
    }

    const uint64_t frameSize = static_cast<uint64_t>(bytesPerSample) * (RAW_CHANNEL_COUNT + MICIN_CHANNEL_COUNT);
    if (frameSize == 0 || frameLen % frameSize != 0) {
        return false;
    }
    frameCount = static_cast<size_t>(frameLen / frameSize);
    return frameCount > 0;
}

static bool InterleaveRawAndMicInFrame(const int8_t *src, uint64_t frameLen, char *dst, size_t bytesPerSample)
{
    if (src == nullptr || dst == nullptr || frameLen == 0 || bytesPerSample == 0) {
        return false;
    }

    size_t frameCount = 0;
    if (!ResolveVoiceRecognitionLayout(frameLen, bytesPerSample, frameCount)) {
        return false;
    }

    const size_t rawFrameSize = static_cast<size_t>(RAW_CHANNEL_COUNT) * bytesPerSample;
    const size_t micInFrameSize = static_cast<size_t>(MICIN_CHANNEL_COUNT) * bytesPerSample;
    const size_t outputFrameSize = rawFrameSize + micInFrameSize;
    const uint8_t *rawBase = reinterpret_cast<const uint8_t *>(src);
    const uint8_t *micInBase = rawBase + frameCount * rawFrameSize;
    uint8_t *dstBase = reinterpret_cast<uint8_t *>(dst);
    for (size_t frameIndex = 0; frameIndex < frameCount; ++frameIndex) {
        if (memcpy_s(dstBase + frameIndex * outputFrameSize, outputFrameSize, rawBase + frameIndex * rawFrameSize,
            rawFrameSize) != EOK) {
            return false;
        }
        if (memcpy_s(dstBase + frameIndex * outputFrameSize + rawFrameSize, outputFrameSize - rawFrameSize,
            micInBase + frameIndex * micInFrameSize, micInFrameSize) != EOK) {
            return false;
        }
    }
    return true;
...
static bool InterleaveRawAndMicInFrameForCamcorder(const int8_t *src, uint64_t frameLen,
    char *dst, size_t bytesPerSample)
{
    if (src == nullptr || dst == nullptr || frameLen == 0 || bytesPerSample == 0) {
        return false;
    }
    ...
    const uint64_t frameSize =
        static_cast<uint64_t>(bytesPerSample) * (CAMCORDER_RAW_CHANNEL_COUNT + CAMCORDER_MICIN_CHANNEL_COUNT);
    if (frameSize == 0 || frameLen % frameSize != 0) {
        return false;
    }

    const size_t frameCount = static_cast<size_t>(frameLen / frameSize);
    const size_t micInFrameSize = static_cast<size_t>(CAMCORDER_MICIN_CHANNEL_COUNT) * bytesPerSample;
    const size_t rawFrameSize = static_cast<size_t>(CAMCORDER_RAW_CHANNEL_COUNT) * bytesPerSample;
    const size_t outputFrameSize = micInFrameSize + rawFrameSize;
    const uint8_t *rawBase = reinterpret_cast<const uint8_t *>(src);
    const uint8_t *micInBase = rawBase + frameCount * rawFrameSize;
    uint8_t *dstBase = reinterpret_cast<uint8_t *>(dst);
    for (size_t frameIndex = 0; frameIndex < frameCount; ++frameIndex) {
        if (memcpy_s(dstBase + frameIndex * outputFrameSize, outputFrameSize,
            rawBase + frameIndex * rawFrameSize, rawFrameSize) != EOK) {
            return false;
        }
        if (memcpy_s(dstBase + frameIndex * outputFrameSize + rawFrameSize, outputFrameSize - rawFrameSize,
            micInBase + frameIndex * micInFrameSize, micInFrameSize) != EOK) {
...
{
    isCaptureThreadRunning_.store(false);
    if (stopThread_ != nullptr && stopThread_->joinable()) {
        stopThread_->join();
        stopThread_ = nullptr;
    }
...
{
    AUDIO_INFO_LOG("in routeFlag:%{public}d", attr.routeFlag);
    ...
    int32_t routeFlag = static_cast<int32_t>(ConvertToAudioInputRouteFlag(attr.sourceType));
    InitPipeInfo(hdiCaptureId_, AudioTypeUtils::HalNameToType(halName_), routeFlag,
...
bool AudioCaptureSource::IsStarted(void)
{
...
int32_t AudioCaptureSource::Start(void)
{
    ...
    if (!address_.empty()) {
        dumpFilePrefix = address_;
    }
#endif
    // eg: primary_source_0_20240527202236189_44100_2_1.pcm
    dumpFileName_ = dumpFilePrefix + "_source_" + std::to_string(attr_.sourceType) + "_" + GetTime() + "_" +
    ...
    micInDumpFileName_ = halName_ + "_source_micin_" + std::to_string(attr_.sourceType) + "_" + GetTime() + "_" +
        std::to_string(attr_.sampleRateMicIn) + "_" + std::to_string(attr_.channelMicIn) + "_" +
        std::to_string(attr_.formatMicIn) + ".pcm";
    DumpFileUtil::OpenDumpFile(DumpFileUtil::DUMP_SERVER_PARA, micInDumpFileName_, &micInDumpFile_);
...
int32_t AudioCaptureSource::Stop(void)
{
    ...
    stopThread_ = std::make_unique<std::thread>([&promiseEnsureLock, this] {
        std::lock_guard<std::mutex> lock(statusMutex_);
...
int32_t AudioCaptureSource::CaptureFrame(char *frame, uint64_t requestBytes, uint64_t &replyBytes)
{
    ...
    if (attr_.sourceType == SOURCE_TYPE_CAMCORDER && attr_.channelMicIn > CHANNEL_UNKNOW) {
        std::vector<int8_t> buffer(reinterpret_cast<int8_t *>(frame),
            reinterpret_cast<int8_t *>(frame) + replyBytes);
        size_t bytesPerSample = static_cast<size_t>(GetByteSizeByFormat(attr_.format));
        if (!InterleaveRawAndMicInFrameForCamcorder(buffer.data(), replyBytes, frame, bytesPerSample)) {
            AUDIO_WARNING_LOG("camcorder interleave failed, fallback to direct data");
        }
    }
...
void AudioCaptureSource::ProcessEcFrame(FrameDesc *fdescEc, uint64_t &replyBytesEc, AudioCaptureFrameInfo &frameInfo)
{
    ...
    uint64_t copyLen = replyBytesEc < fdescEc->frameLen ? replyBytesEc : fdescEc->frameLen;
    if (copyLen == 0) {
        AUDIO_WARNING_LOG("ec dump skip, copyLen is 0, sourceType:%{public}d, frameInfo.replyBytesEc:%{public}" PRIu64
            ", frameLen:%{public}" PRIu64, attr_.sourceType, frameInfo.replyBytesEc, fdescEc->frameLen);
        return;
    }
    if (copyLen != replyBytesEc) {
        AUDIO_WARNING_LOG("ec copyLen clamp, replyBytesEc:%{public}" PRIu64 ", frameLen:%{public}" PRIu64,
            replyBytesEc, fdescEc->frameLen);
        replyBytesEc = copyLen;
    }
    if (memcpy_s(fdescEc->frame, fdescEc->frameLen, frameInfo.frameEc, copyLen) != EOK) {
        AUDIO_ERR_LOG("copy desc ec fail");
    } else {
        HdiDfxUtils::DumpData(fdescEc->frame, copyLen, ecDumpFile_, ecDumpFileName_);
    }
}
    ...
    AudioCaptureFrameInfo &frameInfo)
{
    ...
    if (frameInfo.replyBytes < fdescEc->frameLen ||
        frameInfo.replyBytes - fdescEc->frameLen < fdesc->frameLen) {
        replyBytes = 0;
    ...
    AudioCaptureFrameInfo &frameInfo)
{
    ...
    uint64_t availableLen = 0;
    if (frameInfo.replyBytes > fdescEc->frameLen) {
        availableLen = frameInfo.replyBytes - fdescEc->frameLen;
    }
    uint64_t copyLen = std::min(fdesc->frameLen, availableLen);
    if (copyLen == 0) {
        replyBytes = 0;
        AUDIO_WARNING_LOG("cap dump skip, copyLen is 0, frameInfo.replyBytes:%{public}" PRIu64
            ", cap frameLen:%{public}" PRIu64 ", ec frameLen:%{public}" PRIu64,
            frameInfo.replyBytes, fdesc->frameLen, fdescEc->frameLen);
        return;
    }
    if (copyLen != fdesc->frameLen) {
        AUDIO_WARNING_LOG("cap copyLen clamp, frameInfo.replyBytes:%{public}" PRIu64
            ", cap frameLen:%{public}" PRIu64 ", ec frameLen:%{public}" PRIu64,
            frameInfo.replyBytes, fdesc->frameLen, fdescEc->frameLen);
    }

    bool interleaved = false;
    if (attr_.sourceType == SOURCE_TYPE_VOICE_RECOGNITION) {
        size_t bytesPerSample = static_cast<size_t>(GetByteSizeByFormat(attr_.format));
        interleaved = InterleaveRawAndMicInFrame(frameInfo.frame, copyLen, fdesc->frame, bytesPerSample);
        if (!interleaved) {
            AUDIO_WARNING_LOG("voice recognition interleave failed, fallback to direct copy");
        }
    }
    if (!interleaved && memcpy_s(fdesc->frame, fdesc->frameLen, frameInfo.frame, copyLen) != EOK) {
...
int32_t AudioCaptureSource::SetAudioScene(AudioScene audioScene, bool scoExcludeFlag)
{
    ...
    if (!scoExcludeFlag && (audioScene != currentAudioScene_ || currentScoExcludeFlag_)) {
        struct AudioSceneDescriptor sceneDesc;
    ...
    currentScoExcludeFlag_ = scoExcludeFlag;
...
const std::unordered_map<std::string, AudioInputType> AudioCaptureSource::audioInputTypeMap_ = {
    ...
    {"AUDIO_INPUT_INTERPHON_TYPE", hdiAudioInterphoneInputType}
};
...
enum AudioInputType AudioCaptureSource::ConvertToHDIAudioInputType(int32_t sourceType, std::string hdiSourceType)
{
    ...
    switch (sourceType) {
        ...
        case SOURCE_TYPE_UNPROCESSED_VOICE_ASSISTANT: // This configuration uses the old channel.
            ...
            break;
        case SOURCE_TYPE_INTERPHONE:
            hdiAudioInputType = hdiAudioInterphoneInputType;
            break;
        default:
            hdiAudioInputType = AUDIO_INPUT_MIC_TYPE;
            break;
...
{
    if (hdiSource == static_cast<AudioInputType>(AUDIO_INPUT_INTERPHON_TYPE)) {
        return GenerateUniqueID(AUDIO_HDI_CAPTURE_ID_BASE, HDI_CAPTURE_OFFSET_INTERPHONE);
    }
    switch (hdiSource) {
        ...
        case AUDIO_INPUT_LIVE_TYPE:
            return GenerateUniqueID(AUDIO_HDI_CAPTURE_ID_BASE, HDI_CAPTURE_OFFSET_LIVE);
        ...
        case AUDIO_INPUT_CAMCORDER_TYPE:
            return GenerateUniqueID(AUDIO_HDI_CAPTURE_ID_BASE, HDI_CAPTURE_OFFSET_CAMCORDER);
        default:
            return GenerateUniqueID(AUDIO_HDI_CAPTURE_ID_BASE, HDI_CAPTURE_OFFSET_PRIMARY);
    }
}
...
void AudioCaptureSource::UpdateRawSampleAttr(struct AudioSampleAttributes &param)
{
    if (attr_.sourceType == SOURCE_TYPE_VOICE_RECOGNITION || attr_.sourceType == SOURCE_TYPE_CAMCORDER) {
        param.rawSampleAttributes.rawFormat = ConvertToHdiFormat(attr_.formatMicIn);
        param.rawSampleAttributes.rawSampleRate = attr_.sampleRateMicIn;
        param.rawSampleAttributes.rawChannelLayout = GetChannelLayoutByChannelCount(attr_.channelMicIn);
        AUDIO_INFO_LOG("raw info:[%{public}u_%{public}u_%{public}u] ec info:[%{public}u_%{public}u_%{public}u]",
            attr_.sampleRateMicIn, attr_.channelMicIn, attr_.formatMicIn,
            attr_.sampleRateEc, attr_.channelEc, attr_.formatEc);
    }
}

void AudioCaptureSource::UpdateEcSampleAttr(struct AudioSampleAttributes &param)
{
...
void AudioCaptureSource::InitAudioSampleAttr(struct AudioSampleAttributes &param)
{
    ...
    if (attr_.routeFlag == AUDIO_INPUT_FLAG_INTERPHONE) {
        param.type = static_cast<AudioCategory>(AUDIO_D2D);
    }
    ...
    int silenceThreTest = 4096;
    param.silenceThreshold = silenceThreTest;
    ...
    if ((attr_.hasEcConfig || attr_.sourceType == SOURCE_TYPE_EC) && attr_.channelEc != 0) {
        UpdateEcSampleAttr(param);
    }
    UpdateRawSampleAttr(param);
}
...
int32_t AudioCaptureSource::NonblockingStart(void)
{
    ...
    isCaptureThreadRunning_.store(true);
...
int32_t AudioCaptureSource::UpdateActiveDeviceWithoutLock(DeviceType inputDevice)
{
    ...
    if (currentScoExcludeFlag_) {
        AUDIO_INFO_LOG("currentScoExcludeFlag is true.");
        return SUCCESS;
    }
...
enum AudioFlag AudioCaptureSource::ConvertToAudioInputRouteFlag(int32_t sourceType)
{
    enum AudioFlag audioInputRouteFlag;
    switch (sourceType) {
        case SOURCE_TYPE_VOICE_TRANSCRIPTION:
            audioInputRouteFlag = AUDIO_INPUT_FLAG_AI;
            break;
        case SOURCE_TYPE_UNPROCESSED:
            audioInputRouteFlag = AUDIO_INPUT_FLAG_UNPROCESS;
            break;
        case SOURCE_TYPE_LIVE:
            audioInputRouteFlag = AUDIO_INPUT_FLAG_LIVE;
            break;
        case SOURCE_TYPE_MIC:
        case SOURCE_TYPE_CAMCORDER:
        case SOURCE_TYPE_VOICE_MESSAGE:
        case SOURCE_TYPE_VOICE_CALL:
            audioInputRouteFlag = AUDIO_INPUT_FLAG_NORMAL;
            break;
        case SOURCE_TYPE_ULTRASONIC:
            audioInputRouteFlag = AUDIO_INPUT_FLAG_ULTRASONIC;
            break;
        case SOURCE_TYPE_VOICE_RECOGNITION:
            audioInputRouteFlag = AUDIO_INPUT_FLAG_VOICE_RECOGNITION;
            break;
        case SOURCE_TYPE_UNPROCESSED_VOICE_ASSISTANT:
            audioInputRouteFlag = AUDIO_INPUT_FLAG_RAW_AI;
            break;
        case SOURCE_TYPE_WAKEUP:
            audioInputRouteFlag = AUDIO_INPUT_FLAG_WAKEUP;
            break;
        case SOURCE_TYPE_OFFLOAD_CAPTURE:
            audioInputRouteFlag = AUDIO_INPUT_FLAG_OFFLOAD;
            break;
        case SOURCE_TYPE_INTERPHONE:
            audioInputRouteFlag = AUDIO_INPUT_FLAG_INTERPHONE;
            break;
        default:
            audioInputRouteFlag = AUDIO_FLAG_NONE;
            break;
    }
    return audioInputRouteFlag;
}
} // namespace AudioStandard
} // namespace OHOS
