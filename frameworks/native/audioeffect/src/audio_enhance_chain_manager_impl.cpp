    ...
    const AudioEnhanceDeviceAttr &deviceAttr)
{
    ...
    AudioEnhanceParamAdapter enhancePara = { enhancePara_.muteInfo, enhancePara_.volumeInfo, enhancePara_.foldState,
        enhancePara_.powerState, enhancePara_.appsMuteState, capturerDevice, rendererDeivce, sceneType, "" };
...
int32_t AudioEnhanceChainManagerImpl::SetAudioEnhanceProperty(const AudioEffectPropertyArray &propertyArray,
...
int32_t AudioEnhanceChainManagerImpl::SetAudioEnhancePropertyToChains(const AudioEffectProperty &property)
{
    auto chainArray = ChainPool::GetInstance().GetAllChain();
    for (const auto &chain : chainArray) {
        if (chain != nullptr) {
...
int32_t AudioEnhanceChainManagerImpl::GetAudioEnhanceProperty(AudioEffectPropertyArray &propertyArray,
    DeviceType deviceType)
{
    std::lock_guard<std::mutex> lock(chainManagerMutex_);
...
int32_t AudioEnhanceChainManagerImpl::NotifyChainToStopStream()
{
    auto chainArray = ChainPool::GetInstance().GetAllChain();
    for (const auto &chain : chainArray) {
        if (chain != nullptr) {
            int32_t ret = chain->StopStream();
            CHECK_AND_RETURN_RET_LOG(ret == 0, ERROR,
                "chain: [%{public}" PRIu64 "] stop stream fail", chain->GetChainId());
        }
    }
...
int32_t AudioEnhanceChainManagerImpl::SetAppsMuteStateToChainById(uint64_t sceneKeyCode, bool isMute)
{
    {
        std::lock_guard<std::mutex> lock(chainManagerMutex_);
        enhancePara_.appsMuteState = static_cast<uint32_t>(isMute);
    }
    auto chain = ChainPool::GetInstance().GetChainById(sceneKeyCode);
    CHECK_AND_RETURN_RET_LOG(chain != nullptr, ERROR, "can not find [%{public}" PRIu64 "] chain", sceneKeyCode);
    int32_t ret = chain->SetAppsMuteState(isMute);
    CHECK_AND_RETURN_RET_LOG(ret == 0, ERROR,
        "[%{public}" PRIu64 "] set app mute:%{public}d fail", chain->GetChainId(), static_cast<int32_t>(isMute));
    return SUCCESS;
}
