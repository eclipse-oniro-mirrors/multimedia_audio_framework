...
public:
    ...
    int32_t StopStream();
    int32_t SetAppsMuteState(uint32_t appsMuteState);

private:
    void InitAudioEnhanceChain();
    void InitCollabAudioEnhanceChain();
    ...
    int32_t ProcessStopStream(void);
    ...
    int32_t ProcessSetAppsMuteState(uint32_t appsMuteState);
    ...
    int32_t InitSingleEnhanceModuleForCollab(AudioEffectHandle enhanceHandle, const std::string &enhanceProp);
