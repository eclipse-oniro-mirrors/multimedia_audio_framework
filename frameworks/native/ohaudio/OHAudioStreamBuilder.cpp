    ...
    OH_AudioStreamBuilder* builder, bool enabled)
{
    OHAudioStreamBuilder *audioStreamBuilder = convertBuilder(builder);
    CHECK_AND_RETURN_RET_LOG(audioStreamBuilder != nullptr, AUDIOSTREAM_ERROR_INVALID_PARAM, "convert builder failed");
    return audioStreamBuilder->SetCapturerLoopbackEffectEnabled(enabled);
}
...
OH_AudioStream_Result OHAudioStreamBuilder::SetSamplingRate(int32_t rate)
{
    switch (rate) {
        ...
        case AudioSamplingRate::SAMPLE_RATE_384000:
            AUDIO_DEBUG_LOG("samplingRate input value is valid");
            break;
        default:
            if (streamType_ == RENDERER_TYPE &&
                (rate >= SAMPLE_RATE_8000 && rate <= SAMPLE_RATE_384000 &&
                rate % SAMPLE_RATE_RESOLUTION_10 == 0)) {
                AUDIO_INFO_LOG("custom samplingRate %{public}d", rate);
                customSampleRate_ = static_cast<uint32_t>(rate);
                samplingRate_ = SAMPLE_RATE_8000;
                return AUDIOSTREAM_SUCCESS;
            }
            AUDIO_ERR_LOG("samplingRate input value is invalid");
            return AUDIOSTREAM_ERROR_INVALID_PARAM;
    }
    customSampleRate_ = 0;
    samplingRate_ = rate;
...
OH_AudioStream_Result OHAudioStreamBuilder::Generate(OH_AudioRenderer **renderer)
{
    ...
    CHECK_AND_RETURN_RET_LOG(streamType_ == RENDERER_TYPE, AUDIOSTREAM_ERROR_INVALID_PARAM, "StreamType invalid");
    CHECK_AND_RETURN_RET_LOG(renderer != nullptr, AUDIOSTREAM_ERROR_INVALID_PARAM, "Error, renderer is nullptr");
    ...
    streamInfo.customSampleRate = customSampleRate_;
    ...
    if (audioRenderer != nullptr && audioRenderer->Initialize(options)) {
...
OH_AudioStream_Result OHAudioStreamBuilder::Generate(OH_AudioCapturer **capturer)
{
    ...
    CHECK_AND_RETURN_RET_LOG(capturer != nullptr, AUDIOSTREAM_ERROR_INVALID_PARAM,
        "Error, capturer is nullptr");
    AudioStreamInfo streamInfo = {
        (AudioSamplingRate)samplingRate_,
        encodingType_,
        sampleFormat_,
    ...
    capturerInfo.loopBackEffectEnabled = loopBackEffectEnabled_;
...
OH_AudioStream_Result OHAudioStreamBuilder::SetCapturerLoopbackEffectEnabled(bool enabled)
{
    CHECK_AND_RETURN_RET_LOG(streamType_ == CAPTURER_TYPE, AUDIOSTREAM_ERROR_INVALID_PARAM,
        "SetCapturerLoopbackEffectEnabled error, invalid type input.");
    loopBackEffectEnabled_ = enabled;
    return AUDIOSTREAM_SUCCESS;
}
}  // namespace AudioStandard
}  // namespace OHOS
