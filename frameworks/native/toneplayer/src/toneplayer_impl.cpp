...
{
    std::lock_guard<std::mutex> lock(optMutex_);
    CHECK_AND_RETURN_RET_LOG(toneState_ != TONE_RELEASED, false, "TonePlayerImpl is already released");
...
bool TonePlayerImpl::Release()
{
    ...
    CHECK_AND_RETURN_RET_LOG(toneState_ != TONE_RELEASED, false, "Release audioRenderer_ failed");
...
bool TonePlayerImpl::InitAudioRenderer()
{
    ...
    if (toneType_ == TONE_TYPE_COMMON_SUPERVISORY_CALL_WAITING) {
        rendererOptions_.rendererInfo.isCallWaitingTone = true;
    }
