...
public:
    bool Start() override { return StartImpl() == SUCCESS; }
    int32_t StartWithError() override { return StartImpl(); }
    bool Stop() const override { return StopImpl() == SUCCESS; }
    int32_t StopWithError() const override { return StopImpl(); }
    bool Release() override { return ReleaseImpl() == SUCCESS; }
    ...
    int32_t GetMicInBufferSize(BufferDesc &bufDesc, size_t &processBufSize,
        size_t &micInBufSize, size_t &ecBufSize) override;
    int32_t DeinterleaveBuffer(BufferDesc &bufDesc, BufferDesc &processBufDesc,
        BufferDesc &micInBufDesc, BufferDesc &ecBufferDesc) override;
    ...
    int32_t RegisterCapturerPolicyServiceDiedCallback(const std::shared_ptr<IAudioStream> &ipcStream);
    int32_t RemoveCapturerPolicyServiceDiedCallback(const std::shared_ptr<IAudioStream> &ipcStream);
    ...
    int32_t GetCapturerDebugInfo(AudioCapturerDebugInfo &debugInfo) const override;

    uint32_t GetOverflowCount() const override;
    int32_t SelectInputDevice(const std::shared_ptr<AudioDeviceDescriptor> &desc) const override;
    ...
    int32_t SetInMainThreadState(bool isInMainThread) override;
    int32_t SetIndependentAudioSessionStrategy(const AudioSessionStrategy &strategy, const uint32_t behavior) override;
    ...
    mutable std::shared_ptr<AudioDeviceDescriptor> selectedDevice_ = nullptr;
    ...
    static AudioStreamParams ConvertToAudioStreamParams(const AudioCapturerParams params)
    {
        ...
        audioStreamParams.micInFormat = params.audioMicInSampleFormat;
        audioStreamParams.micInSamplingRate = params.micInSamplingRate;
        audioStreamParams.micInChannels = params.audioMicInChannel;
        audioStreamParams.micInEncoding = params.audioMicInEncoding;
        audioStreamParams.micInChannelLayout = params.micInChannelLayout;

        return audioStreamParams;
    }

private:
    int32_t SelectInputDeviceInner(const std::shared_ptr<AudioDeviceDescriptor> &desc) const;
    ...
    bool GenerateNewStream(IAudioStream::StreamClass targetClass, RestoreInfo restoreInfo, CapturerState previousState,
        IAudioStream::SwitchInfo &info);
    bool FallbackToNormalStreamForGenerateNewStream(const RestoreInfo &restoreInfo,
    ...
    void NotifyFastStatusChange(FastStatus status);
    void SetInnerStreamFastStatusChangeCallback(const std::shared_ptr<IAudioStream> &audioStream);
    ...
    int32_t InitSessionAndOpenDumpFiles(const AudioCapturerParams &params,
        const AudioStreamParams &audioStreamParams);
    void OpenCaptureDumpFiles(const AudioCapturerParams &params, const AudioStreamParams &audioStreamParams,
        uint32_t dumpChannels);
    ...
    void CheckStartResultAndReport(bool result);
    int32_t InitAndReportErrorEvent(AudioStreamParams audioStreamParams,
        IAudioStream::StreamClass streamClass, const AudioCapturerParams params);
    int32_t CheckStateAndReportErrorEvent(CapturerState state);
    int32_t CheckDegradeFallback(AudioStreamParams audioStreamParams, bool degradeFallback, size_t validDataLength,
        const size_t processFrameSize, size_t &processBufSize, size_t &micInBufSize, size_t &ecBufSize);
    bool CheckForGenerateNewStream();
    int32_t CheckForStartImpl(CapturerState state);
    void SendAudioErrorEventAndProcessOther(int32_t uid, int32_t errorCode,
    ...
    uint32_t sessionID_ = INVALID_STREAM_ID;
    ...
    FILE *dumpProcessFile_ = nullptr;
    FILE *dumpMicInFile_ = nullptr;
    FILE *dumpEcFile_ = nullptr;
    ...
    bool isFirstTimeSetMainThreadState_ = true;
};
...
private:
    ...
    bool NoNeedNotifyEvent(const InterruptEvent &interruptEvent);
    int32_t InterruptHintResumeHandle(StreamFocusState focusState);
    int32_t InterruptHintPauseHandle(StreamFocusState focusState);
    void WaitForSwitchTimeout(bool result);
    std::shared_ptr<IAudioStream> audioStream_;
...
class CapturerPolicyServiceDiedCallback : public RemoteDiedCallback,
