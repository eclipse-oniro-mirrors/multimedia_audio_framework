...
public:
    ...
    {
        return sharedAudioCapturer_->StartWithError();
    }
    ...
    {
        return sharedAudioCapturer_->StopWithError();
    }
    ...
    {
        return sharedAudioCapturer_->ReleaseWithError();
    }
    ...
    {
        return sharedAudioCapturer_->GetMicInBufferSize(bufDesc, processBufSize, micInBufSize, ecBufSize);
    }
    ...
    {
        return sharedAudioCapturer_->DeinterleaveBuffer(bufDesc, processBufDesc, micInBufDesc, ecBufferDesc);
    }
    ...
    int32_t SetMuteHint(bool mute) override
    {
        CHECK_AND_RETURN_RET_LOG(sharedAudioCapturer_ != nullptr,
            ERR_MEMORY_ALLOC_FAILED, "sharedAudioCapturer_ is nullptr");
        return sharedAudioCapturer_->SetMuteHint(mute);
    }
    ...
    {
        return sharedAudioCapturer_->SetInMainThreadState(isInMainThread);
    }

    int32_t SetIndependentAudioSessionStrategy(const AudioSessionStrategy &strategy, const uint32_t behavior) override
    {
        return sharedAudioCapturer_->SetIndependentAudioSessionStrategy(strategy, behavior);
    }

    int32_t GetCapturerDebugInfo(AudioCapturerDebugInfo &debugInfo) const override
    {
        return sharedAudioCapturer_->GetCapturerDebugInfo(debugInfo);
    }
    ...
    {
        return sharedAudioCapturer_->SelectInputDevice(desc);
    }
