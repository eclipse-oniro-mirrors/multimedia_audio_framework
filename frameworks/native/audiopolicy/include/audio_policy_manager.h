...
public:
    ...
    int32_t SetSystemAppVolumePercentageForUid(int32_t appUid, int32_t volumePercentage);
    ...
    int32_t GetSystemAppVolumePercentageForUid(int32_t appUid, int32_t &volumePercentage);
    ...
    bool IsMultichannelPlaybackSupported(AudioStreamInfo &streamInfo, StreamUsage usage);

    bool IsDirectPlaybackSupported(AudioStreamInfo &streamInfo, StreamUsage usage);

    bool IsOffloadPlaybackSupported(AudioStreamInfo &streamInfo, StreamUsage usage);

    bool IsEnhancedRoutingSupported();
    ...
    int32_t SetMediaOutputDevice(DeviceType deviceType);
    ...
    int32_t SetAncoSourceOutputStreamMute(int32_t uid, bool setMute);
    ...
    int32_t ActivateAudioSession(const AudioSessionStrategy &strategy, const bool stateChangeCallbackFlag = false);
    ...
    int32_t SetRegisterInterruptCallbackFlag(const uint32_t streamId, const bool interruptCallbackFlag);

    int32_t SetRegisterStateChangeCallbackFlag(const bool stateChangeCallbackFlag);
    ...
    int32_t SetAudioSessionCapturerMuteHint(bool mute);
    ...
    int32_t UpdateStreamState(const int32_t clientUid, StreamSetState streamSetState, StreamUsage streamUsage);
    ...
    int32_t SetDeclaredDeviceTypes(const std::vector<DeviceType> &deviceTypes);

    int32_t SetPreferredOutputDeviceChangeCallback(const AudioRendererInfo &rendererInfo,
        const std::shared_ptr<AudioPreferredOutputDeviceChangeCallback> &callback, const int32_t uid = INVALID_UID);

    int32_t SetPreferredInputDeviceChangeCallback(const AudioCapturerInfo &capturerInfo,
        const std::shared_ptr<AudioPreferredInputDeviceChangeCallback> &callback, const int32_t uid = INVALID_UID);
    ...
    int32_t GetAudioDebugInfo(uint32_t sessionId, AudioDebugInfo &audioDebugInfo);

    int32_t GetSessionDebugInfo(AudioSessionDebugInfo &sessionDebugInfo);
    ...
    bool IsPersonalizedSpatializationEnabled(const std::string address);

    int32_t DownloadPersonalizedHRTF(const int32_t fd, const int64_t length);

    int32_t GetCurrentSpatialAudioSourceType();

    int32_t RegisterSpatializationEnabledEventListener(
        const std::shared_ptr<AudioSpatializationEnabledChangeCallback> &callback, bool isFromBluetooth = false);
    ...
    int32_t RegisterSpatialAudioSourceTypeEventListener(
        const std::shared_ptr<AudioSpatialAudioSourceTypeChangeCallback> &callback);

    int32_t RegisterPersonalizedSpatializationEnabledEventListener(
    ...
    int32_t UnregisterSpatialAudioSourceTypeEventListener();

    int32_t UnregisterPersonalizedSpatializationEnabledEventListener();
    ...
    int32_t EnableSystemVolumeProxy(int32_t zoneId, DeviceType deviceType, bool enable);
    ...
    int32_t SetAudioRouteSelectorCallback(const std::shared_ptr<AudioRouteSelector> &callback);
    ...
    std::vector<AudioDevicePair> GetLoopbackSupportedDevicePairs();
    AudioDevicePair GetLoopbackPreferredDevicePair();
    ...
    bool SetLoopbackVolume(float volume);
    float GetLoopbackVolume();
    ...
    int32_t DestroyLoopback(LoopbackType type);
    ...
    int32_t UnsetAudioSceneChangeCallback(const std::shared_ptr<AudioManagerAudioSceneChangedCallback> &callback);
    ...
    bool IsCollaborativePlaybackSupportedForDevice(
    ...
    int32_t RecordBootAnimationClientStateTime(const BootAnimationInfo &info);
    ...
    int32_t TriggerAudioZoneVolumeSync();

    bool IsAudioSeparationEffectSupported();
    int32_t SetAudioSeparationEffectEnabled(bool enabled, int32_t uid, int64_t streamId = -1);
    int32_t SetAudioSeparationEffectVolume(AudioSeparationVolumeType type, double volume);
    int32_t OnAudioSeparationEffectEnabledChange(
        const std::shared_ptr<AudioSeparationEffectEnabledChangeCallback> &callback);
    int32_t OffAudioSeparationEffectEnabledChange();
...
private:
    ...
    int32_t SetCallbackRendererInfo(const AudioRendererInfo &rendererInfo, const int32_t uid = INVALID_UID);
    int32_t SetCallbackCapturerInfo(const AudioCapturerInfo &capturerInfo, const int32_t uid = INVALID_UID);
    ...
    static std::vector<std::weak_ptr<RemoteDiedCallback>> audioStreamCBMap_;
    ...
    std::vector<std::pair<AudioRendererInfo, int32_t>> rendererInfosWithUid_;
    std::vector<std::pair<AudioCapturerInfo, int32_t>> capturerInfosWithUid_;
