...
public:
    bool Start(StateChangeCmdType cmdType = CMD_FROM_CLIENT) override { return StartImpl(cmdType) == SUCCESS; }
    int32_t StartWithError(StateChangeCmdType cmdType = CMD_FROM_CLIENT) override { return StartImpl(cmdType); }
    bool Drain() const override { return DrainImpl() == SUCCESS; }
    int32_t DrainWithError() const override { return DrainImpl(); }
    bool Flush() const override { return FlushImpl() == SUCCESS; }
    int32_t FlushWithError() const override { return FlushImpl(); }
    bool Pause(StateChangeCmdType cmdType = CMD_FROM_CLIENT) override { return PauseImpl(cmdType) == SUCCESS; }
    int32_t PauseWithError(StateChangeCmdType cmdType = CMD_FROM_CLIENT) override { return PauseImpl(cmdType); }
    bool Stop() override { return StopImpl() == SUCCESS; }
    ...
    int32_t SelectOutputDevice(const std::shared_ptr<AudioDeviceDescriptor> &desc) const override;
    ...
    int32_t RegisterRendererPolicyServiceDiedCallback(const std::shared_ptr<IAudioStream> &ipcStream);
    int32_t RemoveRendererPolicyServiceDiedCallback(const std::shared_ptr<IAudioStream> &ipcStream);
    ...
    void UpdateInterruptState(int32_t state);
    ...
    void SetRegisterInterruptCallbackFlag(const bool interruptCallbackFlag) override;
    int32_t GetRendererDebugInfo(AudioRendererDebugInfo &debugInfo) const override;
    ...
    int32_t SetMainUseArk(bool isMainUseArk) override;
...
private:
    int32_t SelectOutputDeviceInner(const std::shared_ptr<AudioDeviceDescriptor> &desc) const;
    ...
    void UpdateFlagForSwitchInfo(IAudioStream::SwitchInfo &switchInfo, IAudioStream::StreamClass targetClass);
    ...
    int32_t SetSonicPitch(float pitch);
    ...
    std::string &GetBundleNameByUid();
    std::string &GetBundleNameByAppUid(int32_t uid);
    int32_t ActivateAudioInterruptInStart(const bool isMute);
    void HandleAudioInterruptInfo();
    ...
    bool ValidateNotSwitching();
    bool ValidateAudioStream();
    ...
    std::string bundleNameByUid_ = "";
    std::string bundleNameByAppUid_ = "";
    ...
    std::atomic<int32_t> audioInterruptState_ = INVALID;
    bool isFirstSetMainUseArk_ = true;
    bool interruptCallbackByIpcFlag_ = false;
...
private:
    ...
    bool NoNeedNotifyEvent(const InterruptEventInternal &interruptEvent);
    ...
    void HandleForcePause(StreamFocusState currentFocusState);
    void HandleForceStop(StreamFocusState currentFocusState);
    std::shared_ptr<IAudioStream> audioStream_;
...
class RendererPolicyServiceDiedCallback : public RemoteDiedCallback,
