...
int32_t AudioSuiteTempoPitchNode::Init()
{
    ...
    algoInterface_ =
        AudioSuiteAlgoInterface::CreateAlgoInterface(AlgoType::AUDIO_NODE_TYPE_TEMPO_PITCH, nodeParameter_);
    ...
    SetAudioNodeFormat(AudioFormat{{TEMPO_PITCH_ALGO_CHANNEL_LAYOUT, nodeParameter_.inChannels},
        static_cast<AudioSampleFormat>(nodeParameter_.inFormat),
        static_cast<AudioSamplingRate>(nodeParameter_.inSampleRate),
        {TEMPO_PITCH_ALGO_CHANNEL_LAYOUT, nodeParameter_.outChannels},
...
uint32_t AudioSuiteTempoPitchNode::CalculationNeedBytes(uint32_t frameLengthMs)
{
    ...
    CHECK_AND_RETURN_RET_LOG(MIN_SPEED != 0, ERROR, "speedRate is zero.");
    dataBytes = static_cast<size_t>(std::ceil(TEMPO_PITCH_PCM_FRAME_BYTES / MIN_SPEED)) * RESIZE_EXPAND_RATE +
                        RESIZE_EXPAND_BYTES;
    return dataBytes;
}
...
int32_t AudioSuiteTempoPitchNode::PrintNodeInfo(std::string &nodeInfo)
{
    std::string optionValue = "";
    int32_t ret = GetOptions("speedAndPitch", optionValue);
    CHECK_AND_RETURN_RET_LOG((ret == SUCCESS && !optionValue.empty()), ERROR, "GetOptions Error");
    nodeInfo = "    TempoAndPitch: " +
        AudioSuiteUtil::FormatOptionsValue("speedAndPitch", optionValue) + "\n";
    return SUCCESS;
}
