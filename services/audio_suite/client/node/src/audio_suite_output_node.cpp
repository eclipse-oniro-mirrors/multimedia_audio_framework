...
void AudioOutputNode::SetAudioNodeFormat(AudioFormat audioFormat)
{
    ...
    needDataLength_ = SINGLE_FRAME_NEED_LENGTH;
    if (audioFormat.rate == AudioSamplingRate::SAMPLE_RATE_11025) {
        needDataLength_ = DOUBLE_FRAME_NEED_LENGTH;
    }
...
int32_t AudioOutputNode::DoProcess(uint32_t needDataLength)
{
    ...
    SetAudioNodeDataFinishedFlag(outputs_[0]->GetIsFinished());
    bufferUsedOffset_ = 0;
    ...
    int32_t requestFrameSize, int32_t *responseSize, bool *finishedFlag)
{
    ...
    if (GetAudioNodeDataFinishedFlag() && CacheBufferEmpty()) {
        StreamDfxManager::GetInstance().SendAudioErrorEvent(
            ...
            BuildErrorCode(ProblemCategory::FAULT_WRONG_AUDIO_EFFECT, OperationType::EDIT, BusinessScenario::RENDER,
                           ERR_NOT_SUPPORTED),
            "SetEqualizerFrequencyBandGains failed", true);
        AUDIO_ERR_LOG("multi output finished completed.");
    ...
    do {
        ...
        CHECK_AND_RETURN_RET_LOG(DoProcess(needDataLength_) == SUCCESS, ERROR, "Get data from pre node failed.");
