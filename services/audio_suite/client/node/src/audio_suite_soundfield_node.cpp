...
int32_t AudioSuiteSoundFieldNode::Init()
{
    ...
    SetAudioNodeFormat(AudioFormat{{SOUNDFIELD_ALGO_CHANNEL_LAYOUT, nodeParameter_.inChannels},
        ...
        {SOUNDFIELD_ALGO_CHANNEL_LAYOUT, nodeParameter_.outChannels},
...
int32_t AudioSuiteSoundFieldNode::PrintNodeInfo(std::string &nodeInfo)
{
    std::string optionValue = "";
    int32_t ret = GetOptions("SoundFieldType", optionValue);
    CHECK_AND_RETURN_RET_LOG((ret == SUCCESS && !optionValue.empty()), ERROR, "GetOptions Error");
    nodeInfo = "    Sound Field: " +
        AudioSuiteUtil::FormatOptionsValue("SoundFieldType", optionValue) + "\n";
    return SUCCESS;
}
