...
int32_t AudioSuiteEnvNode::Init()
{
    ...
    algoInterface_ =
        AudioSuiteAlgoInterface::CreateAlgoInterface(AlgoType::AUDIO_NODE_TYPE_ENVIRONMENT_EFFECT, nodeParameter_);
    ...
    SetAudioNodeFormat(AudioFormat{{ENV_ALGO_CHANNEL_LAYOUT, nodeParameter_.inChannels},
        static_cast<AudioSampleFormat>(nodeParameter_.inFormat),
        static_cast<AudioSamplingRate>(nodeParameter_.inSampleRate),
        {ENV_ALGO_CHANNEL_LAYOUT, nodeParameter_.outChannels},
...
int32_t AudioSuiteEnvNode::PrintNodeInfo(std::string &nodeInfo)
{
    std::string optionValue = "";
    int32_t ret = GetOptions("EnvironmentType", optionValue);
    CHECK_AND_RETURN_RET_LOG((ret == SUCCESS && !optionValue.empty()), ERROR, "GetOptions Error");
    nodeInfo = "    Environment: " +
        AudioSuiteUtil::FormatOptionsValue("EnvironmentType", optionValue) + "\n";
    return SUCCESS;
}
