...
static constexpr uint32_t RESAMPLE_QUAILTY = 1;
...
int32_t AudioSuiteFormatConversion::ChannelConvert(AudioSuitePcmBuffer *in, AudioSuitePcmBuffer *out)
{
    ...
    CHECK_AND_RETURN_RET_LOG(
        (inChannelCount != 0) && (outChannelCount != 0) && (in->GetChannelLayout() != out->GetChannelLayout()),
        ERROR,
        "Do ChannelConvert error: invalid input, inChannelCount: %{public}u outChannelCount: %{public}u",
        inChannelCount,
        outChannelCount);
    ...
    AudioSuitePcmBuffer *inPcmBuffer, PcmBufferFormat &outFormat)
{
    ...
    if (in->GetChannelLayout() != outFormat.channelLayout) {
