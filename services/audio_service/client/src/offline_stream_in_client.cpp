...
int32_t OfflineStreamInClient::GetParamOfflineEffectChain(std::vector<uint8_t> &param)
{
    CHECK_AND_RETURN_RET_LOG(streamProxy_ != nullptr, ERR_OPERATION_FAILED, "Get parameter failed with null ipcProxy.");
    return streamProxy_->GetParamOfflineEffectChain(param);
}
