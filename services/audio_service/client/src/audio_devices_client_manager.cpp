    ...
    const int32_t audioDeviceSelectMode) const
{
    ...
    CHECK_AND_RETURN_RET_LOG(audioDeviceSelectMode == SELECT_STRATEGY_DEFAULT ||
        audioDeviceSelectMode == SELECT_STRATEGY_INDEPENDENT ||
        audioDeviceSelectMode == SELECT_STRATEGY_STREAM, ERR_INVALID_PARAM, "invalid audioDeviceSelectMode.");
...
int32_t AudioDevicesClientManager::RestoreOutputDevice(sptr<AudioRendererFilter> audioRendererFilter) const
{
    CHECK_AND_RETURN_RET_LOG(audioRendererFilter != nullptr, ERR_INVALID_PARAM, "invalid parameter");
    AUDIO_DEBUG_LOG("[%{public}d] RestoreOutputDevice: uid<%{public}d>", getpid(), audioRendererFilter->uid);
...
int32_t AudioDevicesClientManager::SetDeviceActive(DeviceType deviceType, bool flag, const int32_t clientUid) const
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(IsActiveDeviceType(deviceType), ERR_NOT_SUPPORTED,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            ERR_PLAY_DEVICE_SWITCH_INVALID_PARAM, "SetDeviceActive device not supported", true),
        "SetDeviceActive device=%{public}d not supported", deviceType);

    /* Call Audio Policy SetDeviceActive */
    int32_t ret = AudioPolicyManager::GetInstance().SetDeviceActive(
        static_cast<InternalDeviceType>(deviceType), flag, uid);
    CHECK_AND_CALL_FUNC_RETURN_RET(ret == SUCCESS, ret,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            ERR_PLAY_DEVICE_SWITCH_OPERATION_FAILED, "SetDeviceActive failed", true));
    return ret;
}
