...
static const int32_t LIMIT_FAST_COUNT = 2;
}
...
public:
    ...
    bool GetAudioTime(int64_t &framePos, int64_t &sec, int64_t &nanoSec) override;
    ...
    int32_t SetMuteHint(bool mute) override;
    ...
    int32_t RegisterRemoteDiedCallback(const std::shared_ptr<RemoteDiedCallback> &callback) override;

    int32_t UnregisterRemoteDiedCallback() override;
    ...
    void SetVoipNoPrivacyFlag(bool voipNoPrivacyFlag) override;
    ...
    void CheckInnerCapVoIP() override;
...
private:
    ...
    sptr<IRemoteObject> ipcProxy_ = nullptr;
    ...
    std::atomic<bool> needNotifyStreamSilentChange_ = true;
};
    ...
    : processProxy_(ipcProxy), isVoipMmap_(isVoipMmap)
{
    ...
    ipcProxy_ = processProxy_->AsObject();
}
    ...
    std::weak_ptr<FastAudioStream> weakStream)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(config.audioMode != AUDIO_MODE_PLAYBACK || ret, nullptr,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_OPERATION_FAILED, "CheckIfSupport failed", true),
        "CheckIfSupport failed!");
    sptr<IStandardAudioService> gasp = AudioProcessInClientInner::GetAudioServerProxy();
    CHECK_AND_CALL_FUNC_RETURN_RET_REPORT(gasp != nullptr, nullptr,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_SERVICE_UNAVAILABLE, "Create failed, can not get service", true),
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(errorCode == SUCCESS, nullptr,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_OPERATION_FAILED, "failed with create audio stream", true),
        "failed with create audio stream fail.");
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ipcProxy != nullptr, nullptr,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_NULL_POINTER, "Create failed with null ipcProxy", true),
        "Create failed with null ipcProxy.");
    sptr<IAudioProcess> iProcessProxy = iface_cast<IAudioProcess>(ipcProxy);
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(iProcessProxy != nullptr, nullptr,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_NULL_POINTER, "Create failed when iface_cast", true),
        "Create failed when iface_cast.");
...
bool AudioProcessInClientInner::GetAudioTime(int64_t &framePos, int64_t &sec, int64_t &nanoSec)
{
    ...
    framePos = serverHandlePos > static_cast<uint64_t>(INT64_MAX) ?
        INT64_MAX : static_cast<int64_t>(serverHandlePos);
...
int32_t AudioProcessInClientInner::SetMuteHint(bool mute)
{
    CHECK_AND_RETURN_RET_LOG(processProxy_ != nullptr, ERR_OPERATION_FAILED, "processProxy_ is null");
    return processProxy_->SetMuteHint(mute);
}
...
int32_t AudioProcessInClientInner::RegisterRemoteDiedCallback(const std::shared_ptr<RemoteDiedCallback> &callback)
{
...
int32_t AudioProcessInClientInner::UnregisterRemoteDiedCallback()
{
    std::lock_guard<std::mutex> lock(deathRecipientLock_);
...
int64_t AudioProcessInClientInner::GetFramesRead()
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(audioBuffer_ != nullptr, -1,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            RECORD_QUERY_ILLEGAL_STATE, "buffer is null, maybe not inited", true),
        "buffer is null, maybe not inited.");
...
void AudioProcessInClientInner::SetPreferredFrameSize(int32_t frameSize, bool isRecreate)
{
    ...
    if (tmp <= originalSpanSizeInFrame) {
        if (processConfig_.rendererInfo.originalFlag == AUDIO_FLAG_ULTRA_FAST && isRecreate) {
            clientSpanSizeInFrame_ = tmp;
        } else {
    ...
    AUDIO_INFO_LOG("Preferred frame size: %{public}d, spanSizeInFrame_: %{public}u, isRecreate: %{public}d, "
        "callbackBuffer size:%{public}zu", frameSize, spanSizeInFrame_, isRecreate, clientSpanSizeInByte_);
...
{
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(processProxy_ != nullptr, false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_NULL_POINTER, "Init failed with null ipcProxy", true),
        "Init failed with null ipcProxy.");
    processCbImpl_ = sptr<ProcessCbImpl>::MakeSptr(shared_from_this());
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(processProxy_->RegisterProcessCb(processCbImpl_) == SUCCESS, false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_OPERATION_FAILED, "RegisterProcessCb failed", true),
        "RegisterProcessCb failed.");

    int32_t ret = processProxy_->ResolveBufferBaseAndGetServerSpanSize(audioBuffer_, spanSizeInFrame_);
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ret == SUCCESS && audioBuffer_ != nullptr, false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_OPERATION_FAILED, "Init failed to call ResolveBuffer", true),
        "Init failed to call ResolveBuffer");
    streamStatus_ = audioBuffer_->GetStreamStatus();
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(streamStatus_ != nullptr, false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_NULL_POINTER, "Init failed, access buffer failed", true),
        "Init failed, access buffer failed.");
    ...
    callbackBuffer_ = std::make_unique<uint8_t[]>(clientSpanSizeInByte_);
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(callbackBuffer_ != nullptr, false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_MEMORY_ALLOC_FAILED, "Init callbackBuffer_ failed", true),
        "Init callbackBuffer_ failed.");
    memset_s(callbackBuffer_.get(), clientSpanSizeInByte_, 0, clientSpanSizeInByte_);
...
void AudioProcessInClientInner::InitPlaybackThread(std::weak_ptr<FastAudioStream> weakStream)
{
    ...
    callbackLoop_ = std::thread([this, weakStream] {
        ...
        if (IsUltraFastStreamAndRoute(processConfig_.ultraFastFlag)) {
            BindBigAndMidCore();
...
bool AudioProcessInClientInner::Init(const AudioProcessConfig &config, std::weak_ptr<FastAudioStream> weakStream)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(isBufferInited, isBufferInited,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_OPERATION_FAILED, "init audio buffer fail", true),
        "%{public}s init audio buffer fail.", __func__);

    bool ret = handleTimeModel_.ConfigSampleRate(processConfig_.streamInfo.samplingRate);
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ret != false, false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_OPERATION_FAILED, "Init LinearPosTimeModel failed", true),
        "Init LinearPosTimeModel failed.");
...
void AudioProcessInClientInner::NotifyStreamSilentChange()
{
    CHECK_AND_RETURN(needNotifyStreamSilentChange_.load() && volumeDataCount_ > 0);
    int64_t stamp = ClockTime::GetCurNano();
    Trace trace("AudioProcessInClientInner::NotifyStreamSilentChange " + std::to_string(sessionId_));
...
{
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(audioBuffer_ != nullptr, ERR_INVALID_HANDLE,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            RECORD_SEND_DATA_ILLEGAL_STATE, "audio buffer is null", true),
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ret == SUCCESS && (ringBuffer.dataLength >= spanSizeInByte_),
        ERR_OPERATION_FAILED,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            RECORD_SEND_DATA_OPERATION_FAILED, "get client mmap read buffer failed", true),
        "get client mmap read buffer failed, ret %{public}d.", ret);
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ret == SUCCESS, ERR_OPERATION_FAILED,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            RECORD_SEND_DATA_OPERATION_FAILED, "memcpy fail", true),
        "%{public}s memcpy fail, ret %{public}d," " spanSizeInByte %{public}zu.", __func__, ret, spanSizeInByte_);
...
int32_t AudioProcessInClientInner::GetBufferDesc(BufferDesc &bufDesc)
{
    CHECK_AND_CALL_FUNC_RETURN_RET_REPORT(isInited_, ERR_ILLEGAL_STATE,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_CREATE_OPERATION_FAILED, "GetBufferDesc not inited", true),
...
int32_t AudioProcessInClientInner::Start()
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(isInited_, ERR_ILLEGAL_STATE,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_START_ILLEGAL_STATE, "not inited", true),
        "not inited!");
    volumeDataCount_ = 0;
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(
        ...
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_START_ILLEGAL_STATE, "Start failed, invalid status:" + GetStatusInfo(targetStatus), true),
        "Start failed, invalid status: %{public}s", GetStatusInfo(targetStatus).c_str());

    if (processProxy_->Start() != SUCCESS) {
        streamStatus_->store(StreamStatus::STREAM_IDEL);
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_START_OPERATION_FAILED, "Start failed to call process proxy", true);
...
int32_t AudioProcessInClientInner::Pause(bool isFlush)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(isInited_, ERR_ILLEGAL_STATE,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_PAUSE_ILLEGAL_STATE, "not inited", true),
        "not inited!");
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(
        ret, ERR_ILLEGAL_STATE,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_PAUSE_ILLEGAL_STATE, "Pause failed, invalid status:" + GetStatusInfo(targetStatus), true),
        "Pause failed, invalid status : %{public}s", GetStatusInfo(targetStatus).c_str());
    ClockTime::RelativeSleep(MAX_STOP_FADING_DURATION_NANO);
    if (processProxy_->Pause(isFlush) != SUCCESS) {
        streamStatus_->store(StreamStatus::STREAM_RUNNING);
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_PAUSE_OPERATION_FAILED, "Pause failed to call process proxy", true);
...
int32_t AudioProcessInClientInner::Resume()
{
    ...
    if (!streamStatus_->compare_exchange_strong(pausedStatus, StreamStatus::STREAM_STARTING) &&
        ...
        startFadein_.store(false);
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_START_ILLEGAL_STATE, "Resume failed, invalid status:" + GetStatusInfo(stoppedStatus), true);
    ...
    if (processProxy_->Resume() != SUCCESS) {
        streamStatus_->store(StreamStatus::STREAM_PAUSED);
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_START_OPERATION_FAILED, "Resume failed to call process proxy", true);
...
int32_t AudioProcessInClientInner::Stop(AudioProcessStage stage)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(isInited_, ERR_ILLEGAL_STATE,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_STOP_ILLEGAL_STATE, "not inited", true),
        "not inited!");
    std::lock_guard<std::mutex> lock(statusSwitchLock_);
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(
        oldStatus != STREAM_IDEL && oldStatus != STREAM_RELEASED && oldStatus != STREAM_INVALID,
        ERR_ILLEGAL_STATE,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_STOP_ILLEGAL_STATE, "Stop failed, invalid status:" + GetStatusInfo(oldStatus), true),
        "Stop failed, invalid status : %{public}s", GetStatusInfo(oldStatus).c_str());
    ...
    if (processProxy_->Stop(stage) != SUCCESS) {
        ...
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_STOP_OPERATION_FAILED, "Stop failed in server", true);
...
int32_t AudioProcessInClientInner::Release(bool isSwitchStream)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(isInited_, ERR_ILLEGAL_STATE,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_RELEASE_OPERATION_FAILED, "not inited", true),
        "not inited!");
    ...
    if (processProxy_->Release(isSwitchStream) != SUCCESS) {
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            PLAY_RELEASE_OPERATION_FAILED, "Release may failed in server", true);
...
void AudioProcessInClientInner::CallClientHandleCurrent()
{
    ...
    if (stamp + ONE_MILLISECOND_DURATION > limit) {
        if (stamp + ONE_MILLISECOND_DURATION > LIMIT_FAST_COUNT * limit) {
            processProxy_->UpdateUnderrunInfo(UNDERRUN_TYPE_ISTIMEOUT, true);
        }
...
void AudioProcessInClientInner::UpdateHandleInfo(bool isAysnc, bool resetReadWritePos)
{
    ...
    if (resetReadWritePos) {
        ...
        int32_t ret = audioBuffer_->ResetCurReadWritePos(nextWritePos, nextWritePos, false);
...
bool AudioProcessInClientInner::RecordProcessCallbackFuc(uint64_t &curReadPos, int64_t clientReadCost)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(recordFinish == SUCCESS, true,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            RECORD_SEND_DATA_OPERATION_FAILED, "finish handle current fail", true),
        "finish handle current fail.");
...
{
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(processProxy_ != nullptr && audioBuffer_ != nullptr, ERR_INVALID_HANDLE,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            RECORD_QUERY_INVALID_HANDLE, "process proxy or audio buffer is null", true),
    ...
    while (tryTimes > 0) {
        ...
        CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ret == SUCCESS, ret,
            StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
                RECORD_QUERY_OPERATION_FAILED, "request handle info fail", true),
            "%{public}s request handle info fail, ret %{public}d.", __func__, ret);

        CHECK_AND_CALL_FUNC_RETURN_RET_LOG(audioBuffer_->GetHandleInfo(serverHandlePos, serverHandleTime),
            ERR_OPERATION_FAILED,
            StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
                RECORD_QUERY_OPERATION_FAILED, "get handle info fail", true),
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ret == SUCCESS, ret,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            RECORD_QUERY_OPERATION_FAILED, "set curReadPos fail", true),
        "%{public}s set curReadPos fail, ret %{public}d.", __func__, ret);
    return SUCCESS;
}
...
int32_t AudioProcessInClientInner::RecordFinishHandleCurrent(uint64_t &curReadPos, int64_t &clientReadCost)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ret == SUCCESS, ret,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            RECORD_SEND_DATA_OPERATION_FAILED, "set next hand frame fail", true),
        "%{public}s set next hand frame %{public}" PRIu64" fail, " "ret %{public}d.", __func__, nextWritePos, ret);
...
void AudioProcessInClientInner::SetVoipNoPrivacyFlag(bool voipNoPrivacyFlag)
{
    CHECK_AND_RETURN_LOG(processProxy_ != nullptr, "SetVoipNoPrivacyFlag processProxy_ is nullptr");
    processProxy_->SetVoipNoPrivacyFlag(voipNoPrivacyFlag);
}

void AudioProcessInClientInner::CheckInnerCapVoIP()
{
    sptr<IStandardAudioService> gasp = AudioProcessInClientInner::GetAudioServerProxy();
...
int32_t AudioProcessInClientInner::SetPitch(float pitch)
{
    CHECK_AND_RETURN_RET_LOG(processConfig_.rendererInfo.isStatic, ERR_INCORRECT_MODE, "not support!");
    CHECK_AND_RETURN_RET_LOG(processProxy_ != nullptr, ERR_NULL_POINTER, "processProxy_ is nullptr");

    return processProxy_->SetStaticPitch(pitch);
}
} // namespace AudioStandard
} // namespace OHOS