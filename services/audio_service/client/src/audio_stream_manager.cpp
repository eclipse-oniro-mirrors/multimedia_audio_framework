...
{
    return AudioPolicyManager::GetInstance().IsMultichannelPlaybackSupported(streamInfo, streamUsage);
}

bool AudioStreamManager::IsDirectPlaybackSupported(AudioStreamInfo &streamInfo, StreamUsage streamUsage)
{
    return AudioPolicyManager::GetInstance().IsDirectPlaybackSupported(streamInfo, streamUsage);
}

bool AudioStreamManager::IsOffloadPlaybackSupported(AudioStreamInfo &streamInfo, StreamUsage streamUsage)
{
    return AudioPolicyManager::GetInstance().IsOffloadPlaybackSupported(streamInfo, streamUsage);
}

int32_t AudioStreamManager::SetOffloadAllowedForUid(uint32_t uid, bool allowed)
{
    return AudioPolicyManager::GetInstance().SetOffloadAllowedForUid(uid, allowed);
}
...
int32_t AudioStreamManager::SetAudioRouteSelectorCallback(const std::shared_ptr<AudioRouteSelector> &callback)
{
    AUDIO_INFO_LOG("In");
    CHECK_AND_RETURN_RET_LOG(callback != nullptr, ERR_INVALID_PARAM, "callback is nullptr");
    return AudioPolicyManager::GetInstance().SetAudioRouteSelectorCallback(callback);
}

int32_t AudioStreamManager::UnsetAudioRouteSelectorCallback()
{
    AUDIO_INFO_LOG("In");
    return AudioPolicyManager::GetInstance().UnsetAudioRouteSelectorCallback();
}
} // namespace AudioStandard
} // namespace OHOS
