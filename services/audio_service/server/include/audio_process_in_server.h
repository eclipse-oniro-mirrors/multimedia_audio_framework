...
public:
    ...
    int32_t Stop(int32_t stage = AUDIO_PROC_STAGE_STOP) override;
    ...
    int32_t SetVoipNoPrivacyFlag(bool voipNoPrivacyFlag) override;
    ...
    void NotifyMuteStateChange(bool muteFlag);
    ...
    int32_t SaveAdjustStreamVolumeInfo(float volume, uint32_t sessionId,
        const std::string &adjustTime, uint32_t code) override;
    ...
    int32_t UpdateUnderrunInfo(uint32_t underrunInfoKey, int32_t underrunInfoVal) override;
    int32_t ResetStaticPlayPosition() override;
    int32_t SetMuteHint(bool mute) override;
    void ReportExceptionForFast();
    int32_t SetStaticPitch(float pitch) override;
public:
