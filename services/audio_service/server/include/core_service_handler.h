...
public:
    ...
    int32_t UpdateSessionOperation(uint32_t sessionId, SessionOperation operation,
        SessionOperationMsg opMsg = SESSION_OP_MSG_DEFAULT, bool needRemoveFromMap = false);
    ...
    int32_t RecordBootStateTime(int32_t state, int32_t uid, int64_t curTime);
    void NotifySessionStandbyStatus(uint32_t sessionId, bool standbyStatus);
private:
    CoreServiceHandler();
