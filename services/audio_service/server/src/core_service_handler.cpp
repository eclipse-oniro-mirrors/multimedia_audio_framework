    ...
    SessionOperationMsg opMsg, bool needRemoveFromMap)
{
    ...
    return iCoreServiceProvider_->UpdateSessionOperation(sessionId, operation, opMsg, needRemoveFromMap);
}
...
int32_t CoreServiceHandler::RecordBootStateTime(int32_t state, int32_t uid, int64_t curTime)
{
    CHECK_AND_RETURN_RET(uid == BOOT_ANIMATION_UID, SUCCESS);
    CHECK_AND_RETURN_RET_LOG(iCoreServiceProvider_ != nullptr, ERROR, "iCoreServiceProvider_ is nullptr");
    return iCoreServiceProvider_->RecordBootStateTime(state, uid, curTime);
}
...
int32_t CoreServiceHandler::SetCapturerMuteHint(uint32_t sessionId, bool mute)
{
    CHECK_AND_RETURN_RET_LOG(iCoreServiceProvider_ != nullptr, ERROR, "iCoreServiceProvider_ is nullptr");
    return iCoreServiceProvider_->SetCapturerMuteHint(sessionId, mute);
}

void CoreServiceHandler::NotifySessionStandbyStatus(uint32_t sessionId, bool standbyStatus)
{
    CHECK_AND_RETURN_LOG(iCoreServiceProvider_ != nullptr, "iCoreServiceProvider_ is nullptr");
    iCoreServiceProvider_->NotifySessionStandbyStatus(sessionId, standbyStatus);
}
} // namespace AudioStandard
} // namespace OHOS
