...
void AudioEndpointInner::Release(bool releaseSink)
{
...
bool AudioEndpointInner::Config(const AudioEndpointConfig &endpointConfig)
{
    ...
    if (!ConfigOutputPoint(endpointConfig)) {
        HdiAdapterManager::GetInstance().ReleaseId(fastRenderId_);
        return false;
    }
    bool ret = readTimeModel_.ConfigSampleRate(dstStreamInfo_.samplingRate);
    CHECK_AND_RETURN_RET_LOG(ret != false, false, "Config LinearPosTimeModel failed.");
    StartThread(InitSinkAttr(endpointConfig));
    return true;
}

bool AudioEndpointInner::ConfigOutputPoint(const AudioEndpointConfig &endpointConfig)
{
    std::shared_ptr<IAudioRenderSink> sink = GetFastSink(deviceInfo_, endpointType_);
    CHECK_AND_RETURN_RET_LOG(sink != nullptr, false, "sink is null, get fast sink fail");

    if (sink->IsInited() && deviceInfo_.deviceType_ == DEVICE_TYPE_USB_ARM_HEADSET) {
        AUDIO_WARNING_LOG("need DeInit, sink is inited, device is arm usb");
        sink->DeInit();
    }

    IAudioSinkAttr attr = InitSinkAttr(endpointConfig);
    if (!sink->IsInited()) {
        AUDIO_INFO_LOG("need Init sink:%{public}u", fastRenderId_);
        sink->Init(attr);
        CHECK_AND_RETURN_RET_LOG(sink->IsInited(), false, "Init sink fail");
    }

    if (PrepareDeviceBuffer(deviceInfo_) != SUCCESS) {
        AUDIO_ERR_LOG("PrepareDeviceBuffer fail, nedd DeInit sink");
        sink->DeInit();
        return false;
    }
    AudioEndpointSinkAdapter::GetInstance()->AddOperation(fastRenderId_,
        endpointName_, GetStatus());
...
IAudioSinkAttr AudioEndpointInner::InitSinkAttr(const AudioEndpointConfig &endpointConfig)
{
    ...
    if (endpointConfig.isUltraFast) {
        isEndpointUltraFast_ = true;
        adapterType_ = ADAPTER_TYPE_ULTRA_FAST;
...
int32_t AudioEndpointInner::PrepareDeviceBuffer(const AudioDeviceDescriptor &deviceInfo)
{
    ...
    // To prevent audio pop in ultra fast scenarios, the minimum ahead time is consistent with normal fast(3ms)
    temp = isEndpointUltraFast_ && temp < ULTRA_FAST_MINIMUM_AHEAD_TIME_NS ? ULTRA_FAST_MINIMUM_AHEAD_TIME_NS : temp;
    int64_t setTime = -1;
...
bool AudioEndpointInner::StartDevice(EndpointStatus preferredState, int64_t delayStopTime)
{
    ...
    delayStopTime_.store(delayStopTime == INVALID_DELAY_STOP_HDI_TIME_NO_RUNNING_NS
        ? delayStopTime_.load() : delayStopTime);
    listLock.unlock();
    return true;
}
...
int32_t AudioEndpointInner::OnStart(IAudioProcessStream *processStream)
{
    ...
    delayStopTime_.store(INT64_MAX);
    return SUCCESS;
}
...
int32_t AudioEndpointInner::OnPause(IAudioProcessStream *processStream)
{
    ...
    if (endpointStatus_ == IDEL) {
        ...
        delayStopTime_.store(ClockTime::GetCurNano() + ((audioMode == AUDIO_MODE_PLAYBACK)
            ? PLAYBACK_DELAY_STOP_HDI_TIME_NS : RECORDER_DELAY_STOP_HDI_TIME_NS));
    }
    return SUCCESS;
}

int32_t AudioEndpointInner::OnStreamMute(IAudioProcessStream *processStream, bool mute)
{
    if (processStream != nullptr) {
        NotifyStreamChange(STREAM_CHANGE_TYPE_STATE_CHANGE, processStream, RENDERER_RUNNING, mute);
    }
    return SUCCESS;
}
...
int32_t AudioEndpointInner::UnlinkProcessStream(IAudioProcessStream *processStream)
{
    ...
    if (processList_.size() == 0) {
        ...
        UpdateEndpointStatus(IDEL);
        delayStopTime_.store(DELAY_STOP_HDI_TIME_WHEN_NO_RUNNING_NS);
    }
...
bool AudioEndpointInner::CheckAllBufferReady(int64_t checkTime, uint64_t curWritePos)
{
    ...
    needCheckZeroVolume_ = true;
    {
        ...
        for (size_t i = 0; i < processBufferList_.size(); i++) {
            ...
            needCheckZeroVolume_ = keepRunning ? false : needCheckZeroVolume_;
...
AudioEndpointInner::VolumeResult AudioEndpointInner::CalculateVolume(size_t i)
{
    ...
    float systemAppVolume = AudioVolume::GetInstance()->GetSystemAppVolume(appUid, streamType);
    ...
    float baseVolume = volumeFromOhaudioBuffer * appVolume * systemAppVolume * doNotDisturbStatusVolume * mdmMuteFactor;
...
void AudioEndpointInner::AsyncGetPosTime()
{
    ...
    while (!stopUpdateThread_) {
        ...
        {
            std::unique_lock<std::mutex> lock(startStatusLock_);
            if (endpointStatus_ == IDEL && isStarted_ && ClockTime::GetCurNano() > delayStopTime_.load()) {
...
bool AudioEndpointInner::KeepWorkloopRunning()
{
    ...
    switch (endpointStatus_.load()) {
        case RUNNING:
            return true;
        case IDEL:
            if (ClockTime::GetCurNano() > delayStopTime_.load()) {
...
bool AudioEndpointInner::IsEndpointUltraFast() const
{
    return isEndpointUltraFast_;
}
...
void AudioEndpointInner::NotifyStreamChange(StreamChangeType change,
    IAudioProcessStream *processStream, RendererState state, bool mute)
{
    ...
    if (audioMode_ == AUDIO_MODE_PLAYBACK) {
        ...
        source->NotifyStreamChangeToSource(change, processStream->GetAudioSessionId(), processStream->GetSource(),
            static_cast<CapturerState>(state), processStream->GetAppInfo().appUid, mute);
    }
}
} // namespace AudioStandard
} // namespace OHOS
