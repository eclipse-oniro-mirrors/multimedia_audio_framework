...
static void SetAudioSceneForAllSource(AudioScene audioScene, bool scoExcludeFlag)
{
    ...
    for (const auto& config : sourceConfigs) {
        ...
        if (source != nullptr && source->IsInited()) {
            source->SetAudioScene(audioScene, scoExcludeFlag);
        }
    }
    ...
    if (fastSource != nullptr && fastSource->IsInited()) {
        fastSource->SetAudioScene(audioScene, scoExcludeFlag);
    }
    ...
    if (fastVoipSource != nullptr && fastVoipSource->IsInited()) {
        fastVoipSource->SetAudioScene(audioScene, scoExcludeFlag);
    }
#endif
}
    ...
    const std::vector<std::pair<std::string, std::string>> &kvpairs)
{
    ...
    for (const auto &kvPair : kvpairs) {
        ...
        nlohmann::json json;
        CHECK_AND_BREAK_LOG(nlohmann::json::accept(kvPair.second) == true,
            "Failed to parse dfx: invalid JSON format");
        json = nlohmann::json::parse(kvPair.second);
        CHECK_AND_BREAK_LOG((json.contains("stage") && json.contains("result") && json.contains("errCode") &&
            json["stage"].is_string() && json["result"].is_string() && json["errCode"].is_string()),
            "Failed to parse update_wakeup_trace_result: contains or type check error");
        std::string stageStr = json["stage"].get<std::string>();
        std::string resultStr = json["result"].get<std::string>();
        std::string errCodeStr = json["errCode"].get<std::string>();
    ...
    const std::vector<StringPair> &kvpairs)
{
    ...
    // Framework-only handlers that update local service state without forwarding to HDI.
    ...
    // Keys below are translated and forwarded to HDI adapters.
    ...
    const std::vector<std::pair<std::string, std::string>> &kvpairs)
{
    ...
    return HdiAdapterManager::GetInstance().SetExtraParameters(HDI_DEVICE_MANAGER_TYPE_LOCAL, "primary", "", value);
}
    ...
    const std::vector<std::string> &subKeys, std::vector<std::pair<std::string, std::string>> &result)
{
    ...
    if (mainKey == EFFECT_LIVE_KEY) {
        ...
        return HdiAdapterManager::GetInstance().GetExtraParameters(subKeys, result);
    }
    ...
    return HdiAdapterManager::GetInstance().GetExtraParameters(validSubKeys, result);
}
...
int32_t AudioServer::SetAudioScene(int32_t audioScene, int32_t a2dpOffloadFlag, bool scoExcludeFlag)
{
    ...
    return SetAudioSceneInner(audioSceneIn, static_cast<BluetoothOffloadState>(a2dpOffloadFlag), scoExcludeFlag);
}
    ...
    bool scoExcludeFlag)
{
    ...
    SetAudioSceneForAllSource(audioScene, scoExcludeFlag);
    ...
    std::vector<DeviceType> &deviceTypes, const uint32_t pipeId, const std::string &networkId)
{
    std::shared_ptr<IAudioRenderSink> sink = nullptr;
    std::shared_ptr<IAudioCaptureSource> source = nullptr;
    if (pipeId == DEFAULT_PIPE_ID) { // Default pipe id is used for modem communication and wifi-soundbox
    ...
    float &maxAmplitude)
{
    ...
    if (isOutputDevice) {
        ...
        // get normal source maxAmplitude
        maxAmplitude = HdiAdapterManager::GetInstance().GetNormalSourceMaxAmplitude();
        maxAmplitude = (maxAmplitude > fastMaxAmplitude) ? maxAmplitude : fastMaxAmplitude;
    }
...
int32_t AudioServer::SetSinkMuteForSwitchDevice(const std::string &devceClass, int32_t durationUs, bool mute)
{
    ...
    if (isMmap) {
        AudioService::GetInstance()->SetEndpointMuteForSwitchDevice(devceClass, mute);
    }
...
int32_t AudioServer::CheckInnerCapVoIP(uint32_t sessionId)
{
    CHECK_AND_RETURN_RET_LOG(AudioService::GetInstance() != nullptr, ERR_INVALID_OPERATION, "AudioService is nullptr");
    AudioService::GetInstance()->CheckInnerCapVoIP(sessionId);
    return SUCCESS;
}
