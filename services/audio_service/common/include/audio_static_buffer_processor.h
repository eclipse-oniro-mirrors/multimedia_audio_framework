...
static constexpr float PITCH_NORMAL = 1.0f;
...
public:
    ...
    ~AudioStaticBufferProcessor();

    int32_t ProcessBufferSpeed(float speed);
    int32_t ProcessBufferPitch(float pitch);
...
private:
    ...
    std::unique_ptr<uint8_t[]> tempBuffer_ = nullptr;
