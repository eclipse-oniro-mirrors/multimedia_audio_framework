...
{
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(sourceInputNode_, nullptr,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            ...
            "sourceInputNode_ is nullptr", false),
        "sourceInputNode_ is nullptr");
...
OutputPort<HpaePcmBuffer *> *HpaeSourceInputCluster::GetOutputPort(HpaeNodeInfo &nodeInfo, bool isDisConnect)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(SafeGetMap(fmtConverterNodeMap_, preNodeKey),
        sourceInputNode_->GetOutputPort(nodeInfo),
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_STATE_INCONSISTENT, OperationType::RECORD,
                BusinessScenario::QUERY, ERR_NULL_POINTER),
            "not find the preNodeKey = " + preNodeKey, false),
...
{
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(sourceInputNode_, ERR_ILLEGAL_STATE,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_NO_SOUND, OperationType::RECORD,
                BusinessScenario::QUERY, ERR_NULL_POINTER),
            "sourceInputNode_ is nullptr", false),
        "sourceInputNode_ is nullptr");
    return sourceInputNode_->GetCapturerSourceInstance(deviceClass, deviceNetId, sourceType, sourceName, busAddress);
}
...
{
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(sourceInputNode_, STREAM_MANAGER_INVALID,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_STATE_INCONSISTENT, OperationType::RECORD,
                BusinessScenario::QUERY, ERR_NULL_POINTER),
            "sourceInputNode_ is nullptr", false),
...
{
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(sourceInputNode_, 0,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_STATE_INCONSISTENT, OperationType::RECORD,
                BusinessScenario::QUERY, ERR_NULL_POINTER),
            "sourceInputNode_ is nullptr", false),
...
{
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(sourceInputNode_, HPAE_SOURCE_DEFAULT,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_STATE_INCONSISTENT, OperationType::RECORD,
                BusinessScenario::QUERY, ERR_NULL_POINTER),
            "sourceInputNode_ is nullptr", false),
    ...
    uint32_t sessionId, SourceType source, CapturerState state, uint32_t appUid, bool mute, bool mixWithWakeUp)
{
    ...
    sourceInputNode_->NotifyStreamChangeToSource(change, sessionId, source, state, appUid, mute, mixWithWakeUp);
}
}  // namespace HPAE
}  // namespace AudioStandard
}  // namespace OHOS
