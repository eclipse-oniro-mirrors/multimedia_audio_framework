...
// 支持协同拾音的 sourceType，激活后映射到 HPAE_SCENE_COLLABORATIVE_RECORD
static std::set<SourceType> g_collabRecordSupportedSourceTypes = {
    SOURCE_TYPE_LIVE,
};
...
static std::unordered_map<HpaeProcessorType, AudioEnhanceScene> g_processorTypeToSceneTypeMap = {
    ...
    {HPAE_SCENE_COLLABORATIVE_RECORD, SCENE_COLLABORATIVE_RECORD},
};
...
static std::unordered_map<std::string, AudioPipeType> g_deviceClassToPipeMap = {
    ...
    {"voip", PIPE_TYPE_OUT_DIRECT_NORMAL},
    {"direct", PIPE_TYPE_OUT_DIRECT_NORMAL},
};
...
bool IsCollabRecordSupported(SourceType sourceType)
{
    return g_collabRecordSupportedSourceTypes.count(sourceType) > 0;
}
...
{
    uint32_t effectiveRate = nodeInfo.GetEffectiveSampleRate();
    return usTime * effectiveRate / TIME_US_PER_S / nodeInfo.frameLen;
}

uint64_t ConvertDatalenToUs(size_t bufferSize, const HpaeNodeInfo &nodeInfo)
{
    uint32_t effectiveRate = nodeInfo.GetEffectiveSampleRate();
    if (nodeInfo.channels == 0 || GetSizeFromFormat(nodeInfo.format) == 0 || effectiveRate == 0) {
    ...
    double seconds = samples / static_cast<double>(effectiveRate);
...
int32_t TransModuleInfoToHpaeSinkInfo(const AudioModuleInfo &audioModuleInfo, HpaeSinkInfo &sinkInfo)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(sinkInfo.channels != CHANNEL_UNKNOW && sinkInfo.format != INVALID_WIDTH, ERROR,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            ...
            "sink channels or format is invalid", false),
        "sink channels or format is invalid");
    ...
    sinkInfo.routeFlag = audioModuleInfo.routeFlag;
    sinkInfo.isUltraFast = (audioModuleInfo.className == "ultra_fast");
    if (audioModuleInfo.needEmptyChunk) {
        sinkInfo.needEmptyChunk = audioModuleInfo.needEmptyChunk.value();
    }
    AUDIO_INFO_LOG("sink info ch: %{public}u, channelLayout: %{public}" PRIu64 ", routeFlag: %{public}u",
        sinkInfo.channels,
...
int32_t TransModuleInfoToHpaeSourceInfo(const AudioModuleInfo &audioModuleInfo, HpaeSourceInfo &sourceInfo)
{
    ...
    sourceInfo.busAddress = audioModuleInfo.busAddress;
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(sourceInfo.channels != CHANNEL_UNKNOW && sourceInfo.format != INVALID_WIDTH,
        ERROR, StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_NO_SOUND, OperationType::RECORD,
                BusinessScenario::SEND_DATA, ERR_INVALID_PARAM),
            "source channels or format is invalid", false),
        "source channels or format is invalid");
    ...
    sourceInfo.micInSamplingRate = static_cast<AudioSamplingRate>(StringToNum(audioModuleInfo.micInRate));
    sourceInfo.micInFormat = TransFormatFromStringToEnum(audioModuleInfo.micInFormat);
    sourceInfo.micInChannels = static_cast<AudioChannel>(StringToNum(audioModuleInfo.micInChannels));
    sourceInfo.routeFlag = audioModuleInfo.routeFlag;
    return SUCCESS;
}
...
bool CheckSourceInfoIsDifferent(const HpaeSourceInfo &info, const HpaeSourceInfo &oldInfo)
{
    auto getKey = [](const HpaeSourceInfo &sourceInfo) {
        return std::tie(
            ...
            sourceInfo.micInSamplingRate,
            sourceInfo.micInFormat,
            sourceInfo.micInChannels,
            sourceInfo.routeFlag);
    };
...
std::string TransFormatFromEnumToString(AudioSampleFormat format)
{
    CHECK_AND_RETURN_RET_LOG(
        g_formatFromParserEnumToStr.find(format) != g_formatFromParserEnumToStr.end(), "",
        "error param format");
...
void ConfigNodeInfo(HpaeNodeInfo &nodeInfo, const HpaeStreamInfo &streamInfo)
{
    ...
    nodeInfo.keepRunning = streamInfo.keepRunning;
}
...
{
    size_t frameLen = static_cast<size_t>(FRAME_LEN_20MS) * sampleRate / MS_PER_SECOND;
    AUDIO_DEBUG_LOG("CalculateFrameLenBySampleRate: unified 20ms, sampleRate=%{public}u, frameLen=%{public}zu",
        sampleRate, frameLen);
    return frameLen;
}
