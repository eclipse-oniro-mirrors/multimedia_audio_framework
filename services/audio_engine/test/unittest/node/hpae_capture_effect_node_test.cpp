...
/*
 * tc.name   : Test SetAppsEnhanceMuteState API
 * tc.type   : FUNC
 * tc.number : HpaeCaptureEffectNodeTest_101
 * tc.desc   : Test SetAppsEnhanceMuteState with valid node
 */
HWTEST_F(HpaeCaptureEffectNodeTest, HpaeCaptureEffectNodeTest_101, TestSize.Level0)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    std::shared_ptr<HpaeCaptureEffectNode> hpaeCaptureEffectNode = std::make_shared<HpaeCaptureEffectNode>(nodeInfo);

    // Test SetAppsEnhanceMuteState without effect created
    // Should return error because sceneKeyCode_ is 0 (not set)
    EXPECT_EQ(hpaeCaptureEffectNode->SetAppsEnhanceMuteState(true), ERROR);
    EXPECT_EQ(hpaeCaptureEffectNode->SetAppsEnhanceMuteState(false), ERROR);
}

/*
 * tc.name   : Test SetAppsEnhanceMuteState API
 * tc.type   : FUNC
 * tc.number : HpaeCaptureEffectNodeTest_102
 * tc.desc   : Test SetAppsEnhanceMuteState with different mute states
 */
HWTEST_F(HpaeCaptureEffectNodeTest, HpaeCaptureEffectNodeTest_102, TestSize.Level0)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    std::shared_ptr<NodeStatusCallback> testStatuscallback = std::make_shared<NodeStatusCallback>();
    ...
    // Create capture effect (this will set sceneKeyCode_)
    uint64_t sceneKeyCode = 12345;
    EXPECT_NE(hpaeCaptureEffectNode->CaptureEffectCreate(sceneKeyCode, attr), 0);

    // Test SetAppsEnhanceMuteState after effect release
    EXPECT_EQ(hpaeCaptureEffectNode->CaptureEffectRelease(sceneKeyCode), 0);
}

/*
 * tc.name   : Test collaborative recording branch
 * tc.type   : FUNC
 * tc.number : HpaeCaptureEffectNodeTest_103
 * tc.desc   : Test SignalProcess with SCENE_COLLABORATIVE_RECORD
 */
HWTEST_F(HpaeCaptureEffectNodeTest, HpaeCaptureEffectNodeTest_103, TestSize.Level1)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    nodeInfo.effectInfo.enhanceScene = SCENE_COLLABORATIVE_RECORD;
    std::shared_ptr<HpaeCaptureEffectNode> hpaeCaptureEffectNode = std::make_shared<HpaeCaptureEffectNode>(nodeInfo);
    EXPECT_EQ(hpaeCaptureEffectNode->Reset(), true);

    PcmBufferInfo pcmBufferInfo(STEREO, DEFAULT_FRAME_LENGTH, SAMPLE_RATE_48000);
    hpaeCaptureEffectNode->outPcmBuffer_ = std::make_unique<HpaePcmBuffer>(pcmBufferInfo);
    uint32_t bufferLength = DEFAULT_FRAME_LENGTH * STEREO * sizeof(float);
    hpaeCaptureEffectNode->micCache_.resize(bufferLength);
    hpaeCaptureEffectNode->micRefCache_.resize(bufferLength);
    hpaeCaptureEffectNode->micBufferLength_ = bufferLength;
    hpaeCaptureEffectNode->micrefBufferLength_ = bufferLength;

    std::vector<HpaePcmBuffer *> inputs;
    HpaePcmBuffer micBuffer(pcmBufferInfo);
    micBuffer.SetSourceBufferType(HPAE_SOURCE_BUFFER_TYPE_MIC);
    inputs.emplace_back(&micBuffer);

    HpaePcmBuffer refBuffer(pcmBufferInfo);
    refBuffer.SetSourceBufferType(HPAE_SOURCE_BUFFER_TYPE_DEFAULT);
    inputs.emplace_back(&refBuffer);

    EXPECT_EQ(hpaeCaptureEffectNode->SignalProcess(inputs), hpaeCaptureEffectNode->outPcmBuffer_.get());
}

/*
 * tc.name   : Test collaborative recording branch with only mic
 * tc.type   : FUNC
 * tc.number : HpaeCaptureEffectNodeTest_104
 * tc.desc   : Test SignalProcessCollaboration with only mic input
 */
HWTEST_F(HpaeCaptureEffectNodeTest, HpaeCaptureEffectNodeTest_104, TestSize.Level1)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    nodeInfo.effectInfo.enhanceScene = SCENE_COLLABORATIVE_RECORD;
...
/*
 * tc.name   : Test collaborative recording branch with only ref
 * tc.type   : FUNC
 * tc.number : HpaeCaptureEffectNodeTest_105
 * tc.desc   : Test SignalProcessCollaboration with only ref input
 */
HWTEST_F(HpaeCaptureEffectNodeTest, HpaeCaptureEffectNodeTest_105, TestSize.Level1)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    nodeInfo.effectInfo.enhanceScene = SCENE_COLLABORATIVE_RECORD;
...
/*
 * tc.name   : Test collaborative recording with CaptureEffectCreate
 * tc.type   : FUNC
 * tc.number : HpaeCaptureEffectNodeTest_106
 * tc.desc   : Test CaptureEffectCreate with SCENE_COLLABORATIVE_RECORD
 */
HWTEST_F(HpaeCaptureEffectNodeTest, HpaeCaptureEffectNodeTest_106, TestSize.Level1)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    nodeInfo.effectInfo.enhanceScene = SCENE_COLLABORATIVE_RECORD;
...
/*
 * tc.name   : Test collaborative recording with different channels
 * tc.type   : FUNC
 * tc.number : HpaeCaptureEffectNodeTest_107
 * tc.desc   : Test CaptureEffectCreate with different channel configs
 */
HWTEST_F(HpaeCaptureEffectNodeTest, HpaeCaptureEffectNodeTest_107, TestSize.Level1)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    nodeInfo.effectInfo.enhanceScene = SCENE_COLLABORATIVE_RECORD;
    ...
    CaptureEffectAttr attr = {
        .micChannels = CHANNEL_4,
        .ecChannels = 0,
        .micRefChannels = CHANNEL_4,
    };
...
/*
 * tc.name   : Test SetCapturerEffectConfig with SCENE_COLLABORATIVE_RECORD
 * tc.type   : FUNC
 * tc.number : HpaeCaptureEffectNodeTest_108
 * tc.desc   : Test SetCapturerEffectConfig covers SCENE_COLLABORATIVE_RECORD branch
 */
HWTEST_F(HpaeCaptureEffectNodeTest, HpaeCaptureEffectNodeTest_108, TestSize.Level1)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    nodeInfo.effectInfo.enhanceScene = SCENE_COLLABORATIVE_RECORD;
    ...
    EXPECT_EQ(nodeInfo1.samplingRate, audioBufferConfig1.samplingRate);
    EXPECT_EQ(nodeInfo1.channels, audioBufferConfig1.channels);
    EXPECT_EQ(nodeInfo1.format, SAMPLE_F32LE);
    ...
    HpaeNodeInfo nodeInfo3;
    EXPECT_EQ(hpaeCaptureEffectNode->GetCapturerEffectConfig(nodeInfo3, HPAE_SOURCE_BUFFER_TYPE_DEFAULT), true);
    EXPECT_EQ(nodeInfo3.samplingRate, audioBufferConfig3.samplingRate);
    EXPECT_EQ(nodeInfo3.channels, audioBufferConfig3.channels);
    EXPECT_EQ(nodeInfo3.format, SAMPLE_F32LE);
}
 ...
 * @tc.desc  : Test CaptureEffectCreate returns ERR_ILLEGAL_STATE when no enhance chain manager
 */
HWTEST_F(HpaeCaptureEffectNodeTest, FaultCode_CaptureEffectCreate_001, TestSize.Level1)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    std::shared_ptr<HpaeCaptureEffectNode> hpaeCaptureEffectNode =
        std::make_shared<HpaeCaptureEffectNode>(nodeInfo);
    CaptureEffectAttr attr = {
        .micChannels = STEREO,
        .ecChannels = STEREO,
        .micRefChannels = STEREO,
    };
    int32_t ret = hpaeCaptureEffectNode->CaptureEffectCreate(TEST_SCENE_KEY_CODE, attr);
    EXPECT_NE(ret, 0);
}

/**
 * @tc.name  : Test GetCapturerEffectConfig fault code
 * @tc.type  : FUNC
 * @tc.number: FaultCode_GetCapturerEffectConfig_001
 * @tc.desc  : Test GetCapturerEffectConfig returns false when config not found
 */
HWTEST_F(HpaeCaptureEffectNodeTest, FaultCode_GetCapturerEffectConfig_001, TestSize.Level1)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    std::shared_ptr<HpaeCaptureEffectNode> hpaeCaptureEffectNode =
    ...
    bool ret = hpaeCaptureEffectNode->GetCapturerEffectConfig(outNodeInfo, HPAE_SOURCE_BUFFER_TYPE_MIC);
    EXPECT_EQ(ret, false);
}

/**
 * @tc.name  : Test SignalProcess fault code
 * @tc.type  : FUNC
 * @tc.number: FaultCode_SignalProcess_001
 * @tc.desc  : Test SignalProcess returns outPcmBuffer_ when processLength is 0
 */
HWTEST_F(HpaeCaptureEffectNodeTest, FaultCode_SignalProcess_001, TestSize.Level1)
{
    HpaeNodeInfo nodeInfo;
    GetTestNodeInfo(nodeInfo);
    std::shared_ptr<HpaeCaptureEffectNode> hpaeCaptureEffectNode =
    ...
    hpaeCaptureEffectNode->outPcmBuffer_ = std::make_unique<HpaePcmBuffer>(pcmBufferInfo);
    std::vector<HpaePcmBuffer *> inputs;
    HpaePcmBuffer ecBuffer(pcmBufferInfo);
    ecBuffer.SetSourceBufferType(HPAE_SOURCE_BUFFER_TYPE_EC);
    inputs.emplace_back(&ecBuffer);
    EXPECT_EQ(hpaeCaptureEffectNode->SignalProcess(inputs), hpaeCaptureEffectNode->outPcmBuffer_.get());
}
}  // namespace HPAE
}  // namespace AudioStandard
