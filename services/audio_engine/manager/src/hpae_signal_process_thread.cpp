...
void HpaeSignalProcessThread::SetFastThread(bool isFast)
{
    isFastThread_ = isFast;
}

void HpaeSignalProcessThread::BindCore()
{
    if (coreBinded_) {
        return;
    }
    ...
    if (ret != 0) {
        HILOG_COMM_ERROR("[BindCore]set target cpu failed, ret: %{public}d", ret);
        return;
    }
