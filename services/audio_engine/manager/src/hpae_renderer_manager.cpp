...
constexpr int64_t UNDERRUN_BYPASS_DURATION_OFFLOAD_NS = 40 * 1000 * 1000; // 40ms
...
constexpr size_t RENDERER_REQUEST_COUNT = CURRENT_REQUEST_COUNT;
constexpr int32_t COLL_ALING_COUNT = 10; // 10 frames = 200 ms
...
DeviceType HpaeRendererManager::GetCurrentSinkDeviceType()
{
    CHECK_AND_RETURN_RET(outputCluster_ != nullptr, DEVICE_TYPE_NONE);
    return outputCluster_->GetCurrentOutputDevice();
}
...
int32_t HpaeRendererManager::CreateInputSession(const HpaeStreamInfo &streamInfo)
{
    ...
    if (IS_DEVICE_ENHANCED_SUPPORTED) {
        nodeInfo.deviceType = static_cast<int32_t>(GetCurrentSinkDeviceType());
        nodeInfo.renderId = renderId_;
    }
...
void HpaeRendererManager::AddSingleNodeToSink(const std::shared_ptr<HpaeSinkInputNode> &node, bool isConnect)
{
    ...
    AdjustFrameLen(nodeInfo, outNodeInfo);
    node->SetNodeInfo(nodeInfo);
    node->SetIsLowLatency(false);
    uint32_t sessionId = nodeInfo.sessionId;

    sinkInputNodeMap_[sessionId] = node;
    TriggerCallback(UPDATE_SPAN_SIZE, sessionId,
        static_cast<uint32_t>(nodeInfo.frameLen), HPAE_STREAM_CLASS_TYPE_PLAY);
...
void HpaeRendererManager::CreateDefaultProcessCluster(HpaeNodeInfo &nodeInfo)
{
    ...
    if (!SafeGetMap(sceneClusterMap_, HPAE_SCENE_DEFAULT)) {
        ...
        temp.renderId = renderId_;
        temp.deviceType = static_cast<int32_t>(GetCurrentSinkDeviceType());
...
void HpaeRendererManager::CreateProcessCluster(HpaeNodeInfo &nodeInfo)
{
    ...
    if (IS_DEVICE_ENHANCED_SUPPORTED) {
        processClusterDecision = AudioEffectChainManager::GetInstance()->CheckProcessClusterInstances(
            sceneType, static_cast<DeviceType>(nodeInfo.deviceType), nodeInfo.renderId);
    } else {
        processClusterDecision = AudioEffectChainManager::GetInstance()->CheckProcessClusterInstances(sceneType);
    }
...
HpaeProcessorType HpaeRendererManager::GetProcessorType(uint32_t sessionId)
{
    ...
    std::string sceneType = TransProcessorTypeToSceneType(nodeInfo.sceneType);
    bool noArmEffect = false;
    if (IS_DEVICE_ENHANCED_SUPPORTED) {
        noArmEffect = !AudioEffectChainManager::GetInstance()->ExistAudioEffectChainArm(
            sceneType, nodeInfo.effectInfo.effectMode,
            static_cast<DeviceType>(nodeInfo.deviceType), nodeInfo.renderId);
    } else {
        noArmEffect = !AudioEffectChainManager::GetInstance()->ExistAudioEffectChainArm(
            sceneType, nodeInfo.effectInfo.effectMode);
    }
    if ((!isSplitProcessorType(nodeInfo.sceneType)) && (sessionNodeMap_[sessionId].bypass || noArmEffect)) {
        return HPAE_SCENE_EFFECT_NONE;
    }
    return nodeInfo.sceneType;
}

void HpaeRendererManager::DisconnectNodeFromEffectChain(HpaeNodeInfo &nodeInfo)
{
    ...
    sceneClusterMap_[HPAE_SCENE_EFFECT_NONE]->AudioRendererCreate(nodeInfo, sinkInfo_);
}

void HpaeRendererManager::SwitchBypassState(const std::shared_ptr<HpaeSinkInputNode> &node,
    HpaeNodeInfo &nodeInfo, int32_t processClusterDecision)
{
    if (node->connectedProcessorType_ == HPAE_SCENE_EFFECT_NONE &&
        ...
        DisconnectNodeFromEffectChain(nodeInfo);
...
void HpaeRendererManager::RefreshProcessClusterByDeviceInner(const std::shared_ptr<HpaeSinkInputNode> &node)
{
    ...
    if ((processClusterDecision != USE_NONE_PROCESSCLUSTER && sessionNodeMap_[nodeInfo.sessionId].bypass) ||
        ...
        SwitchBypassState(node, nodeInfo, processClusterDecision);
    } else if (node->connectedProcessorType_ != GetProcessorType(nodeInfo.sessionId) &&
        node->GetState() == HPAE_SESSION_RUNNING) {
        AUDIO_INFO_LOG("sessionId: %{public}u need to refresh %{public}d", nodeInfo.sessionId, processClusterDecision);
        DisConnectInputSession(nodeInfo.sessionId);
...
int32_t HpaeRendererManager::DeleteProcessCluster(uint32_t sessionId, bool needInitEffect)
{
    if (!SafeGetMap(sinkInputNodeMap_, sessionId)) {
        AUDIO_INFO_LOG("could not find session:%{public}d", sessionId);
        return SUCCESS;
    }
    AudioEffectChainManager::GetInstance()->SetInitEffectBufferFlag(needInitEffect);
    HpaeNodeInfo nodeInfo = sinkInputNodeMap_[sessionId]->GetNodeInfo();
...
void HpaeRendererManager::DisConnectCoBufferFromDeleteProcessCluster(HpaeProcessorType sceneType)
{
    CHECK_AND_RETURN_LOG(sceneType == HPAE_SCENE_COLLABORATIVE && hpaeCoBufferNode_ != nullptr,
        "no need DisConnect CoBufferNode, sceneType: %{public}d", static_cast<int32_t>(sceneType));
    CHECK_AND_RETURN_LOG(SafeGetMap(sceneClusterMap_, sceneType), "sceneCluster is nullptr, sceneType: %{public}d",
        static_cast<int32_t>(sceneType));
    CHECK_AND_RETURN_LOG(sceneClusterMap_[sceneType]->GetPreOutNum() == 0, "no DisConnect, GetPreOutNum: %{public}d",
        static_cast<int32_t>(sceneClusterMap_[sceneType]->GetPreOutNum()));
...
int32_t HpaeRendererManager::ConnectInputSession(uint32_t sessionId)
{
    ...
    CHECK_AND_RETURN_RET_LOG(outputCluster_ != nullptr, ERR_INVALID_PARAM, "outputCluster_ is nullptr");
...
{
    CHECK_AND_RETURN_LOG(sceneType == HPAE_SCENE_COLLABORATIVE && hpaeCoBufferNode_ != nullptr,
        "no need Connect CoBufferNode, sceneType: %{public}d", static_cast<int32_t>(sceneType));
    CHECK_AND_RETURN_LOG(SafeGetMap(sceneClusterMap_, sceneType), "sceneCluster is nullptr, sceneType: %{public}d",
        static_cast<int32_t>(sceneType));
    CHECK_AND_RETURN_LOG(sceneClusterMap_[sceneType]->GetPreOutNum() == 0, "no Connect, GetPreOutNum: %{public}d",
        static_cast<int32_t>(sceneClusterMap_[sceneType]->GetPreOutNum()));
...
void HpaeRendererManager::OnDisConnectProcessCluster(HpaeProcessorType sceneType)
{
    auto request = [this, sceneType]() {
        ...
        if (SafeGetMap(sceneClusterMap_, sceneType) && sceneClusterMap_[sceneType]->GetPreOutNum() == 0) {
            AudioEffectChainManager::GetInstance()->SetInitEffectBufferFlag(false);
            DisConnectOutputCluster(sceneType);
            // for collaboration
...
int32_t HpaeRendererManager::Pause(uint32_t sessionId, bool isStandby)
{
    auto request = [this, sessionId, isStandby]() {
        Trace trace("[" + std::to_string(sessionId) + "]HpaeRendererManager::Pause");
        AUDIO_INFO_LOG("Pause sessionId %{public}u deviceName %{public}s,isStandb:%{public}d",
            sessionId, GetEncryptStr(sinkInfo_.deviceName).c_str(), isStandby);
        CHECK_AND_RETURN_LOG(SafeGetMap(sinkInputNodeMap_, sessionId),
        ...
        StopInterphoneSinkIfNeeded();
    };
    SendRequest(request, __func__);
    return SUCCESS;
}

void HpaeRendererManager::StopInterphoneSinkIfNeeded()
{
    if (sinkInfo_.deviceName.find("Interphone") != std::string::npos) {
        AUDIO_INFO_LOG("Interphone stream paused, stopping output cluster immediately");
        if (outputCluster_ != nullptr) {
            outputCluster_->Stop();
        }
    }
...
int32_t HpaeRendererManager::Flush(uint32_t sessionId)
{
    auto request = [this, sessionId]() {
        ...
        auto converter = sceneClusterMap_[sceneType]->GetConverterNodeById(sessionId);
...
int32_t HpaeRendererManager::ReloadRenderManager(const HpaeSinkInfo &sinkInfo, bool isReload)
{
    ...
    auto request = [this, sinkInfo, isReload]() {
        ReloadRenderManagerInner(sinkInfo, isReload, true);
    };
...
void HpaeRendererManager::ReloadRenderManagerInner(const HpaeSinkInfo &sinkInfo, bool isReload, bool isNeedCallBack)
{
    ...
    for (const auto &it : sinkInputNodeMap_) {
        ...
        DeleteProcessCluster(it.first, false);
    }
    ...
    InitManager(isReload, isNeedCallBack);
...
int32_t HpaeRendererManager::InitManager(bool isReload, bool isNeedCallBack)
{
    HpaeNodeInfo nodeInfo;
    if (auto checkRet = CheckFramelen(sinkInfo_); checkRet != SUCCESS) {
    ...
    nodeInfo.deviceNetId = sinkInfo_.deviceNetId;
    nodeInfo.deviceClass = sinkInfo_.deviceClass;
    nodeInfo.routeFlag = sinkInfo_.routeFlag;
    nodeInfo.statusCallback = weak_from_this();
    if (sinkInfo_.lib == "libmodule-split-stream-sink.z.so") {
    ...
    if (IS_DEVICE_ENHANCED_SUPPORTED) {
        outputCluster_->SetDeviceChangeCallback([this]() {
            RefreshProcessClusterByDevice();
        });
    }
    int32_t ret = InitOutputCluster();
    if (!sceneClusterMap_.count(HPAE_SCENE_EFFECT_NONE) || isReload == true) {
        InitDefaultNodeInfo();
    }
    if (isNeedCallBack == true) {
...
int32_t HpaeRendererManager::InitOutputCluster()
{
    ...
    renderId_ = outputCluster_->GetRenderId();
    AUDIO_INFO_LOG("init sink:%{public}s, renderid:%{public}u", sinkInfo_.deviceName.c_str(), renderId_);
    return ret;
}
...
void HpaeRendererManager::InitDefaultNodeInfo()
{
    ...
    defaultNodeInfo.deviceType = static_cast<int32_t>(GetCurrentSinkDeviceType());
    defaultNodeInfo.renderId = renderId_;
...
void HpaeRendererManager::Process()
{
    ...
    if (outputCluster_ != nullptr && IsRunning()) {
        UpdateAppsUid();
        // no stream running need stop
        if (appsUid_.empty() && !coBufferNodeIsConnected_) {
            // For interphone stream, stop immediately without waiting 3s timeout
            if (sinkInfo_.deviceName.find("Interphone") != std::string::npos) {
                outputCluster_->Stop();
                return;
            }
...
int32_t HpaeRendererManager::SetOffloadPolicy(uint32_t sessionId, int32_t state)
{
    auto request = [this, sessionId, state]() {
        ...
        if (SafeGetMap(sinkInputNodeMap_, sessionId)) {
            ...
            sinkInputNodeMap_[sessionId]->SetOffloadType(state);
        } else {
...
int32_t HpaeRendererManager::HandlePriPaPower(uint32_t sessionId)
{
    ...
    float curVolume = audioVolume->GetVolume(sessionId, nodeInfo.streamType, sinkInfo_.sinkId, &volumes);
...
int32_t HpaeRendererManager::ConnectCoBufferNode(const std::shared_ptr<HpaeCoBufferNode> &coBufferNode)
{
    auto request = [this, coBufferNode]() {
        ...
        if (!coBufferNode->IsOutputClusterConnected()) {
            ...
            coBufferNode->SetDefaultRendererManagerCallBack(weak_from_this());
...
int32_t HpaeRendererManager::DisConnectCoBufferNode(const std::shared_ptr<HpaeCoBufferNode> &coBufferNode)
{
    auto request = [this, coBufferNode]() {
        ...
        if (coBufferNode->IsOutputClusterConnected()) {
            ...
            defaultCoBufferNode_ = coBufferNode;
            coBufferNodeIsConnected_ = false;
            outputCluster_->SetCollaborationState(false);
            AUDIO_INFO_LOG("will disconnectCoBufferNode");
        }
    };
    SendRequest(request, __func__);
    return SUCCESS;
...
void HpaeRendererManager::OnDisConnectCoBufferWaitFadeOutDone()
{
    auto request = [this]() {
        CHECK_AND_RETURN_LOG(defaultCoBufferNode_ != nullptr, "defaultCoBufferNode_ is nullptr");
        CHECK_AND_RETURN_LOG(defaultCoBufferNode_->IsOutputClusterConnected() == false,
            "defaultCoBufferNode_ is connect, dont allow disconnect");
        outputCluster_->DisConnect(defaultCoBufferNode_);
        outputCluster_->SetCollaborationState(false);
        defaultCoBufferNode_ = nullptr;
        coBufferNodeIsConnected_ = false;
...
void HpaeRendererManager::EnableCollaboration()
{
    ...
    int32_t runningCollStreamCount = 0;
    isReloadManagerFromCollStateChange_ = false;
    for (auto& [key, node] : sinkInputNodeMap_) {
        HpaeNodeInfo nodeInfo = node->GetNodeInfo();
        if (nodeInfo.effectInfo.effectScene == SCENE_MOVIE || nodeInfo.effectInfo.effectScene == SCENE_MUSIC) {
            AUDIO_INFO_LOG("move to coll sessionId %{public}u, state: %{public}d", key,
                static_cast<int32_t>(node->GetState()));
            if (node->GetState() == HPAE_SESSION_RUNNING) {
                HpaeProcessorType sceneType = node->connectedProcessorType_;
                std::shared_ptr<HpaeGainNode> sessionGainNode = nullptr;
                CHECK_AND_CONTINUE_LOG(SafeGetMap(sceneClusterMap_, sceneType) &&
                    sceneClusterMap_[sceneType]->GetGainNodeById(key) != nullptr, "NULL");
                sessionGainNode = sceneClusterMap_[sceneType]->GetGainNodeById(key);
                sessionGainNode->SetCollFadeState(OPERATION_PAUSED);
                runningCollStreamCount++;
            } else {
    ...
    if (runningCollStreamCount == 0) {
        ReloadRenderManagerInner(sinkInfo_, true, false);
        isReloadManagerFromCollStateChange_ = true;
        AUDIO_INFO_LOG("runningCollStreamCount: 0, ReloadRenderManager from coll open");
    }
}

void HpaeRendererManager::OnCollborationFadeDone(uint32_t sessionId)
{
    auto request = [this, sessionId]() {
        if (isReloadManagerFromCollStateChange_ == false) {
            ReloadRenderManagerInner(sinkInfo_, true, false);
            isReloadManagerFromCollStateChange_ = true;
            AUDIO_INFO_LOG("ReloadRenderManager finish, sessionId %{public}u", sessionId);
        }
        ReConnectNodeForCollaboration(sessionId);
        CHECK_AND_RETURN_LOG(sinkInputNodeMap_.find(sessionId) != sinkInputNodeMap_.end() &&
            sinkInputNodeMap_[sessionId] != nullptr, "nullptr");
        HpaeProcessorType sceneType = sinkInputNodeMap_[sessionId]->connectedProcessorType_;
        CHECK_AND_RETURN_LOG(SafeGetMap(sceneClusterMap_, sceneType),
            "sceneType no find in sceneClusterMap_, sceneType: %{public}d", static_cast<int32_t>(sceneType));
        CHECK_AND_RETURN_LOG(sceneClusterMap_[sceneType]->GetGainNodeById(sessionId) != nullptr,
            "HpaeGainNode is null, sessionId: %{public}u", sessionId);
        std::shared_ptr<HpaeGainNode> sessionGainNode = sceneClusterMap_[sceneType]->GetGainNodeById(sessionId);
        sessionGainNode->SetCollFadeState(OPERATION_STARTED);
        AUDIO_INFO_LOG("OnCollborationFadeDone sessionId %{public}u", sessionId);
    };
    SendRequest(request, __func__);
}
...
void HpaeRendererManager::DisableCollaboration()
{
    for (auto& [key, node] : sinkInputNodeMap_) {
        ...
        if (nodeInfo.effectInfo.effectScene == SCENE_COLLABORATIVE) {
            AUDIO_INFO_LOG("delete coll sessionId %{public}u, state: %{public}d", key,
                static_cast<int32_t>(node->GetState()));
            if (node->GetState() == HPAE_SESSION_RUNNING) {
                HpaeProcessorType sceneType = node->connectedProcessorType_;
                std::shared_ptr<HpaeGainNode> sessionGainNode = nullptr;
...
bool HpaeRendererManager::QueryOneStreamUnderrun()
{
    ...
    if (underrunFlag) {
        lastOnUnderrunTime_ = lastOnUnderrunTime_ == 0 ? ClockTime::GetCurNano() : lastOnUnderrunTime_;
         int64_t underrunBypassDurationNs = AudioEffectChainManager::GetInstance()->GetOffloadEnabled()
            ? UNDERRUN_BYPASS_DURATION_NS : UNDERRUN_BYPASS_DURATION_OFFLOAD_NS;
        int64_t sleepTimeInNs = lastOnUnderrunTime_ + underrunBypassDurationNs - ClockTime::GetCurNano();
...
void HpaeRendererManager::DeleteNodesByTraversal(uint32_t sessionId)
{
    ...
    for (; it != sceneClusterMap_.end(); ++it) {
        if (SafeGetMap(sceneClusterMap_, it->first) && it->second->DestroyNodes(sessionId) == SUCCESS) {
            AUDIO_INFO_LOG("SessionId %{public}u, Nodes found in processorType %{public}d and destroyed",
                sessionId, it->first);
            break;
        }
    }
