...
static const std::string DEFAULT_SPEAKER_SINK_NAME = "Speaker";
...
HpaeManager::HpaeManager() : hpaeNoLockQueue_(CURRENT_REQUEST_COUNT)  // todo Message queue exceeds the upper limit
{
    ...
    RegisterHandler(UPDATE_SPAN_SIZE, &HpaeManager::HandleUpdateSpanSize);
...
int32_t HpaeManager::Init()
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(!IsInit(), SUCCESS,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_NO_SOUND, OperationType::GENERAL,
                BusinessScenario::CREATE, ERR_ILLEGAL_STATE),
            "already inited", false),
        "already inited");
...
int32_t HpaeManager::SetSourceOutputMuteByStreamId(int32_t streamId, bool isMute)
{
    auto request = [this, streamId, isMute]() {
        AUDIO_INFO_LOG("SetSourceOutputMuteByStreamId streamId: %{public}d setMute: %{public}d", streamId, isMute);
        for (const auto &sourceInfo : sourceOutputs_) {
            CHECK_AND_CONTINUE(sourceInfo.second.streamId == streamId);
            auto captureManager = GetCapturerManagerById(sourceInfo.first);
            CHECK_AND_CONTINUE_LOG(captureManager != nullptr,
                "mute can not find CaptureManager by id:%{public}u with streamId:%{public}d",
                sourceInfo.first, streamId);
            captureManager->SetStreamMute(streamId, isMute);
        }
        if (auto serviceCallback = serviceCallback_.lock()) {
            serviceCallback->OnSetSourceOutputMuteCb(SUCCESS);
        }
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(IsInit(), SUCCESS,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_STATE_INCONSISTENT, OperationType::GENERAL,
                BusinessScenario::RELEASE, ERR_ILLEGAL_STATE),
            "isn't inited", false),
        "isn't inited");
    ...
    uint32_t sinkSourceIndex, bool isReload)
{
    ...
    if (sinkInfo.isUltraFast && gHpaeUltraFastDevicesSet.find(sinkInfo.deviceType) == gHpaeUltraFastDevicesSet.end()) {
        AUDIO_WARNING_LOG("Device type %{public}d does not support ultra fast, className:%{public}s",
            sinkInfo.deviceType, sinkInfo.deviceClass.c_str());
    }
...
int32_t HpaeManager::CreateCaptureManager(HpaeSourceInfo &sourceInfo, uint32_t sinkSourceIndex, bool isReload)
{
    ...
    auto capturerManager = IHpaeCapturerManager::CreateCapturerManager(sourceInfo);
...
bool HpaeManager::MovingSinkStateChange(uint32_t sessionId, const std::shared_ptr<HpaeSinkInputNode> &sinkInput)
{
    if (movingIds_.find(sessionId) != movingIds_.end()) {
        ...
        sinkInput->SetOffloadType(rendererIdStreamInfoMap_[sessionId].offloadType);
...
void HpaeManager::HandleUpdateSpanSize(uint32_t sessionId, uint32_t spanSizeInFrame,
    HpaeStreamClassType streamClassType)
{
    AUDIO_INFO_LOG("HandleUpdateSpanSize streamClassType:%{public}d sessionId:%{public}u span:%{public}u",
        streamClassType, sessionId, spanSizeInFrame);
    if (streamClassType == HPAE_STREAM_CLASS_TYPE_PLAY) {
        auto it = rendererIdStreamInfoMap_.find(sessionId);
        CHECK_AND_RETURN_LOG(it != rendererIdStreamInfoMap_.end(), "renderer session not found");
        it->second.streamInfo.frameLen = spanSizeInFrame;
        UpdateStatus(it->second.statusCallback, OPERATION_UPDATE_SPANSIZE, sessionId);
    } else if (streamClassType == HPAE_STREAM_CLASS_TYPE_RECORD) {
        auto it = capturerIdStreamInfoMap_.find(sessionId);
        CHECK_AND_RETURN_LOG(it != capturerIdStreamInfoMap_.end(), "capturer session not found");
        it->second.streamInfo.frameLen = spanSizeInFrame;
        UpdateStatus(it->second.statusCallback, OPERATION_UPDATE_SPANSIZE, sessionId);
    }
}
...
{
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(!(operation == OPERATION_STOPPED && currentState != HPAE_SESSION_STOPPING) &&
        !(operation == OPERATION_PAUSED && currentState != HPAE_SESSION_PAUSING), false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_STATE_INCONSISTENT, OperationType::GENERAL,
                BusinessScenario::STOP, ERR_ILLEGAL_STATE),
            "stopped or paused, currentState:" + std::to_string(currentState), false),
...
{
    if (streamType == HPAE_STREAM_CLASS_TYPE_PLAY) {
        CHECK_AND_CALL_FUNC_RETURN_RET_LOG(rendererIdStreamInfoMap_.find(sessionId) != rendererIdStreamInfoMap_.end() &&
            ...
            "renderer session: " + std::to_string(sessionId) + " already released", false),
            "renderer session: %{public}u already released", sessionId);
    } else if (streamType == HPAE_STREAM_CLASS_TYPE_RECORD) {
        CHECK_AND_CALL_FUNC_RETURN_RET_LOG(capturerIdStreamInfoMap_.find(sessionId) != capturerIdStreamInfoMap_.end() &&
            ...
            BuildErrorCode(ProblemCategory::FAULT_STATE_INCONSISTENT, OperationType::RECORD,
                BusinessScenario::QUERY, ERR_NULL_POINTER),
            "capturer session: " + std::to_string(sessionId) + " already released", false),
...
int32_t HpaeManager::Pause(HpaeStreamClassType streamClassType, uint32_t sessionId, bool isStandby)
{
    auto request = [this, streamClassType, sessionId, isStandby]() {
        CHECK_AND_RETURN_LOG(ShouldNotSkipProcess(streamClassType, sessionId),
            "Pause session: %{public}u failed, session already released", sessionId);
        AUDIO_INFO_LOG(
            "Pause sessionId: %{public}u streamClassType:%{public}d isStandby:%{public}d",
            sessionId, streamClassType, isStandby);
        ...
        if (streamClassType == HPAE_STREAM_CLASS_TYPE_PLAY &&
            ...
            CHECK_AND_RETURN_LOG(SafeGetMap(rendererManagerMap_, rendererIdSinkNameMap_[sessionId]),
                "cannot find device:%{public}s", GetEncryptStr(rendererIdSinkNameMap_[sessionId]).c_str());
            rendererManagerMap_[rendererIdSinkNameMap_[sessionId]]->Pause(sessionId, isStandby);
...
int32_t HpaeManager::GetStreamSpanSize(HpaeStreamClassType streamClassType, uint32_t sessionId,
    uint32_t &spanSizeInFrame)
{
    if (streamClassType == HPAE_STREAM_CLASS_TYPE_PLAY &&
        rendererIdStreamInfoMap_.find(sessionId) != rendererIdStreamInfoMap_.end()) {
        spanSizeInFrame = static_cast<uint32_t>(rendererIdStreamInfoMap_[sessionId].streamInfo.frameLen);
        return SUCCESS;
    }
    if (streamClassType == HPAE_STREAM_CLASS_TYPE_RECORD &&
        capturerIdStreamInfoMap_.find(sessionId) != capturerIdStreamInfoMap_.end()) {
        spanSizeInFrame = static_cast<uint32_t>(capturerIdStreamInfoMap_[sessionId].streamInfo.frameLen);
        return SUCCESS;
    }
    return ERR_INVALID_OPERATION;
}

int32_t HpaeManager::GetStreamRouteFlag(HpaeStreamClassType streamClassType, uint32_t sessionId,
    uint32_t &routeFlag)
{
    if (streamClassType == HPAE_STREAM_CLASS_TYPE_PLAY &&
        rendererIdSinkNameMap_.find(sessionId) != rendererIdSinkNameMap_.end()) {
        std::string sinkName = rendererIdSinkNameMap_[sessionId];
        if (rendererManagerMap_.find(sinkName) != rendererManagerMap_.end()) {
            routeFlag = rendererManagerMap_[sinkName]->GetSinkInfo().routeFlag;
            return SUCCESS;
        }
    }
    if (streamClassType == HPAE_STREAM_CLASS_TYPE_RECORD &&
        capturerIdSourceNameMap_.find(sessionId) != capturerIdSourceNameMap_.end()) {
        std::string sinkName = capturerIdSourceNameMap_[sessionId];
        if (capturerManagerMap_.find(sinkName) != capturerManagerMap_.end()) {
            routeFlag = capturerManagerMap_[sinkName]->GetSourceInfo().routeFlag;
            return SUCCESS;
        }
    }
    return ERR_INVALID_OPERATION;
...
std::shared_ptr<IHpaeRendererManager> HpaeManager::GetRendererManagerByDeviceClass(const std::string &deviceClass)
{
    for (const auto &[name, rendererManager] : rendererManagerMap_) {
        CHECK_AND_CONTINUE(rendererManager != nullptr);
        HpaeSinkInfo sinkInfo = rendererManager->GetSinkInfo();
        if (sinkInfo.deviceClass == deviceClass) {
            AUDIO_INFO_LOG("GetRendererManagerByDeviceClass found deviceClass:%{public}s name:%{public}s",
                deviceClass.c_str(), GetEncryptStr(name).c_str());
            return rendererManager;
        }
    }
    AUDIO_WARNING_LOG("can not find rendererManager by deviceClass: %{public}s", deviceClass.c_str());
    return nullptr;
}
...
int32_t HpaeManager::UpdateSpatializationState(AudioSpatializationState spatializationState)
{
    auto request = [this, spatializationState]() {
        ...
        UpdateBypassSpatializationForStereo(bypassSpatializationForStereo_);
        auto serviceCallback = serviceCallback_.lock();
        CHECK_AND_RETURN_LOG(serviceCallback != nullptr, "serviceCallback is nullptr");
        serviceCallback->OnUpdateSpatializationStateCb(SUCCESS);
    };
    SendRequest(request, __func__);
    return SUCCESS;
}
...
bool HpaeManager::HandleRendererManager(const std::string &sinkName, const HpaeStreamInfo &streamInfo)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(rendererManager, false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_NO_SOUND, OperationType::PLAY,
                BusinessScenario::CREATE, ERR_NULL_POINTER),
            "can not find sink in rendererManagerMap_", false),
        "can not find sink[%{public}s] in rendererManagerMap_", GetEncryptStr(sinkName).c_str());
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(rendererManager->IsInit(), false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_NO_SOUND, OperationType::PLAY,
                BusinessScenario::CREATE, ERR_NULL_POINTER),
            "sink is not init", false),
        "sink[%{public}s] is not init", GetEncryptStr(sinkName).c_str());
...
int32_t HpaeManager::SetAudioEnhanceProperty(const AudioEffectPropertyArray &propertyArray, DeviceType deviceType)
{
...
int32_t HpaeManager::GetAudioEnhanceProperty(AudioEffectPropertyArray &propertyArray, DeviceType deviceType)
{
...
{
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(!params.empty(), false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_NO_SOUND, OperationType::CONTROL,
                BusinessScenario::SEND_DATA, ERR_INVALID_PARAM),
            "params is empty", false),
        "params is empty");
...
void HpaeManager::HandleConnectCoBufferNode(std::shared_ptr<HpaeCoBufferNode> hpaeCoBufferNode)
{
    auto request = [this, hpaeCoBufferNode]() {
        ...
        std::shared_ptr<IHpaeRendererManager> defaultRendererManager =
            GetRendererManagerByName(DEFAULT_SPEAKER_SINK_NAME);
        CHECK_AND_RETURN_LOG(defaultRendererManager != nullptr,
            "can not find sink[%{public}s] in rendererManagerMap_", DEFAULT_SPEAKER_SINK_NAME.c_str());
    ...
    const uint32_t &sinkPortIndex, uint8_t *buffer, size_t bufferSize, AudioStreamInfo &streamInfo)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(sinkVirtualOutputNode != nullptr, ERROR_INVALID_PARAM,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            BuildErrorCode(ProblemCategory::FAULT_STATE_INCONSISTENT, OperationType::PLAY,
                BusinessScenario::QUERY, ERR_NULL_POINTER),
            "sinkPort not exit", false),
...
int32_t HpaeManager::UpdateCollaborativeProductId(const std::string &productId)
{
    auto request = [this, productId]() {
        int32_t result = HpaePolicyManager::GetInstance().UpdateCollaborativeProductId(productId);
        auto serviceCallback = serviceCallback_.lock();
        CHECK_AND_RETURN_LOG(serviceCallback != nullptr, "serviceCallback is nullptr");
        serviceCallback->OnGetCollaborativeUpdateCb(result);
    };
    SendRequest(request, __func__);
    return SUCCESS;
}

int32_t HpaeManager::UpdatePersonalizedHRTFBin(const int32_t &fd, const long &length)
{
    auto request = [this, fd, length]() {
        int32_t result = HpaePolicyManager::GetInstance().UpdatePersonalizedHRTFBin(fd, length);
        auto serviceCallback = serviceCallback_.lock();
        CHECK_AND_RETURN_LOG(serviceCallback != nullptr, "serviceCallback is nullptr");
        serviceCallback->OnUpdatePersonalizedHRTFBinCb(result);
    };
    SendRequest(request, __func__);
    return SUCCESS;
}
...
float HpaeManager::GetMaxAmplitude()
{
    float maxAmplitude = 0.0f;
    for (const auto &[name, rendererManager] : rendererManagerMap_) {
        CHECK_AND_CONTINUE(rendererManager != nullptr);
        maxAmplitude = std::max(maxAmplitude, rendererManager->GetMaxAmplitude());
    }
    return maxAmplitude;
}

void HpaeManager::UpdateBypassSpatializationForStereo(bool bypass)
{
    bypassSpatializationForStereo_ = bypass;
    AUDIO_INFO_LOG("HpaeManager::UpdateBypassSpatializationForStereo %{public}d", bypass);
    HpaePolicyManager::GetInstance().SetBypassSpatializationForStereo(bypass && adaptiveSpatialRenderingEnabled_);
}
 
void HpaeManager::HandleBypassSpatializationForStereo(bool bypass)
{
    AUDIO_INFO_LOG("HpaeManager::HandleBypassSpatializationForStereo %{public}d", bypass);
    auto request = [this, bypass]() {
        UpdateBypassSpatializationForStereo(bypass);
    };
    SendRequest(request, __func__);
}

int32_t HpaeManager::SetAppsEnhanceMuteState(const uint32_t &sessionId, bool isMute)
{
    AUDIO_INFO_LOG("HpaeManager::SetAppsEnhanceMuteState %{public}d", isMute);
    auto request = [this, sessionId, isMute]() {
        auto captureManager = GetCapturerManagerById(sessionId);
        if (captureManager != nullptr) {
            captureManager->SetAppsEnhanceMuteState(sessionId, isMute);
        }
    };
    SendRequest(request, __func__);
    return SUCCESS;
...
void HpaeManager::EnableCaptureCollaboration(uint32_t sourcePortIndex, uint32_t collabSourcePortIndex,
    SourceType sourceType)
{
    auto request = [this, sourcePortIndex, sourceType] {
        CHECK_AND_RETURN_LOG(sourceIdSourceNameMap_.find(sourcePortIndex) != sourceIdSourceNameMap_.end(),
            "EnableCaptureCollaboration::sourcePortIndex[%{public}u] not exist", sourcePortIndex);
        AUDIO_INFO_LOG("Enable collaboration on source %{public}s",
            sourceIdSourceNameMap_[sourcePortIndex].c_str());
        auto capturerManager = SafeGetMap(capturerManagerMap_, sourceIdSourceNameMap_[sourcePortIndex]);
        CHECK_AND_RETURN_LOG(capturerManager, "source[%{public}s] is in wrong state",
            sourceIdSourceNameMap_[sourcePortIndex].c_str());
        capturerManager->EnableCaptureCollaboration(sourceType);
    };
    SendRequest(request, __func__);
}

void HpaeManager::DisableCaptureCollaboration(uint32_t sourcePortIndex, uint32_t collabSourcePortIndex,
    SourceType sourceType)
{
    auto request = [this, sourcePortIndex, sourceType] {
        CHECK_AND_RETURN_LOG(sourceIdSourceNameMap_.find(sourcePortIndex) != sourceIdSourceNameMap_.end(),
            "DisableCaptureCollaboration::sourcePortIndex[%{public}u] not exist", sourcePortIndex);
        AUDIO_INFO_LOG("Disable collaboration on source %{public}s",
            sourceIdSourceNameMap_[sourcePortIndex].c_str());
        auto capturerManager = SafeGetMap(capturerManagerMap_, sourceIdSourceNameMap_[sourcePortIndex]);
        CHECK_AND_RETURN_LOG(capturerManager, "source[%{public}s] is in wrong state",
            sourceIdSourceNameMap_[sourcePortIndex].c_str());
        capturerManager->DisableCaptureCollaboration(sourceType);
    };
    SendRequest(request, __func__);
}

void HpaeManager::SetSinkMuteForSwitchDevice(const std::string &deviceClass, bool mute)
{
    auto request = [this, deviceClass, mute]() {
        AUDIO_INFO_LOG("HpaeManager::SetSinkMuteForSwitchDevice deviceClass:%{public}s mute:%{public}d",
            deviceClass.c_str(), mute);
        auto rendererManager = GetRendererManagerByDeviceClass(deviceClass);
        CHECK_AND_RETURN_LOG(rendererManager != nullptr, "rendererManager is nullptr for deviceClass:%{public}s",
            deviceClass.c_str());
        rendererManager->SetMuteForSwitchDevice(mute);
    };
    SendRequest(request, __func__);
}
}  // namespace HPAE
