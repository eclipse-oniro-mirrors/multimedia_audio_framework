...
map<pair<ContentType, StreamUsage>, AudioStreamType> AudioStreamCollector::CreateStreamMap()
{
    ...
    streamMap[make_pair(CONTENT_TYPE_UNKNOWN, STREAM_USAGE_INTERPHONE)] = STREAM_VOICE_COMMUNICATION;
...
void AudioStreamCollector::RegisteredRendererTrackerClientDied(const int32_t uid, const int32_t pid)
{
    ...
    while (audioRendererBegin != audioRendererChangeInfos_.end()) {
        ...
        if (audioRendererChangeInfo->rendererState == RENDERER_RUNNING) {
            ResetRingerModeMute(RENDERER_RELEASED, audioRendererChangeInfo->rendererInfo.streamUsage);
        }
...
void AudioStreamCollector::ReportExceptionForBackgroundMute(uint32_t sessionId, uint32_t usage, int32_t uid)
{
    ...
    bean->Add("STREAM_TYPE", static_cast<int32_t>(usage));
    bean->Add("TYPE", TYPE_BACKGROUND_PLAYBACK);
    AUDIO_ERR_LOG("ReportExceptionForBackMute: APP_NAME:%{public}s, STREAM_TYPE:%{public}d, TYPE:%{public}d",
        appName.c_str(), static_cast<int32_t>(usage), TYPE_BACKGROUND_PLAYBACK);
    ...
    for (const auto &changeInfo : audioRendererChangeInfos_) {
        if (changeInfo != nullptr && changeInfo->clientUID == uid && changeInfo->clientPid == pid) {
            ...
            if (mute && !changeInfo->backMute) {
                ReportExceptionForBackgroundMute(changeInfo->sessionId, setStateEvent.streamUsage, uid);
...
bool AudioStreamCollector::IsStreamActive(AudioStreamType volumeType)
{
    ...
    for (auto &changeInfo: audioRendererChangeInfos_) {
        ...
        if (volumeType == STREAM_ALL) {
            AUDIO_INFO_LOG("For STREAM_ALL matched clientUid: %{public}d sessionId: %{public}d",
...
bool AudioStreamCollector::IsCallStreamUsage(StreamUsage usage)
{
    if (usage == STREAM_USAGE_VOICE_COMMUNICATION || usage == STREAM_USAGE_VIDEO_COMMUNICATION ||
        usage == STREAM_USAGE_VOICE_MODEM_COMMUNICATION || usage == STREAM_USAGE_INTERPHONE) {
        return true;
    }
    return false;
}
...
std::vector<std::pair<int32_t, AudioStreamType>> AudioStreamCollector::GetActiveStreams()
{
    std::lock_guard<std::mutex> lock(streamsInfoMutex_);
    std::vector<std::pair<int32_t, AudioStreamType>> result;
    for (auto &changeInfo : audioRendererChangeInfos_) {
        if (changeInfo != nullptr && changeInfo->rendererState == RENDERER_RUNNING && !changeInfo->backMute) {
            int32_t uid = changeInfo->clientUID;
            AudioStreamType streamType = GetStreamType(
                changeInfo->rendererInfo.contentType, changeInfo->rendererInfo.streamUsage);
            std::pair streamPair {uid, streamType};
            if (std::find(result.begin(), result.end(), streamPair) == result.end()) {
                result.push_back(streamPair);
                AUDIO_INFO_LOG("clientUID: %{public}d, streamType: %{public}d", uid, streamType);
            }
        }
    }
    return result;
