...
void AudioSession::SetRegisterStateChangeCallbackFlag(const bool registerCallbackFlag,
    const bool activatedRegisterCallbackFlag)
{
    registerCallbackFlag_ = registerCallbackFlag;
    activatedRegisterCallbackFlag_ = activatedRegisterCallbackFlag;
}

void AudioSession::GetStateChangeCallbackFlag(bool &registerCallbackFlag, bool &activatedRegisterCallbackFlag)
{
    registerCallbackFlag = registerCallbackFlag_;
    activatedRegisterCallbackFlag = activatedRegisterCallbackFlag_;
}
...
std::vector<std::pair<AudioInterrupt, AudioFocuState>> AudioSession::GetStreams()
{
    std::vector<std::pair<AudioInterrupt, AudioFocuState>> streams;
    for (const auto &it : interruptMap_) {
...
void AudioSession::AddStreamInfo(const std::pair<AudioInterrupt, AudioFocuState> &interruptPair)
{
    if (interruptMap_.count(interruptPair.first.streamId) != 0) {
        AUDIO_INFO_LOG("stream already exist.");
        return;
    }

    interruptMap_[interruptPair.first.streamId] = interruptPair;

    if (IsSessionDefaultDeviceEnabled()) {
        (void)EnableSingleVoipStreamDefaultOutputDevice(interruptPair.first);
    }
...
{
    auto it = interruptMap_.find(streamId);
    if (it != interruptMap_.end()) {
        if (IsSessionDefaultDeviceEnabled()) {
            UpdateSingleVoipStreamDefaultOutputDevice(it->second.first);
        }
        interruptMap_.erase(it);
    }
    ...
    if (interruptMap_.size() == 0) {
        ...
        if ((audioSessionScene_ != AudioSessionScene::INVALID) &&
            fakeFocusInterruptEvent_.hintType != INTERRUPT_HINT_PAUSE &&
            IsBackGroundApp()) {
...
void AudioSession::SetFakeFocusInterruptEvent(const InterruptEventInternal &interruptEvent)
{
    fakeFocusInterruptEvent_ = interruptEvent;
}

InterruptEventInternal AudioSession::GetFakeFocusInterruptEvent()
{
    return fakeFocusInterruptEvent_;
}
...
int32_t AudioSession::Activate(const AudioSessionStrategy strategy)
{
    ...
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> preferredOutputDevices =
        coreService->GetPreferredOutputDeviceDescInner(rendererInfo, LOCAL_NETWORK_ID);
    if ((preferredOutputDevices.size() == 0) || (preferredOutputDevices[0] == nullptr)) {
        deviceDescriptor_ = AudioRouterSelectStrategy::GetInstance().Get1stCurrentOutputDevice(callerUid_);
    } else {
...
void AudioSession::UpdateSingleVoipStreamDefaultOutputDevice(const AudioInterrupt &interrupt)
{
    if (CanCurrentStreamSetDefaultOutputDevice(interrupt)) {
        ...
        AudioRouterSelectStrategy::GetInstance().UpdateDefaultOutputDevice(DEVICE_TYPE_DEFAULT,
            interrupt.uid, interrupt.streamId, audioStreamDescriptor->rendererInfo_.streamUsage);
    }
}
...
void AudioSession::UpdateVoipStreamsDefaultOutputDevice()
{
    ...
    AudioRouterInfra::GetInstance().UpdateOutputStreamState(callerUid_, fakeStreamId_,
        GetStreamUsageInner(), RendererState::RENDERER_STOPPED, false);
}
...
int32_t AudioSession::EnableSingleVoipStreamDefaultOutputDevice(const AudioInterrupt &interrupt)
{
    ...
    if (CanCurrentStreamSetDefaultOutputDevice(interrupt) && (pipeManager_ != nullptr)) {
        ...
        if (audioStreamDescriptor != nullptr &&
            ...
            ret = AudioRouterSelectStrategy::GetInstance().UpdateDefaultOutputDevice(defaultDeviceType_,
                interrupt.uid, interrupt.streamId, audioStreamDescriptor->rendererInfo_.streamUsage);
        }
    }
...
int32_t AudioSession::EnableVoipStreamsDefaultOutputDevice()
{
    ...
    for (const auto &it : interruptMap_) {
        ret = EnableSingleVoipStreamDefaultOutputDevice(it.second.first);
        if (ret != SUCCESS) {
            AUDIO_ERR_LOG("enable default output device for stream %d failed, ret is %d",
                it.second.first.streamId, ret);
        } else {
            success = true;
        }
    }
...
int32_t AudioSession::EnableDefaultDevice(bool forceFetch)
{
    if (!IsSessionDefaultDeviceEnabled()) {
        return SUCCESS;
    }
    ...
    AudioRouterInfra::GetInstance().UpdateOutputStreamState(callerUid_, fakeStreamId_,
        GetStreamUsageInner(), RendererState::RENDERER_RUNNING, false);
    int32_t ret = AudioRouterSelectStrategy::GetInstance().UpdateDefaultOutputDevice(
        defaultDeviceType_, callerUid_, fakeStreamId_, GetStreamUsageInner());
...
int32_t AudioSession::SetSessionDefaultOutputDevice(const DeviceType &deviceType, bool forceFetch)
{
    ...
    return EnableDefaultDevice(forceFetch);
}
...
{
    return interruptMap_.count(streamId) != 0;
}

int32_t AudioSession::UpdateStreamFocusState(uint32_t streamId, AudioFocuState focuState)
{
    auto it = interruptMap_.find(streamId);
    if (it == interruptMap_.end()) {
        AUDIO_ERR_LOG("streamId %{public}u not found in interruptMap", streamId);
        return ERR_INVALID_PARAM;
    }
    if (focuState == AudioFocuState::STANDBY) {
        if (it->second.second != AudioFocuState::ACTIVE &&
            it->second.second != AudioFocuState::DUCK) {
            AUDIO_INFO_LOG("streamId %{public}u state %{public}d, cannot set to STANDBY",
                streamId, it->second.second);
            return SUCCESS;
        }
    }
    it->second.second = focuState;
    return SUCCESS;
}

bool AudioSession::IsAllStreamsInState(AudioFocuState state)
{
    if (interruptMap_.empty()) {
        return false;
    }
    for (const auto &iter : interruptMap_) {
        if (iter.second.second != state) {
            return false;
        }
    }
    return true;
    ...
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> &oldDeviceDescriptors)
{
    ...
    if (deviceDescriptor_.IsSameDeviceDescPtr(desc)) {
        return false;
    }
    oldDeviceDescriptors = {std::make_shared<AudioDeviceDescriptor>(deviceDescriptor_)};
...
StreamUsage AudioSession::GetAudioSessionStreamUsageForDevice()
{
    ...
    for (const auto& iter : interruptMap_) {
        if (iter.second.first.audioFocusType.streamType == STREAM_RING) {
            AUDIO_INFO_LOG("pid [%{public}d] ringtone exists", callerPid_);
            return iter.second.first.streamUsage;
        }
    }
...
int32_t AudioSession::SetAudioSessionBehavior(const uint32_t behavior)
{
    behaviorFlags_ = {};
    if (behavior & static_cast<uint32_t>(AudioSessionBehaviorFlags::VOIP_PRIVACY_TYPE_PUBLIC)) {
        behaviorFlags_.voipNoPrivacyFlag = true;
        AUDIO_INFO_LOG("Session pid: %{public}d activated VOIP_PRIVACY_TYPE_PUBLIC success", callerPid_);
    }
    if (behavior & static_cast<uint32_t>(AudioSessionBehaviorFlags::MUTE_WHEN_INTERRUPTED)) {
        behaviorFlags_.muteWhenInterruptFlag = true;
        AUDIO_INFO_LOG("Session pid: %{public}d activated MUTE_WHEN_INTERRUPT success", callerPid_);
    }
    return SUCCESS;
}

AudioSessionActiveBehaviors AudioSession::GetSessionBehaviors()
{
    return behaviorFlags_;
}

void AudioSession::GetSessionDebugInfo(AudioSessionDebugInfo &sessionDebugInfo, const bool isAudioSessionInterrupt)
{
    sessionDebugInfo.strategy = strategy_;
    sessionDebugInfo.defaultDeviceType = defaultDeviceType_;
    sessionDebugInfo.state = state_;
    if (isAudioSessionInterrupt) {
        sessionDebugInfo.audioSessionScene = audioSessionScene_;
        sessionDebugInfo.streamInfos.clear();
        for (const auto &iter : interruptMap_) {
            AudioStreamDebugInfo streamDebugInfo;
            streamDebugInfo.streamId = iter.second.first.streamId;
            streamDebugInfo.streamType = iter.second.first.audioFocusType.streamType;
