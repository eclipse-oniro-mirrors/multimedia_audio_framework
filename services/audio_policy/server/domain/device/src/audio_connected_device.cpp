...
std::vector<std::shared_ptr<AudioDeviceDescriptor>> AudioConnectedDevice::GetDevicesInner(DeviceFlag deviceFlag)
{
    ...
    for (auto device : connectedDevices_) {
        ...
        AUDIO_INFO_LOG("networkId=[%{public}s], deviceRole=[%{public}d], deviceType=[%{public}d], isLocal=[%{public}d]",
            device->networkId_.c_str(), static_cast<int32_t>(device->deviceRole_),
                static_cast<int32_t>(device->deviceType_), (device->networkId_ == LOCAL_NETWORK_ID ? 1 : 0));
...
DeviceType AudioConnectedDevice::FindConnectedHeadset()
{
    ...
    const auto& itr = std::find_if(connectedDevices_.begin(), connectedDevices_.end(),
        ...
        return ((devDesc->deviceType_ == DEVICE_TYPE_WIRED_HEADSET) ||
            ...
            (devDesc->deviceType_ == DEVICE_TYPE_WIRED_MICIN) ||
...
std::unordered_set<DeviceType> AudioConnectedDevice::GetAllOutputDeviceTypes()
{
    std::shared_lock<std::shared_mutex> lock(mutex_);
    std::unordered_set<DeviceType> deviceTypes;
    for (const auto& device : connectedDevices_) {
        if (device->deviceRole_ == OUTPUT_DEVICE) {
            deviceTypes.insert(device->deviceType_);
        }
    }
    return deviceTypes;
}

std::vector<std::shared_ptr<AudioDeviceDescriptor>> AudioConnectedDevice::GetAllOutputDevices()
{
    std::shared_lock<std::shared_mutex> lock(mutex_);
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> devices;
    for (const auto& device : connectedDevices_) {
