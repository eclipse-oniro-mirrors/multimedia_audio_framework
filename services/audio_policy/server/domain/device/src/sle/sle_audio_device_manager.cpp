...
const std::map<uint32_t, std::set<StreamUsage>> STREAM_USAGE_TO_SLE_STREAM_TYPE = {
    ...
    {SLE_AUDIO_STREAM_VOIP, {STREAM_USAGE_VOICE_COMMUNICATION, STREAM_USAGE_VIDEO_COMMUNICATION,
        STREAM_USAGE_VOICE_CALL_ASSISTANT, STREAM_USAGE_INTERPHONE}},
...
bool SleAudioDeviceManager::IsAudioLoopbackSupported(const std::string &remoteAddr)
{
    (void)remoteAddr;
    // in plan: use callback_ to get result
    return true;
}
...
uint32_t SleAudioDeviceManager::GetSleStreamTypeByStreamUsage(StreamUsage streamUsage, int32_t uid, bool isLoopback)
{
    if (isLoopback && streamUsage == STREAM_USAGE_MUSIC) {
        // in plan: change to target type.
...
int32_t SleAudioDeviceManager::SetActiveDevice(const AudioDeviceDescriptor &deviceDesc, StreamUsage streamUsage,
    int32_t uid, bool isLoopback)
{
    CHECK_AND_RETURN_RET_LOG(IsNearlinkDevice(deviceDesc.deviceType_), ERROR, "device type is not nearlink");
    return SetActiveSinkDevice(deviceDesc.macAddress_, GetSleStreamTypeByStreamUsage(streamUsage, uid, isLoopback));
}
    ...
    int32_t uid, bool isLoopback)
{
    ...
    return StartPlaying(deviceDesc.macAddress_, GetSleStreamTypeByStreamUsage(streamUsage, uid, isLoopback),
    ...
    uint32_t sessionId, bool isAdd)
{
    ...
    AUDIO_INFO_LOG("sle device %{public}s, add [%{public}d] sle streamType %{public}u with size[%{public}zu]\
        sessionId %{public}d", AudioPolicyUtils::GetInstance().GetEncryptAddr(deviceAddr).c_str(), isAdd,
        streamType, sessionSet.size(), sessionId);
    if (isAdd) {
    ...
    if (streamDesc->audioMode_ == AUDIO_MODE_PLAYBACK) {
        ...
        if (IsNearlinkMoveToOtherDevice(streamDesc)) {
            streamType = GetSleStreamTypeByStreamUsage(streamUsage, streamDesc->GetRealUid(),
                streamDesc->rendererInfo_.isLoopback);
        ...
        if (IsMoveToNearlinkDevice(streamDesc)) {
            ...
            if (streamDesc->streamStatus_ == STREAM_STATUS_STARTED && !streamDesc->isStandby_) {
...
void SleAudioDeviceManager::ResetSleStreamTypeCount(const std::string &deviceAddr)
{
    CHECK_AND_RETURN_LOG(!deviceAddr.empty(), "deviceAddr is empty");

    std::lock_guard<std::mutex> lock(startedSleStreamTypeMutex_);
    auto it = startedSleStreamType_.find(deviceAddr);
    CHECK_AND_RETURN_LOG(it != startedSleStreamType_.end(), "device %{public}s not found",
        AudioPolicyUtils::GetInstance().GetEncryptAddr(deviceAddr).c_str());
    ...
    SetActiveSinkDevice(deviceAddr, SLE_AUDIO_STREAM_NONE);
...
int32_t SleAudioDeviceManager::SetNearlinkDeviceVolume(const std::string &device, AudioStreamType streamType,
    VolumeScale &volume)
{
    std::lock_guard<std::mutex> lock(deviceVolumeConfigMutex_);
    CHECK_AND_RETURN_RET_LOG(deviceVolumeConfigInfo_.find(device) != deviceVolumeConfigInfo_.end(),
        ERR_INVALID_PARAM, "device not found");
    SleVolumeConfigInfo tmp{};
    auto *config = &tmp;
    if (streamType == STREAM_MUSIC) {
        config = &deviceVolumeConfigInfo_[device].first;
    } else if (streamType == STREAM_VOICE_CALL && volume.VolumeLevel() > 0) {
        config = &deviceVolumeConfigInfo_[device].second;
    }

    volume.Adjust({config->volumeLevel, config->volumeDegree});
    config->volumeLevel = volume.VolumeLevel();
    config->volumeDegree = volume.VolumeDegree();
    AUDIO_INFO_LOG("Set Volume: streamType:%{public}d volumeLevel:%{public}d, volumeDegree:%{public}d",
        config->volumeType, volume.VolumeLevel(), volume.VolumeDegree());
    return SUCCESS;
}
...
bool SleAudioDeviceManager::GetMuteStatusByVolumeType(AudioVolumeType volumeType,
    const std::string &nearLinkDevice)
{
    std::lock_guard<std::mutex> lock(deviceVolumeConfigMutex_);
    CHECK_AND_RETURN_RET_LOG(deviceVolumeConfigInfo_.find(nearLinkDevice) != deviceVolumeConfigInfo_.end(),
        false, "device not found");
    if (volumeType == STREAM_MUSIC) {
        return deviceVolumeConfigInfo_[nearLinkDevice].first.isMute;
    } else if (volumeType == STREAM_VOICE_CALL) {
        return false;
    }
    return false;
...
std::string SleAudioDeviceManager::GetActivatedDevice()
{
    std::string activatedDevice = "";
    std::lock_guard<std::mutex> lock(startedSleStreamTypeMutex_);
    for (const auto &devicePair : startedSleStreamType_) {
        for (const auto &streamPair : devicePair.second) {
            if (streamPair.second.isStarted) {
                activatedDevice = devicePair.first;
                break;
            }
        }
    }
