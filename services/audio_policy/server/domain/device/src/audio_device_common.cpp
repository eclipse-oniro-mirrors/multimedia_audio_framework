...
static void ExcludeIndirectUsbArmInput(const AudioDeviceDescriptor &updatedDesc)
{
    ...
    AudioRouterSelectStrategy::GetInstance().ExcludeDevices(descs, MEDIA_INPUT_DEVICES | CALL_INPUT_DEVICES);
}
...
static void UnexcludeIndirectUsbArmInput(const AudioDeviceDescriptor &updatedDesc)
{
    CHECK_AND_RETURN(!updatedDesc.hasPair_ && updatedDesc.deviceType_ == DEVICE_TYPE_USB_ARM_HEADSET);
    vector<shared_ptr<AudioDeviceDescriptor>> descs{make_shared<AudioDeviceDescriptor>(updatedDesc)};
    AudioRouterSelectStrategy::GetInstance().UnexcludeDevices(descs, MEDIA_INPUT_DEVICES | CALL_INPUT_DEVICES);
}
#endif
...
void AudioDeviceCommon::ClearPreferredDevices(const vector<shared_ptr<AudioDeviceDescriptor>> &descForCb)
{
    ...
    for (const auto& desc : descForCb) {
        if (strategy.GetMediaOutputDevice(INVALID_UID, INVALID_STREAM_ID) != nullptr &&
            desc->IsSameDeviceDesc(*strategy.GetMediaOutputDevice(INVALID_UID, INVALID_STREAM_ID))) {
            AudioPolicyUtils::GetInstance().SetPreferredDevice(AUDIO_MEDIA_RENDER,
                std::make_shared<AudioDeviceDescriptor>());
        }
        if (strategy.GetCallOutputDevice(INVALID_UID, INVALID_STREAM_ID) != nullptr &&
            desc->IsSameDeviceDesc(*strategy.GetCallOutputDevice(INVALID_UID, INVALID_STREAM_ID))) {
        ...
        auto callInputDevice = strategy.GetCallInputDevice(INVALID_UID, INVALID_STREAM_ID);
        if (callInputDevice != nullptr && callInputDevice->deviceType_ != DEVICE_TYPE_NONE &&
            desc->IsSameDeviceDesc(*callInputDevice)) {
            AudioPolicyUtils::GetInstance().SetPreferredDevice(AUDIO_CALL_CAPTURE,
                std::make_shared<AudioDeviceDescriptor>());
        }
        auto mediaInputDevice = strategy.GetMediaInputDevice(INVALID_UID, INVALID_STREAM_ID, SOURCE_TYPE_INVALID);
        if (mediaInputDevice != nullptr && mediaInputDevice->deviceType_ != DEVICE_TYPE_NONE &&
            desc->IsSameDeviceDesc(*mediaInputDevice)) {
    ...
    const AudioDeviceDescriptor &updatedDesc, std::vector<std::shared_ptr<AudioDeviceDescriptor>> &descForCb)
{
    ...
    if (!NeedUpdatePreferredDevice(updatedDesc)) {
        return;
    }
    DeviceUsage usage = audioDeviceManager_.GetDeviceUsage(updatedDesc);
    FetchDeviceInfo info = { STREAM_USAGE_MEDIA, "updateConnectedDescWhenCon_1", ROUTER_TYPE_USER_SELECT };
    if (NeedClearPreferredMediaRenderer(
        AudioRouterSelectStrategy::GetInstance().GetMediaOutputDevice(INVALID_UID, INVALID_STREAM_ID),
        audioDescriptor, audioRouterCenter_.FetchOutputDevices(info), usage)) {
        AudioPolicyUtils::GetInstance().SetPreferredDevice(AUDIO_MEDIA_RENDER,
            std::make_shared<AudioDeviceDescriptor>());
    }
    info = { STREAM_USAGE_VOICE_COMMUNICATION, "updateConnectedDescWhenCon_2", ROUTER_TYPE_USER_SELECT };
    if (audioDescriptor->networkId_ == LOCAL_NETWORK_ID && audioDescriptor->IsSameDeviceDesc(
        *audioRouterCenter_.FetchOutputDevices(info).front()) && (usage & VOICE) == VOICE) {
        AudioPolicyUtils::GetInstance().SetPreferredDevice(AUDIO_CALL_RENDER,
...
bool AudioDeviceCommon::NeedUpdatePreferredDevice(const AudioDeviceDescriptor &desc)
{
    if (desc.connectState_ == VIRTUAL_CONNECTED) {
    ...
    switch (desc.deviceType_) {
        ...
        case DEVICE_TYPE_NEARLINK:
            AUDIO_INFO_LOG("The device is wireless device, no need to update preferred device");
            return false;
        default:
            return true;
    }
    ...
    std::shared_ptr<AudioRendererChangeInfo> &rendererChangeInfo)
{
    ...
    if (VolumeUtils::IsPCVolumeEnable() && !isFirstScreenOn_) {
        ...
        FetchDeviceInfo info = { rendererChangeInfo->rendererInfo.streamUsage,
...
{
    bool isDistOld = IsDistributedOutput(audioRouterSelectStrategy_.Get1stCurrentOutputDevice());
...
{
    auto &strategy = AudioRouterSelectStrategy::GetInstance();
    shared_ptr<AudioDeviceDescriptor> userSelectedMediaDevice =
        strategy.GetMediaOutputDevice(INVALID_UID, INVALID_STREAM_ID);
    shared_ptr<AudioDeviceDescriptor> userSelectedCallDevice =
