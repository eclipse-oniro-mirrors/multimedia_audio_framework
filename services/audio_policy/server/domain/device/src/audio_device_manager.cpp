    ...
    vector<shared_ptr<AudioDeviceDescriptor>> &descArray)
{
    ...
    if (result) {
        descArray.push_back(make_shared<AudioDeviceDescriptor>(*devDesc));
...
void AudioDeviceManager::AddDefaultDevices(const std::shared_ptr<AudioDeviceDescriptor> &devDesc)
{
    ...
    bool defaultMicCondition = devType == DEVICE_TYPE_MIC;
#ifdef MULTI_BUS_ENABLE
    defaultMicCondition = defaultMicCondition && devDesc->macAddress_.empty();
#endif
    if (devType == DEVICE_TYPE_EARPIECE) {
        earpiece_ = devDesc;
    } else if (devType == DEVICE_TYPE_SPEAKER) {
        speaker_ = devDesc;
    } else if (defaultMicCondition) {
        defalutMic_ = devDesc;
    }
}
...
bool AudioDeviceManager::UpdateExistDeviceDescriptor(const std::shared_ptr<AudioDeviceDescriptor> &deviceDescriptor)
{
    ...
    if (iter != connectedDevices_.end()) {
        deviceDescriptor->deviceId_ = (*iter)->deviceId_;
...
{
    deviceDescriptor->connectState_ = AudioCoreConfigManager::GetInstance().IsDeviceSimulated(deviceDescriptor)
        ? SIMULATED_CONNECTED : deviceDescriptor->connectState_;
...
shared_ptr<AudioDeviceDescriptor> AudioDeviceManager::GetCommRenderDefaultDevice(StreamUsage streamUsage)
{
    ...
    if (hasEarpiece_ && streamUsage != STREAM_USAGE_VIDEO_COMMUNICATION && streamUsage != STREAM_USAGE_INTERPHONE) {
    ...
    switch (usage) {
        case MEDIA_OUTPUT_DEVICES:
            if ((static_cast<uint32_t>(dev->deviceRole_) & OUTPUT_DEVICE) &&
                ...
                (dev->deviceType_ != DEVICE_TYPE_BLUETOOTH_SCO || dev->deviceCategory_ == BT_WATCH)) {
...
bool AudioDeviceManager::UpdateDevicesListInfo(const std::shared_ptr<AudioDeviceDescriptor> &devDesc,
    const DeviceInfoUpdateCommand updateCommand, AudioStreamDeviceChangeReasonExt &reason)
{
    CHECK_AND_RETURN_RET_LOG(devDesc != nullptr, false, "desc is nullptr");
    ...
    switch (updateCommand) {
        case CATEGORY_UPDATE:
            ...
            reason = devDesc->deviceCategory_ == BT_UNWEAR_HEADPHONE ?
        ...
        case ENABLE_UPDATE:
            ...
            reason = devDesc->isEnable_ ? AudioStreamDeviceChangeReason::NEW_DEVICE_AVAILABLE :
                AudioStreamDeviceChangeReason::OLD_DEVICE_UNAVALIABLE;
            break;
        case EXCEPTION_FLAG_UPDATE:
            ...
            break;
        case USAGE_UPDATE:
            ret = UpdateDeviceUsage(devDesc, reason);
            break;
        default:
            break;
    }
    if (!ret) {
        int32_t audioId = devDesc->deviceId_;
        AUDIO_ERR_LOG("do not update type:id %{public}d:%{public}d mac:%{public}s\
            networkid:%{public}s in connected list",
...
bool AudioDeviceManager::TriggerDeviceInfoUpdatedCallback(AudioDeviceDescriptor &desc,
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> &descForCb)
{
    if (desc.glassesWearCount_ == AudioDeviceDescriptor::WEAR_FIRST && desc.deviceCategory_ == BT_UNWEAR_HEADPHONE) {
        return false;
    }
...
bool AudioDeviceManager::UpdateDeviceCategory(const std::shared_ptr<AudioDeviceDescriptor> &devDesc)
{
    ...
    for (auto &desc : connectedDevices_) {
        if (desc->deviceType_ == devDesc->deviceType_ &&
            ...
            TriggerDeviceInfoUpdatedCallback(*desc, descForCb);
            updateFlag = true;
        }
    }
    return updateFlag;
...
bool AudioDeviceManager::UpdateDeviceUsage(const shared_ptr<AudioDeviceDescriptor> &deviceDesc,
    AudioStreamDeviceChangeReasonExt &reason)
{
    CHECK_AND_RETURN_RET_LOG(deviceDesc != nullptr, false, "device is nullptr");
    bool updateFlag = false;
    for (const auto &desc : connectedDevices_) {
    ...
    CHECK_AND_RETURN_RET_LOG(updateFlag == true, false, "Update usage fails");
    return true;
}
...
shared_ptr<AudioDeviceDescriptor> AudioDeviceManager::GetBuiltinOutputDevice(DeviceType devType)
{
...
{
    if (desc->deviceType_ == DEVICE_TYPE_DP && AudioZoneService::GetInstance().CheckDeviceInAudioZone(*desc)) {
...
bool AudioDeviceManager::IsDeviceAvailable(AudioDeviceUsage audioDevUsage,
    const std::shared_ptr<AudioDeviceDescriptor> &desc)
{
    auto IsDeviceUsageSupported = [](AudioDeviceUsage audioDevUsage,
        const std::shared_ptr<AudioDeviceDescriptor> &deviceDesc) -> bool {
    ...
    if (desc != nullptr && !desc->exceptionFlag_ && desc->isEnable_ &&
...
void AudioDeviceManager::UpdateDeviceDescriptorByCapability(std::shared_ptr<AudioDeviceDescriptor> deviceDescriptor)
{
    CHECK_AND_RETURN_LOG(deviceDescriptor != nullptr, "devDesc is nullptr");
    auto isPresent = [&deviceDescriptor](const shared_ptr<AudioDeviceDescriptor> &desc) {
        return desc->deviceRole_ == deviceDescriptor->deviceRole_
            && desc->deviceType_ == deviceDescriptor->deviceType_
            && desc->networkId_ == deviceDescriptor->networkId_
    ...
    std::lock_guard<std::mutex> currentActiveDevicesLock(currentActiveDevicesMutex_);
    auto itr = std::find_if(connectedDevices_.begin(), connectedDevices_.end(), isPresent);
    CHECK_AND_RETURN_LOG(itr != connectedDevices_.end(), "Device not found");
    AudioDeviceStatus::GetInstance().UpdateDeviceDescriptorByCapability(*(*itr));
}
}
}
