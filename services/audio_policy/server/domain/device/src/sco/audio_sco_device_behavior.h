...
#ifndef AUDIO_SCO_DEVICE_BEHAVIOR_H
#define AUDIO_SCO_DEVICE_BEHAVIOR_H

#include "i_device_behavior_base.h"

namespace OHOS {
namespace AudioStandard {
class AudioScoDeviceBehavior : public IDeviceBehaviorBase {
public:
    AudioScoDeviceBehavior() = default;
    virtual ~AudioScoDeviceBehavior() = default;
...
private:
    int32_t HandleScoOutputDeviceFetched(const std::shared_ptr<AudioStreamDescriptor> &streamDesc,
        const AudioStreamDeviceChangeReasonExt reason);
    int32_t BluetoothScoInputFetch(const std::shared_ptr<AudioStreamDescriptor> &streamDesc);
