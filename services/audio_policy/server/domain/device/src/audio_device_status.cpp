    ...
    const std::string& deviceName, const AudioStreamInfo& streamInfo, DeviceRole role, bool hasPair)
{
    ...
    CheckAndWriteDeviceChangeExceptionEvent(result == SUCCESS, reason, devType, role, result, "handle type failed.");
    ...
    CHECK_AND_RETURN_LOG(coreService != nullptr, "AudioCoreService is null");
    ...
    coreService->FetchOutputDeviceAndRoute("OnDeviceStatusUpdated_2", reason);
    coreService->FetchInputDeviceAndRoute("OnDeviceStatusUpdated_2", reason);
    ...
    AudioCoreService::GetCoreService()->ReloadSourceForDeviceChange(
        audioRouterSelectStrategy_.Get1stCurrentInputDevice(),
        audioRouterSelectStrategy_.Get1stCurrentOutputDevice(), "OnDeviceStatusUpdated 5 param");
}
    ...
    const vector<std::shared_ptr<AudioDeviceDescriptor>> &desc, bool isConnected)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN(audioPolicyServerHandler_ != nullptr,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(
            IPCSkeleton::GetCallingUid(), ERR_PLAY_DEVICE_SWITCH_CALLBACK_NULL_POINTER,
            "TriggerAvailableDeviceChangedCallback handler is nullptr", false));
    ...
    bool isConnected)
{
    ...
    CHECK_AND_CALL_FUNC_RETURN(audioPolicyServerHandler_ != nullptr,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(IPCSkeleton::GetCallingUid(),
            ERR_PLAY_DEVICE_SWITCH_CALLBACK_NULL_POINTER, "TriggerDeviceChangedCallback handler is nullptr", false));
...
int32_t AudioDeviceStatus::UpdateNearlinkDeviceVolume(AudioDeviceDescriptor &updatedDesc)
{
    ...
    audioVolumeManager_.SetNearlinkDeviceVolume(updatedDesc.macAddress_, STREAM_MUSIC, volume);

    volume =
        SleAudioDeviceManager::GetInstance().GetVolumeLevelByVolumeType(STREAM_VOICE_CALL, updatedDesc);
    audioVolumeManager_.SetNearlinkDeviceVolume(updatedDesc.macAddress_, STREAM_VOICE_CALL, volume);
    if (!VolumeUtils::IsPCVolumeEnable()) {
        volume = audioVolumeManager_.GetMaxVolumeLevel(STREAM_VOICE_ASSISTANT);
        audioVolumeManager_.SetNearlinkDeviceVolume(updatedDesc.macAddress_, STREAM_VOICE_ASSISTANT, volume);
...
int32_t AudioDeviceStatus::HandleLocalDeviceDisconnected(const AudioDeviceDescriptor &updatedDesc)
{
    if (updatedDesc.deviceType_ == DEVICE_TYPE_BLUETOOTH_A2DP) {
        ...
        AudioCoreService::GetCoreService()->CloseUsbArmDevice(updatedDesc);
    } else if (updatedDesc.deviceType_ == DEVICE_TYPE_ACCESSORY) {
    ...
    for (auto &moduleInfo : moduleInfoList) {
        ...
        if (audioIOHandleMap_.CheckIOHandleExist(moduleInfo.name) == false) {
            ...
            if (moduleInfo.role == ROLE_SINK) {
                ...
                AudioCoreService::GetCoreService()->SetDpSinkModuleInfo(moduleInfo);
            }
...
void AudioDeviceStatus::OnPnpDeviceStatusUpdated(AudioDeviceDescriptor &desc, bool isConnected)
{
    ...
    if (desc.deviceType_ != DEVICE_TYPE_LINE_DIGITAL) {
    ...
    const std::string &deviceName, const AudioStreamInfo &streamInfo)
{
    ...
    for (auto &moduleInfo : moduleInfoList) {
        ...
        audioPolicyManager_.RemoveAudioPipeVolume(activateDeviceIOHandle);
...
DeviceType AudioDeviceStatus::GetDeviceTypeFromPin(AudioPin hdiPin)
{
    ...
    switch (hdiPin) {
        ...
        case OHOS::AudioStandard::AUDIO_PIN_IN_LINEIN:
            return DeviceType::DEVICE_TYPE_LINE_DIGITAL;
...
void AudioDeviceStatus::OnDeviceStatusUpdated(DStatusInfo statusInfo, bool isStop)
{
    ...
    if (isStop) {
        ...
        AudioCoreService::GetCoreService()->ReloadSourceForDeviceChange(
            audioRouterSelectStrategy_.Get1stCurrentInputDevice(),
            audioRouterSelectStrategy_.Get1stCurrentOutputDevice(), "OnDeviceStatusUpdated 2.1 param");
        return;
    }
    AudioStreamDeviceChangeReasonExt reason = AudioStreamDeviceChangeReasonExt::ExtEnum::UNKNOWN;
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> descForCb = {};
    ...
    AudioCoreService::GetCoreService()->ReloadSourceForDeviceChange(
        audioRouterSelectStrategy_.Get1stCurrentInputDevice(),
        audioRouterSelectStrategy_.Get1stCurrentOutputDevice(), "OnDeviceStatusUpdated 2.2 param");
}
    ...
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> &descForCb, AudioStreamDeviceChangeReasonExt &reason)
{
    ...
    if (statusInfo.isConnected) {
        ...
        if (connDevDesc != nullptr) {
            ...
            UpdateDeviceDescriptorByCapability(*connDevDesc);
            audioDeviceManager_.UpdateDeviceDescriptorByCapability(connDevDesc);
            audioDeviceCommon_.OnPreferredOutputDeviceUpdated(connDevDesc, AudioStreamDeviceChangeReason::OVERRODE);
            AUDIO_INFO_LOG("Update capability, send preferred output device update");
            return SUCCESS_BUT_NOT_CONTINUE;
        }
...
bool AudioDeviceStatus::OpenPortAndAddDeviceOnServiceConnected(AudioModuleInfo &moduleInfo)
{
    ...
    if (needOpenPort) {
        ...
        if (devType == DEVICE_TYPE_SPEAKER) {
            ...
            AudioCoreService::GetCoreService()->SetAudioScene(audioSceneManager_.GetAudioScene());
        }
    }

    if (devType == DEVICE_TYPE_MIC) {
        AudioCoreService::GetCoreService()->SetPrimaryMicModuleInfo(moduleInfo);
    }
...
void AudioDeviceStatus::AddDevice(const PolicyAdapterInfo &adapterInfo, const AdapterDeviceInfo &deviceInfo)
{
    ...
    AUDIO_INFO_LOG("add device name: '%{public}s' addr: '%{public}s'",
        deviceInfo.name_.c_str(), deviceInfo.address_.c_str());
    audioDescriptor->SetDeviceInfo(deviceInfo.name_, deviceInfo.address_);
...
int32_t AudioDeviceStatus::OnServiceConnected(AudioServiceIndex serviceIndex)
{
    ...
    if (result == SUCCESS) {
        ...
        audioRouterSelectStrategy_.UpdateCurrentOutputDevice(SYSTEM_UID, {outDevice});
        shared_ptr<AudioDeviceDescriptor> inDevice = audioDeviceManager_.GetCaptureDefaultDevice();
        audioRouterSelectStrategy_.UpdateCurrentInputDevice(SYSTEM_UID, {inDevice});
        AudioDeviceDescriptor curDevice = audioRouterSelectStrategy_.Get1stCurrentOutputDevice();
        audioVolumeManager_.SetVolumeForSwitchDevice(curDevice);
        OnPreferredDeviceUpdated(audioRouterSelectStrategy_.Get1stCurrentOutputDevice(),
            audioActiveDevice_.GetCurrentInputDeviceType(SYSTEM_UID));
        AddEarpiece();
    ...
    sptr<AudioRendererFilter> filter, const std::string &caller)
{
    ...
    for (auto &devDesc : devDescs) {
        if (devDesc->deviceRole_ == OUTPUT_DEVICE) {
            AudioCoreService::GetCoreService()->SelectOutputDevice(filter, {devDesc}, 0, isNeedNotifyBt);
        }
    }
}
    ...
    const std::string &caller)
{
    ...
    FetchDeviceInfo info = {
        STREAM_USAGE_VOICE_COMMUNICATION, "OnPrivacyDeviceSelected", ROUTER_TYPE_USER_SELECT
    };
    ...
    bool isConnected)
{
    ...
    AudioCoreService::GetCoreService()->ReloadSourceForDeviceChange(
        audioRouterSelectStrategy_.Get1stCurrentInputDevice(),
        audioRouterSelectStrategy_.Get1stCurrentOutputDevice(), "OnDeviceStatusUpdated 2 param");
}

void AudioDeviceStatus::OnConnectFailed(AudioDeviceDescriptor &desc)
{
    CHECK_AND_RETURN(AudioRouterSelectStrategy::GetInstance().IsPreferredDevice(desc));
    ...
    CHECK_AND_RETURN(audioPolicyServerHandler_ != nullptr);
...
void AudioDeviceStatus::OnDeviceInfoUpdated(AudioDeviceDescriptor &desc, const DeviceInfoUpdateCommand command)
{
    ...
    bool isStatusUpdated = audioDeviceManager_.UpdateDevicesListInfo(audioDescriptor, command, reason);
    CheckForA2dpSuspend(desc);
    if (isStatusUpdated) {
        OnPreferredStateUpdated(desc, command, reason);
    }
#ifdef BLUETOOTH_ENABLE
    ClearActiveHfpDevice(desc, command, reason);
#endif
    ...
    AudioCoreService::GetCoreService()->ReloadSourceForDeviceChange(
        audioRouterSelectStrategy_.Get1stCurrentInputDevice(),
        audioRouterSelectStrategy_.Get1stCurrentOutputDevice(), "OnDeviceInfoUpdated");
}
...
std::vector<std::shared_ptr<AudioDeviceDescriptor>> AudioDeviceStatus::UserSelectDeviceMapInit()
{
    ...
    shared_ptr<AudioDeviceDescriptor> userSelectMediaRenderDevice =
        strategy.GetMediaOutputDevice(INVALID_UID, INVALID_STREAM_ID);
    shared_ptr<AudioDeviceDescriptor> userSelectCallRenderDevice =
        strategy.GetCallOutputDevice(INVALID_UID, INVALID_STREAM_ID);
    shared_ptr<AudioDeviceDescriptor> userSelectCallCaptureDevice =
        strategy.GetCallInputDevice(INVALID_UID, INVALID_STREAM_ID);
    shared_ptr<AudioDeviceDescriptor> userSelectRecordCaptureDevice =
        strategy.GetMediaInputDevice(INVALID_UID, INVALID_STREAM_ID, SOURCE_TYPE_INVALID);
    ...
    const DeviceInfoUpdateCommand updateCommand, AudioStreamDeviceChangeReasonExt &reason)
{
    ...
    if (updateCommand == CATEGORY_UPDATE) {
        if (desc.deviceCategory_ == BT_UNWEAR_HEADPHONE) {
            ...
            AudioRouterSelectStrategy::GetInstance().UnexcludeDevices(unexcludedDevice, D_ALL_DEVICES);
...
void AudioDeviceStatus::ClearPreferredWhenCategoryUpdated(AudioDeviceDescriptor &desc)
{
    ...
    FetchDeviceInfo info = {
        STREAM_USAGE_MEDIA, "ClearPreferredWhenCategoryUpdated_1", ROUTER_TYPE_USER_SELECT
    };
    auto mediaFetched = *AudioRouterCenter::GetAudioRouterCenter()
        .FetchOutputDevices(info)
        .front();
    auto &strategy = AudioRouterSelectStrategy::GetInstance();
    if ((usage & MEDIA) == MEDIA &&
        NeedClearPreferredWhenCategoryUpdated(*existed, mediaFetched,
            *strategy.GetMediaOutputDevice(INVALID_UID, INVALID_STREAM_ID))) {
    ...
    info = {
        STREAM_USAGE_VOICE_COMMUNICATION, "ClearPreferredWhenCategoryUpdated_2", ROUTER_TYPE_USER_SELECT
    };
    auto callFetched = *AudioRouterCenter::GetAudioRouterCenter()
        .FetchOutputDevices(info)
        .front();
    if ((usage & VOICE) == VOICE &&
        NeedClearPreferredWhenCategoryUpdated(*existed, callFetched,
            *strategy.GetCallOutputDevice(INVALID_UID, INVALID_STREAM_ID))) {
        AudioPolicyUtils::GetInstance().SetPreferredDevice(AUDIO_CALL_RENDER,
    ...
    AudioDeviceDescriptor &desc, const std::shared_ptr<AudioDeviceDescriptor> &selectDesc)
{
    ...
    if (userSelectDeviceMap[CALL_CAPTURE_ID]->deviceType_ == desc.deviceType_ &&
        ...
        userSelectDeviceMap[CALL_CAPTURE_ID]->connectState_ != VIRTUAL_CONNECTED) {
    ...
    if (userSelectDeviceMap[RECORD_CAPTURE_ID]->deviceType_ == desc.deviceType_ &&
        ...
        userSelectDeviceMap[RECORD_CAPTURE_ID]->connectState_ != VIRTUAL_CONNECTED) {
...
void AudioDeviceStatus::UpdateDeviceDescriptorByCapability(AudioDeviceDescriptor &device)
{
    ...
    if (!capability.streamInfoList_.empty()) {
        ...
        device.BuildCapabilitiesFromDeviceStreamInfo();
    ...
    device.hdPlaybackMode_ = static_cast<HdPlaybackMode>(capability.hdPlaybackMode_);
    AUDIO_INFO_LOG("Update hdPlaybackMode success, mode: %{public}d", device.hdPlaybackMode_);
}
