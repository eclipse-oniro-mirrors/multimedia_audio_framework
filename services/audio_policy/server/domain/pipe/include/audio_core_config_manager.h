 ...
 */
#ifndef AUDIO_CORE_CONFIG_MANAGER_H
#define AUDIO_CORE_CONFIG_MANAGER_H
...
class AudioCoreConfigManager {
public:
    static AudioCoreConfigManager& GetInstance()
    {
        static AudioCoreConfigManager instance;
        return instance;
    }
    ...
    void UpdateHasEarpieceForChipProp();
    void OnIsChipGenneralConfig(bool isChipGeneral);
    ...
    DirectPlaybackMode GetDirectPlaybackSupport(std::shared_ptr<AudioDeviceDescriptor> desc,
        const AudioStreamInfo &streamInfo);
    bool GetPlaybackStreamSupport(AudioStreamInfo &streamInfo, uint32_t flag,
        std::shared_ptr<AudioDeviceDescriptor> &audioDeviceDesc);
    bool IsMultichannelPlaybackSupported(AudioStreamInfo &streamInfo,
        std::vector<std::shared_ptr<AudioDeviceDescriptor>> &desc);
    bool IsDirectPlaybackSupported(AudioStreamInfo &streamInfo,
        std::vector<std::shared_ptr<AudioDeviceDescriptor>> &desc, int32_t usage);
    bool IsOffloadPlaybackSupported(AudioStreamInfo &streamInfo,
        std::vector<std::shared_ptr<AudioDeviceDescriptor>> &desc, int32_t usage);
    ...
    std::shared_ptr<PipeStreamPropInfo> GetAvailableStreamPropInfoForDevice(
    ...
    bool IsDeviceSimulated(std::shared_ptr<AudioDeviceDescriptor> desc);

    AudioCoreConfigManager() : audioDeviceManager_(AudioDeviceManager::GetAudioDeviceManager()),
        audioPolicyConfig_(AudioPolicyConfigData::GetInstance())
    {
    }
    ~AudioCoreConfigManager()
    {
    }
private:
    ...
    bool isChipGeneralConfig_ = false;
...
#endif // AUDIO_CORE_CONFIG_MANAGER_H
