...
public:
    ...
    int32_t SetSystemAppVolumePercentageForUid(int32_t appUid, int32_t volumePercentage) override;
    ...
    int32_t GetSystemAppVolumePercentageForUid(int32_t appUid, int32_t &volumePercentage) override;
    ...
    int32_t IsMultichannelPlaybackSupported(const AudioStreamInfo &streamInfo, int32_t streamUsage,
        bool &support) override;
    int32_t IsDirectPlaybackSupported(const AudioStreamInfo &streamInfo, int32_t streamUsage,
        bool &support) override;
    int32_t IsOffloadPlaybackSupported(const AudioStreamInfo &streamInfo, int32_t streamUsage,
        bool &support) override;
    ...
    int32_t SetMediaOutputDevice(int32_t deviceTypeIn) override;
    ...
    int32_t IsEnhancedRoutingSupported(bool &ret) override;
    ...
    void SetLatestMuteStateForUid(int32_t uid, bool setMute);

    int32_t SetSourceOutputStreamMuteByStreamId(int32_t sessionId, bool mute);

    void NotifyVoipPriorityLoweredByMute(uint32_t streamId, bool lowered);

    int32_t SetSourceOutputStreamMute(int32_t uid, bool mute);
    ...
    int32_t ActivateAudioSession(int32_t strategy, bool stateChangeCallbackFlag = false) override;
    ...
    int32_t SetRegisterInterruptCallbackFlag(const uint32_t streamId, const bool interruptCallbackFlag) override;

    int32_t SetRegisterStateChangeCallbackFlag(const bool stateChangeCallbackFlag) override;
    ...
    int32_t SetAudioSessionCapturerMuteHint(bool mute) override;
    ...
    int32_t SetOffloadAllowedForUid(uint32_t uid, bool allowed) override;

    int32_t SetAppSilentOnDisplay(const int32_t displayId = -1) override;

    int32_t DeactivateAudioInterrupt(const AudioInterrupt &audioInterrupt, int32_t zoneId,
        bool isRemoveFocusHis = false) override;
    ...
    int32_t SetAncoSourceOutputStreamMute(int32_t uid, bool setMute) override;
    ...
    int32_t GetLoopbackSupportedDevicePairs(std::vector<std::shared_ptr<AudioDeviceDescriptor>> &inputDevices,
        std::vector<std::shared_ptr<AudioDeviceDescriptor>> &outputDevices, int32_t &ret) override;
    int32_t GetLoopbackPreferredDevicePair(std::shared_ptr<AudioDeviceDescriptor> &inputDevice,
        std::shared_ptr<AudioDeviceDescriptor> &outputDevice, int32_t &ret) override;
    ...
    int32_t SetLoopbackVolume(float volume, bool &ret) override;
    int32_t GetLoopbackVolume(float &volume, bool &ret) override;

    int32_t CreateLoopback(int32_t mode, int32_t type, const sptr<IRemoteObject> &cbObject,
        sptr<IRemoteObject> &loopback, int32_t &ret) override;
    int32_t DestroyLoopback(int32_t type, int32_t &ret) override;
    ...
    int32_t SetDeclaredDeviceTypes(const std::set<int32_t> &deviceTypes) override;
    ...
    int32_t SetCallbackRendererInfo(const AudioRendererInfo &rendererInfo, const int32_t uid = INVALID_UID) override;

    int32_t SetCallbackCapturerInfo(const AudioCapturerInfo &capturerInfo, const int32_t uid = INVALID_UID) override;
    ...
    int32_t GetAudioDebugInfo(uint32_t sessionId, std::shared_ptr<AudioDebugInfo> &audioDebugInfo) override;

    int32_t GetSessionDebugInfo(std::shared_ptr<AudioSessionDebugInfo> &sessionDebugInfo) override;
    ...
    int32_t IsPersonalizedSpatializationEnabled(const std::string &address, bool &ret) override;

    int32_t SetPersonalizedSpatializationEnabled(
        const std::shared_ptr<AudioDeviceDescriptor> &selectedAudioDevice, const bool enable) override;

    int32_t DownloadPersonalizedHRTF(int32_t fd, int64_t length) override;

    int32_t GetCurrentSpatialAudioSourceType(int32_t &ret) override;
    ...
    int32_t IsPersonalizedSpatializationSupported(bool &ret) override;
    ...
    int32_t GetAudioZoneForApp(const std::vector<int32_t> &userIds, std::vector<int32_t> &retUserIds) override;
    ...
    int32_t EnableSystemVolumeProxy(int32_t zoneId, int32_t deviceType, bool enable) override;
    ...
    int32_t AddStreamIdToAudioZone(int32_t zoneId, uint32_t streamId) override;

    int32_t RemoveStreamIdFromAudioZone(int32_t zoneId, uint32_t streamId) override;
    ...
    int32_t GetCurrentRendererChangeInfosForZone(int32_t zoneId,
        std::vector<std::shared_ptr<AudioRendererChangeInfo>> &audioRendererChangeInfos) override;
    ...
    int32_t SetSplitModeReady() override;
    ...
    int32_t IsCollaborativePlaybackSupportedForDevice(
        const std::shared_ptr<AudioDeviceDescriptor> &selectedAudioDevice, bool &isSupported) override;
    ...
    int32_t RecordBootAnimationClientStateTime(const BootAnimationInfo &info) override;
    void RegisterPermStateChangeCallback();
    void GetScreenStates();
    ...
    int32_t UpdateSafeVolumeByStrMode() override;
    ...
    int32_t SetAudioSessionBehavior(const uint32_t behavior) override;
    int32_t GetActiveStreamsVolumeInfo(std::vector<ActiveStreamVolumeInfo> &activeStreamsVolumeInfo) override;
    int32_t SetAudioZoneVolumeConfig(const std::string &zoneName,
        const std::vector<AudioZoneVolumeConfig> &configs) override;
    int32_t TriggerAudioZoneVolumeSync() override;

    int32_t IsAudioSeparationEffectSupported(bool &ret) override;
    int32_t SetAudioSeparationEffectEnabled(bool enabled, int32_t uid, int64_t streamId) override;
    int32_t SetAudioSeparationEffectVolume(int32_t type, double volume) override;
...
private:
    ...
    void Init();
    int32_t HandleRouteSelectorCallbackEnable(bool enable);
    int32_t VerifyVoiceCallPermission(uint64_t fullTokenId, Security::AccessToken::AccessTokenID tokenId);
    int32_t ValidateAudioInterrupt(const AudioInterrupt &audioInterrupt);
    ...
    int32_t SetRingerModeInternal(AudioRingerMode inputRingerMode, bool hasUpdatedRingtoneVolume = false);
    ...
    bool VerifyPermission(const std::string &permissionName, uint32_t tokenId = 0, bool isRecording = false);
    ...
    void SubscribeVolumeKeyEventsRetryFunc();
    ...
    void SendMonitortEvent(const int32_t keyType, int32_t resultOfVolumeKey);
    ...
    bool IsDeviceSupportLoopback(std::shared_ptr<AudioDeviceDescriptor> device);

    int32_t SetRingerModeInner(AudioRingerMode ringMode);
    void AddSystemAbilityListeners();
    void AddSystemAbilityListenerAfterAudioServerStart();
    void HandleAudioServerRelatedSystemAbility(int32_t systemAbilityId, const std::string &deviceId);
    ...
    int32_t UpdateAudioSceneAfterActivateSession(const int32_t zoneId, const int32_t callerPid,
        const AudioSessionStrategy &strategy, const bool isStandalone, const bool stateChangeCallbackFlag = false,
        int32_t callerUid = INVALID_UID);
    bool IsScreenOnAdjustVolume();
    bool ShouldNotifyForLegacyVolume(int32_t volumeLevel);
    int32_t VerifyUidAndPid(const int32_t uid, const int32_t pid);
    ...
    std::shared_ptr<AudioLoopbackManager> loopbackManager_;
    ...
    std::atomic<bool> isScreenOn_ = false; // for TV
    ...
    std::unordered_set<int32_t> failedVolumeKeyEvents_;
#endif
    ...
    std::atomic<float> loopbackVolume_ = 0.0f;
    std::mutex descLock_;
    ...
    int32_t micRefEnableState_ = 0;
    int32_t sessionIdByRemote_ = -1;
    bool isNeedPublished_ = false;
