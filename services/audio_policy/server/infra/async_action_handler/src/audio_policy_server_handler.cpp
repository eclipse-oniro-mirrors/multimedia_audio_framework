...
static const int32_t FOCUS_REQUESTED_TIMEOUT_SECONDS = 60;
...
void AudioPolicyServerHandler::RemoveAudioPolicyClientProxyMap(pid_t clientPid)
{
    ...
    lastSentOutputDeviceMap_.erase(clientPid);
    lastSentInputDeviceMap_.erase(clientPid);
    pidsStrForPrinting_ = GeneratePidsStrForPrinting(audioPolicyClientProxyAPSCbsMap_);
...
bool AudioPolicyServerHandler::SendVolumeKeyEventCallback(const VolumeEvent &volumeEvent, const int32_t zoneId)
{
    std::shared_ptr<EventContextObj> eventContextObj = std::make_shared<EventContextObj>();
    CHECK_AND_RETURN_RET_LOG(eventContextObj != nullptr, false, "EventContextObj get nullptr");
...
bool AudioPolicyServerHandler::SendVolumeDegreeEventCallback(const VolumeEvent &volumeEvent, const int32_t zoneId)
{
    std::shared_ptr<EventContextObj> eventContextObj = std::make_shared<EventContextObj>();
    CHECK_AND_RETURN_RET_LOG(eventContextObj != nullptr, false, "EventContextObj get nullptr");
    if (volumeEvent.volumeType == AudioStreamType::STREAM_VOICE_CALL_ASSISTANT) {
...
bool AudioPolicyServerHandler::SendSelfAppVolumeChangeCallback(int32_t appUid, const VolumeEvent &volumeEvent)
{
    std::shared_ptr<EventContextObj> eventContextObj = std::make_shared<EventContextObj>();
    CHECK_AND_RETURN_RET_LOG(eventContextObj != nullptr, false, "EventContextObj get nullptr");
    ...
    eventContextObj->volumeEvent = volumeEvent;
    lock_guard<mutex> runnerlock(runnerMutex_);
    bool ret = SendEvent(AppExecFwk::InnerEvent::Get(EventAudioServerCmd::SELF_APP_VOLUME_CHANGE_EVENT,
        eventContextObj));
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ret == SUCCESS, ret,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(IPCSkeleton::GetCallingUid(),
            ERR_VOLUME_CALLBACK_FAILED, "Send SELF_APP_VOLUME_CHANGE_EVENT event failed", false),
        "Send SELF_APP_VOLUME_CHANGE_EVENT event failed");
    return ret;
}

bool AudioPolicyServerHandler::SendSystemAppVolumeChangeCallback(int32_t appUid, const VolumeEvent &volumeEvent)
{
    std::shared_ptr<EventContextObj> eventContextObj = std::make_shared<EventContextObj>();
    CHECK_AND_RETURN_RET_LOG(eventContextObj != nullptr, false, "EventContextObj get nullptr");
    eventContextObj->appUid = appUid;
    eventContextObj->volumeEvent = volumeEvent;
    lock_guard<mutex> runnerlock(runnerMutex_);
    bool ret = SendEvent(AppExecFwk::InnerEvent::Get(EventAudioServerCmd::SYSTEM_APP_VOLUME_CHANGE_EVENT,
        eventContextObj));
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ret, ret,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(IPCSkeleton::GetCallingUid(),
            ERR_VOLUME_CALLBACK_FAILED, "Send SYSTEM_APP_VOLUME_CHANGE_EVENT event failed", false),
        "Send SYSTEM_APP_VOLUME_CHANGE_EVENT event failed");
    return ret;
}
...
{
    std::shared_ptr<EventContextObj> eventContextObj = std::make_shared<EventContextObj>();
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(eventContextObj != nullptr, false,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            INTERRUPT_CALLBACK_NULL_POINTER, "EventContextObj get nullptr", false),
        "EventContextObj get nullptr");
    eventContextObj->interruptEvent = interruptEvent;
    ...
    CHECK_AND_CALL_FUNC_RETURN_RET_LOG(ret, ret,
        StreamDfxManager::GetInstance().SendAudioErrorEvent(static_cast<int32_t>(getuid()),
            INTERRUPT_CALLBACK_NULL_POINTER, "Send INTERRUPT_EVENT_WITH_STREAMID event failed", false),
        "Send INTERRUPT_EVENT_WITH_STREAMID event failed");
    return ret;
}
...
bool AudioPolicyServerHandler::SendRendererDeviceChangeEvent(const int32_t clientPid, const uint32_t sessionId,
    ...
    const AudioDeviceDescriptor &preOutputDeviceInfo)
{
    std::shared_ptr<RendererDeviceChangeEvent> eventContextObj = std::make_shared<RendererDeviceChangeEvent>(
        clientPid, sessionId, outputDeviceInfo, reason, preOutputDeviceInfo);
    CHECK_AND_RETURN_RET_LOG(eventContextObj != nullptr, false, "EventContextObj get nullptr");
...
bool AudioPolicyServerHandler::SendBypassSpatialForStereo(const std::shared_ptr<AudioDeviceDescriptor> &selectedDevice,
    bool spatializationEnabled, bool headtrackingEnabled)
{
    std::shared_ptr<EventContextObj> eventContextObj = std::make_shared<EventContextObj>();
    CHECK_AND_RETURN_RET_LOG(eventContextObj != nullptr, false, "EventContextObj get nullptr");
    eventContextObj->spatializationEnabled = spatializationEnabled;
    eventContextObj->headTrackingEnabled = headtrackingEnabled;
    eventContextObj->descriptor = selectedDevice;
    lock_guard<mutex> runnerlock(runnerMutex_);
    bool ret = SendEvent(AppExecFwk::InnerEvent::Get(
        EventAudioServerCmd::BYPASS_SPATIAL_FOR_STEREO_CHANGE_FOR_CURRENT_DEVICE, eventContextObj));
    CHECK_AND_RETURN_RET_LOG(ret, ret, "Send BYPASS_SPATIAL_FOR_STEREO_CHANGE_FOR_CURRENT_DEVICE event failed");
    AUDIO_INFO_LOG("Send BYPASS_SPATIAL_FOR_STEREO_CHANGE_FOR_CURRENT_DEVICE event %{public}d, %{public}d",
        spatializationEnabled, headtrackingEnabled);
    return ret;
}

bool AudioPolicyServerHandler::SendSpatialAudioSourceTypeChangeEvent(SpatialAudioSourceType mode)
{
    std::shared_ptr<EventContextObj> eventContextObj = std::make_shared<EventContextObj>();
    CHECK_AND_RETURN_RET_LOG(eventContextObj != nullptr, false, "EventContextObj get nullptr");
    eventContextObj->mode = mode;
    lock_guard<mutex> runnerlock(runnerMutex_);
    bool ret = SendEvent(AppExecFwk::InnerEvent::Get(
        EventAudioServerCmd::EFFECT_MODE_CHANGE_FOR_ANY_DEVICE, eventContextObj));
    CHECK_AND_RETURN_RET_LOG(ret, ret, "Send EFFECT_MODE_CHANGE_FOR_ANY_DEVICE event failed");
    AUDIO_INFO_LOG("Send EFFECT_MODE_CHANGE_FOR_ANY_DEVICE event %{public}d", mode);
    return ret;
}

bool AudioPolicyServerHandler::SendPersonalizedSpatializationEnabledChangeForAnyDeviceEvent(
    const std::shared_ptr<AudioDeviceDescriptor> &selectedAudioDevice, const bool &enabled)
{
    std::shared_ptr<EventContextObj> eventContextObj = std::make_shared<EventContextObj>();
    CHECK_AND_RETURN_RET_LOG(eventContextObj != nullptr, false, "EventContextObj get nullptr");
    eventContextObj->personalizedSpatializationEnabled = enabled;
    eventContextObj->descriptor = selectedAudioDevice;
    lock_guard<mutex> runnerlock(runnerMutex_);
    bool ret = SendEvent(AppExecFwk::InnerEvent::Get(
        EventAudioServerCmd::PERSONALIZED_SPATIALIZATION_ENABLED_CHANGE_FOR_ANY_DEVICE, eventContextObj));
    CHECK_AND_RETURN_RET_LOG(ret, ret, "Send PERSONALIZED_SPATIALIZATION_ENABLED_CHANGE_FOR_ANY_DEVICE event failed");
    AUDIO_INFO_LOG("Send PERSONALIZED_SPATIALIZATION_ENABLED_CHANGE_FOR_ANY_DEVICE event");
    return ret;
}

bool AudioPolicyServerHandler::SendAudioSeparationEffectEnabledChangeEvent(const bool &enabled)
{
    std::shared_ptr<EventContextObj> eventContextObj = std::make_shared<EventContextObj>();
    CHECK_AND_RETURN_RET_LOG(eventContextObj != nullptr, false, "EventContextObj get nullptr");
    eventContextObj->audioSeparationEffectEnabled = enabled;
    lock_guard<mutex> runnerlock(runnerMutex_);
    bool ret = SendEvent(AppExecFwk::InnerEvent::Get(
        EventAudioServerCmd::AUDIO_SEPARATION_EFFECT_ENABLED_CHANGE, eventContextObj));
    CHECK_AND_RETURN_RET_LOG(ret, ret, "Send AUDIO_SEPARATION_EFFECT_ENABLED_CHANGE event failed");
    return ret;
}
    ...
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> &deviceDescs)
{
    ...
    if (IsForceGetZoneDevice(pidUidMap_[clientPid]) && zoneId > 0) {
        deviceDescs = AudioZoneService::GetInstance().FetchOutputDevices(zoneId,
            STREAM_USAGE_UNKNOWN, 0, ROUTER_TYPE_DEFAULT);
        return;
    }
    zoneId = AudioZoneService::GetInstance().FindAudioZoneByUid(pidUidMap_[clientPid], OUTPUT_DEVICE);
    CHECK_AND_RETURN(zoneId != 0);
    auto tmpDeviceDescs = AudioZoneService::GetInstance().FetchOutputDevices(zoneId,
        STREAM_USAGE_UNKNOWN, 0, ROUTER_TYPE_DEFAULT);
    CHECK_AND_RETURN(!tmpDeviceDescs.empty());
    deviceDescs = tmpDeviceDescs;
}
...
bool AudioPolicyServerHandler::IsTargetDeviceForVolumeKeyEvent(int32_t pid, const VolumeEvent &volumeEvent)
{
    ...
    bool isForceVerify = IsForceGetDevByVolumeType(uid);
    CHECK_AND_RETURN_RET(!isForceVerify, true);
    auto deviceDesc = AudioActiveDevice::GetInstance().GetActiveDeviceForVolume(uid);
...
bool AudioPolicyServerHandler::IsTargetZoneForVolumeKeyEvent(int32_t pid, int32_t zoneId)
{
    CHECK_AND_RETURN_RET(pidUidMap_.count(pid) > 0, true);
    int32_t uid = pidUidMap_[pid];
    int zoneIdForUid = AudioZoneService::GetInstance().FindAudioZoneByUid(uid, OUTPUT_DEVICE);
    AUDIO_INFO_LOG("uid: %{public}d, zoneId: %{public}d, target zoneId: %{public}d, is target zone: %{public}s",
        uid, zoneIdForUid, zoneId, zoneId == zoneIdForUid ? "true" : "false");
    return zoneId == zoneIdForUid;
}
...
void AudioPolicyServerHandler::HandleVolumeKeyEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    ...
    for (auto it = audioPolicyClientProxyAPSCbsMap_.begin(); it != audioPolicyClientProxyAPSCbsMap_.end(); ++it) {
        ...
        CHECK_AND_CONTINUE(IsTargetZoneForVolumeKeyEvent(it->first, eventContextObj->zoneId));
#endif
...
void AudioPolicyServerHandler::HandleRequestCateGoryEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    std::shared_ptr<EventContextObj> eventContextObj = event->GetSharedObject<EventContextObj>();
    CHECK_AND_RETURN_LOG(eventContextObj != nullptr, "EventContextObj get nullptr");
    AudioXCollie audioXCollie("AudioPolicyServerHandler::HandleRequestCateGoryEvent", FOCUS_REQUESTED_TIMEOUT_SECONDS,
        [](void *) {
            AUDIO_ERR_LOG("HandleRequestCateGoryEvent timeout");
...
void AudioPolicyServerHandler::HandleSelfAppVolumeChangeEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    std::shared_ptr<EventContextObj> eventContextObj = event->GetSharedObject<EventContextObj>();
    CHECK_AND_RETURN_LOG(eventContextObj != nullptr, "EventContextObj get nullptr");
    std::lock_guard<std::mutex> lock(handleMapMutex_);
    for (auto it = audioPolicyClientProxyAPSCbsMap_.begin(); it != audioPolicyClientProxyAPSCbsMap_.end(); ++it) {
        ...
        if (clientCallbacksMap_.count(it->first) > 0 &&
            ...
            appVolumeChangeListenerCb->OnSelfAppVolumeChanged(eventContextObj->appUid, eventContextObj->volumeEvent);
        }
    }
}

void AudioPolicyServerHandler::HandleSystemAppVolumeChangeEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    std::shared_ptr<EventContextObj> eventContextObj = event->GetSharedObject<EventContextObj>();
    CHECK_AND_RETURN_LOG(eventContextObj != nullptr, "EventContextObj get nullptr");
    std::lock_guard<std::mutex> lock(handleMapMutex_);
    for (auto it = audioPolicyClientProxyAPSCbsMap_.begin(); it != audioPolicyClientProxyAPSCbsMap_.end(); ++it) {
        CHECK_AND_CONTINUE_LOG(it->second != nullptr, "nullptr for client %{public}d", it->first);

        if (clientCallbacksMap_.count(it->first) > 0 &&
            clientCallbacksMap_[it->first].count(CALLBACK_APP_VOLUME_CHANGE) > 0 &&
            clientCallbacksMap_[it->first][CALLBACK_APP_VOLUME_CHANGE]) {
            AUDIO_INFO_LOG("client:%{public}d, appUid:%{public}d", it->first, eventContextObj->appUid);
            it->second->OnSystemAppVolumeChanged(eventContextObj->appUid, eventContextObj->volumeEvent);
        }
    }
}
...
void AudioPolicyServerHandler::HandleInterruptEventForAudioSession(const AppExecFwk::InnerEvent::Pointer &event)
{
    ...
    if (clientCallbacksMap_.count(iterator->first) > 0 &&
        ...
        PublishCtrlCmdEvent(hintType, eventContextObj->audioInterrupt.uid, eventContextObj->audioInterrupt.streamId);
    } else {
...
void AudioPolicyServerHandler::HandlePreferredOutputDeviceUpdated()
{
    ...
    std::stringstream ss;
    for (auto it = audioPolicyClientProxyAPSCbsMap_.begin(); it != audioPolicyClientProxyAPSCbsMap_.end(); ++it) {
        ProcessPreferredOutputDeviceForClient(it->first, it->second, ss);
    }
    CHECK_AND_RETURN(!ss.str().empty());
    AUDIO_INFO_LOG("Updated: %{public}s", ss.str().c_str());
}

void AudioPolicyServerHandler::ProcessPreferredOutputDeviceForClient(int32_t clientPid,
    const std::shared_ptr<AudioPolicyClientHolder>& clientHolder, std::stringstream &ss)
{
    ...
    for (auto rendererFilter : rendererFilterList) {
        ...
        if (!clientHolder->hasBTPermission_) {
            AudioPolicyService::GetAudioPolicyService().UpdateDescWhenNoBTPermission(deviceDescs);
        }
        CHECK_AND_CONTINUE(IsCallbackRegistered(clientPid, CALLBACK_PREFERRED_OUTPUT_DEVICE_CHANGE));
        CHECK_AND_CONTINUE_LOG(!deviceDescs.empty() && deviceDescs[0] != nullptr, "device is null.");

        int32_t uid = rendererFilter.uid;
        StreamUsage streamUsage = rendererFilter.rendererInfo.streamUsage;
        CHECK_AND_CONTINUE(!ShouldSkipDuplicateOutputCallback(clientPid, uid, streamUsage, deviceDescs[0]));

        lastSentOutputDeviceMap_[clientPid][uid][streamUsage] = deviceDescs[0];
        clientHolder->OnPreferredOutputDeviceUpdated(rendererFilter.rendererInfo, uid, deviceDescs);

        clientSs << uid << ":" << static_cast<int32_t>(streamUsage)
                 << "->" << deviceDescs[0]->deviceType_ << ":" << deviceDescs[0]->deviceId_ << ",";
    }

    std::string clientStr = clientSs.str();
    if (!clientStr.empty()) {
        ss << clientPid << "(" << clientStr << ") ";
    }
}

bool AudioPolicyServerHandler::IsCallbackRegistered(int32_t clientPid, CallbackChange callbackType)
{
    return clientCallbacksMap_.count(clientPid) > 0 &&
        clientCallbacksMap_[clientPid].count(callbackType) > 0 &&
        clientCallbacksMap_[clientPid][callbackType];
}

bool AudioPolicyServerHandler::ShouldSkipDuplicateOutputCallback(int32_t clientPid, int32_t uid,
    StreamUsage streamUsage, const std::shared_ptr<AudioDeviceDescriptor> &currentDevice)
{
    auto pidIt = lastSentOutputDeviceMap_.find(clientPid);
    if (pidIt == lastSentOutputDeviceMap_.end()) {
        return false;
    }
    auto uidIt = pidIt->second.find(uid);
    if (uidIt == pidIt->second.end()) {
        return false;
    }
    auto streamUsageIt = uidIt->second.find(streamUsage);
    if (streamUsageIt == uidIt->second.end() || streamUsageIt->second == nullptr) {
        return false;
    }
    if (currentDevice->IsSameDeviceDescPtr(streamUsageIt->second)) {
        AUDIO_DEBUG_LOG("Skip duplicate callback for pid:%{public}d, uid:%{public}d, streamUsage:%{public}d",
            clientPid, uid, static_cast<int32_t>(streamUsage));
        return true;
    }
    return false;
}
...
void AudioPolicyServerHandler::HandlePreferredInputDeviceUpdated()
{
    ...
    std::stringstream ss;
    for (auto it = audioPolicyClientProxyAPSCbsMap_.begin(); it != audioPolicyClientProxyAPSCbsMap_.end(); ++it) {
        ProcessPreferredInputDeviceForClient(it->first, it->second, ss);
    }
    CHECK_AND_RETURN(!ss.str().empty());
    AUDIO_INFO_LOG("Updated: %{public}s", ss.str().c_str());
}

void AudioPolicyServerHandler::ProcessPreferredInputDeviceForClient(int32_t clientPid,
    const std::shared_ptr<AudioPolicyClientHolder>& clientHolder, std::stringstream &ss)
{
    std::vector<AudioCapturerFilter> capturerFilterList = GetCallbackCapturerInfoList(clientPid);
    std::stringstream clientSs;
    for (auto capturerFilter : capturerFilterList) {
        auto deviceDescs = AudioCoreService::GetCoreService()->
            GetPreferredInputDeviceDescInner(capturerFilter.capturerInfo, LOCAL_NETWORK_ID, capturerFilter.uid);
        if (!clientHolder->hasBTPermission_) {
            AudioPolicyService::GetAudioPolicyService().UpdateDescWhenNoBTPermission(deviceDescs);
        }
        for (auto &desc : deviceDescs) {
        ...
        CHECK_AND_CONTINUE(IsCallbackRegistered(clientPid, CALLBACK_PREFERRED_INPUT_DEVICE_CHANGE));
        CHECK_AND_CONTINUE_LOG(!deviceDescs.empty() && deviceDescs[0] != nullptr, "device is null.");

        int32_t uid = capturerFilter.uid;
        SourceType sourceType = capturerFilter.capturerInfo.sourceType;
        CHECK_AND_CONTINUE(!ShouldSkipDuplicateInputCallback(clientPid, uid, sourceType, deviceDescs[0]));

        lastSentInputDeviceMap_[clientPid][uid][sourceType] = deviceDescs[0];
        clientHolder->OnPreferredInputDeviceUpdated(capturerFilter.capturerInfo, uid, deviceDescs);

        clientSs << uid << ":" << static_cast<int32_t>(sourceType)
                 << "->" << deviceDescs[0]->deviceType_ << ":" << deviceDescs[0]->deviceId_ << ",";
    }
...
bool AudioPolicyServerHandler::ShouldSkipDuplicateInputCallback(int32_t clientPid, int32_t uid,
    SourceType sourceType, const std::shared_ptr<AudioDeviceDescriptor> &currentDevice)
{
    auto pidIt = lastSentInputDeviceMap_.find(clientPid);
    if (pidIt == lastSentInputDeviceMap_.end()) {
        return false;
    }
    auto uidIt = pidIt->second.find(uid);
    if (uidIt == pidIt->second.end()) {
    ...
    auto sourceTypeIt = uidIt->second.find(sourceType);
    if (sourceTypeIt == uidIt->second.end() || sourceTypeIt->second == nullptr) {
        return false;
    }
    if (currentDevice->IsSameDeviceDescPtr(sourceTypeIt->second)) {
        AUDIO_DEBUG_LOG("Skip duplicate callback for pid:%{public}d, uid:%{public}d, sourceType:%{public}d",
            clientPid, uid, static_cast<int32_t>(sourceType));
        return true;
    }
    return false;
}
...
void AudioPolicyServerHandler::HandleAudioSessionDeviceChangeEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    ...
    for (auto it = audioPolicyClientProxyAPSCbsMap.begin(); it != audioPolicyClientProxyAPSCbsMap.end(); ++it) {
        ...
        if ((clientCallbacksMap.count(it->first) > 0) &&
            ...
            auto streamUsage = audioSessionService.GetAudioSessionStreamUsage(it->first);
            rendererInfo.streamUsage = (streamUsage <= STREAM_USAGE_UNKNOWN || streamUsage > STREAM_USAGE_MAX) ?
                STREAM_USAGE_MUSIC : streamUsage;
            auto uid = pidUidMap_.count(it->first) > 0 ? pidUidMap_[it->first] : INVALID_UID;
            deviceChangedEvent.devices = AudioCoreService::GetCoreService()->GetPreferredOutputDeviceDescInner(
                rendererInfo, LOCAL_NETWORK_ID, uid);
...
void AudioPolicyServerHandler::HandleRendererDeviceChangeEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    ...
    CHECK_AND_RETURN_LOG(eventContextObj != nullptr, "EventContextObj get nullptr");
    const auto &[pid, sessionId, outputDeviceInfo, reason, preOutputDeviceInfo] = *eventContextObj;
    bool cbFlag = false;
    ...
    if (clientCallbacksMap_.count(pid) > 0 &&
        ...
        cbFlag = true;
        capturerStateChangeCb->OnRendererDeviceChange(sessionId, outputDeviceInfo, reason, preOutputDeviceInfo);
    }
...
void AudioPolicyServerHandler::HandleCapturerCreateEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    ...
    if (AudioCoreService::GetCoreService() != nullptr &&
        AudioCoreService::GetCoreService()->GetEventEntry() != nullptr) {
...
void AudioPolicyServerHandler::HandleSpatializationEnabledChangeForBT(
    const AppExecFwk::InnerEvent::Pointer &event)
{
    std::shared_ptr<EventContextObj> eventContextObj = event->GetSharedObject<EventContextObj>();
    CHECK_AND_RETURN_LOG(eventContextObj != nullptr, "EventContextObj get nullptr");
    ...
    for (auto it = audioPolicyClientProxyAPSCbsMap_.begin(); it != audioPolicyClientProxyAPSCbsMap_.end(); ++it) {
        ...
        if (clientCallbacksMap_.count(it->first) > 0 &&
            clientCallbacksMap_[it->first].count(CALLBACK_SPATIALIZATION_ENABLED_CHANGE) > 0 &&
            clientCallbacksMap_[it->first][CALLBACK_SPATIALIZATION_ENABLED_CHANGE]) {
                AUDIO_INFO_LOG("HandleSpatializationEnabledChangeForBT enabled %{public}d %{public}d",
                    eventContextObj->spatializationEnabled, eventContextObj->headTrackingEnabled);
            spatializationEnabledChangeCb->OnSpatializationEnabledChangeForBTDevice(eventContextObj->descriptor,
                eventContextObj->spatializationEnabled, eventContextObj->headTrackingEnabled);
        }
    }
}

void AudioPolicyServerHandler::HandleSpatialAudioSourceTypeChange(const AppExecFwk::InnerEvent::Pointer &event)
{
    std::shared_ptr<EventContextObj> eventContextObj = event->GetSharedObject<EventContextObj>();
    CHECK_AND_RETURN_LOG(eventContextObj != nullptr, "EventContextObj get nullptr");
    std::lock_guard<std::mutex> lock(handleMapMutex_);
    for (auto it = audioPolicyClientProxyAPSCbsMap_.begin(); it != audioPolicyClientProxyAPSCbsMap_.end(); ++it) {
        std::shared_ptr<AudioPolicyClientHolder> spatialAudioSourceTypeChangeCb = it->second;
        if (spatialAudioSourceTypeChangeCb == nullptr) {
            AUDIO_ERR_LOG("spatialAudioSourceTypeChangeCb : nullptr for client : %{public}d", it->first);
            continue;
        }
        if (clientCallbacksMap_.count(it->first) > 0 &&
            clientCallbacksMap_[it->first].count(CALLBACK_SPATIAL_AUDIO_SOURCE_TYPE_CHANGE) > 0 &&
            clientCallbacksMap_[it->first][CALLBACK_SPATIAL_AUDIO_SOURCE_TYPE_CHANGE]) {
                AUDIO_INFO_LOG("AudioPolicyServerHandler::HandleSpatialAudioSourceTypeChange enabled %{public}d",
                    eventContextObj->mode);
            spatialAudioSourceTypeChangeCb->OnSpatialAudioSourceTypeChange(eventContextObj->mode);
        }
    }
}

void AudioPolicyServerHandler::HandlePersonalizedSpatializationEnabledChangeForAnyDeviceEvent(
    const AppExecFwk::InnerEvent::Pointer &event)
{
    std::shared_ptr<EventContextObj> eventContextObj = event->GetSharedObject<EventContextObj>();
    CHECK_AND_RETURN_LOG(eventContextObj != nullptr, "EventContextObj get nullptr");
    std::lock_guard<std::mutex> lock(handleMapMutex_);
    AUDIO_INFO_LOG("AudioPolicyServerHandler::HandlePersonalizedSpatializationEnabledChangeForAnyDeviceEvent");
    for (auto it = audioPolicyClientProxyAPSCbsMap_.begin(); it != audioPolicyClientProxyAPSCbsMap_.end(); ++it) {
        std::shared_ptr<AudioPolicyClientHolder> personalizedSpatializationEnabledChangeCb = it->second;
        CHECK_AND_CONTINUE_LOG(personalizedSpatializationEnabledChangeCb != nullptr,
            "PersonalizedSpatializationEnabledChangeCb : nullptr for client : %{public}d", it->first);
        CHECK_AND_CONTINUE(clientCallbacksMap_.count(it->first) > 0 &&
            clientCallbacksMap_[it->first].count(CALLBACK_PERSONALIZE_SPATIALIZATION_ENABLED_CHANGE) > 0 &&
            clientCallbacksMap_[it->first][CALLBACK_PERSONALIZE_SPATIALIZATION_ENABLED_CHANGE]);
        personalizedSpatializationEnabledChangeCb->OnPersonalizedSpatializationEnabledChangeForAnyDevice(
            eventContextObj->descriptor, eventContextObj->personalizedSpatializationEnabled);
    }
}

void AudioPolicyServerHandler::HandleAudioSeparationEffectEnabledChangeEvent(
    const AppExecFwk::InnerEvent::Pointer &event)
{
    std::shared_ptr<EventContextObj> eventContextObj = event->GetSharedObject<EventContextObj>();
    CHECK_AND_RETURN_LOG(eventContextObj != nullptr, "EventContextObj get nullptr");
    std::lock_guard<std::mutex> lock(handleMapMutex_);
    for (auto it = audioPolicyClientProxyAPSCbsMap_.begin(); it != audioPolicyClientProxyAPSCbsMap_.end(); ++it) {
        std::shared_ptr<AudioPolicyClientHolder> separationEnabledChangeCb = it->second;
        CHECK_AND_CONTINUE_LOG(separationEnabledChangeCb != nullptr,
            "separationEnabledChangeCb : nullptr for client : %{public}d", it->first);
        CHECK_AND_CONTINUE(clientCallbacksMap_.count(it->first) > 0 &&
            clientCallbacksMap_[it->first].count(CALLBACK_AUDIO_SEPARATION_EFFECT_ENABLED_CHANGE) > 0 &&
            clientCallbacksMap_[it->first][CALLBACK_AUDIO_SEPARATION_EFFECT_ENABLED_CHANGE]);
        separationEnabledChangeCb->OnAudioSeparationEffectEnabledChange(
            eventContextObj->audioSeparationEffectEnabled);
    }
}
    ...
    const AppExecFwk::InnerEvent::Pointer &event)
{
    switch (eventId) {
        ...
        case EventAudioServerCmd::SPATIALIZATION_ENABLED_CHANGE_FOR_CURRENT_DEVICE:
            ...
            break;
        case EventAudioServerCmd::SELF_APP_VOLUME_CHANGE_EVENT:
            HandleSelfAppVolumeChangeEvent(event);
            break;
        case EventAudioServerCmd::SYSTEM_APP_VOLUME_CHANGE_EVENT:
            HandleSystemAppVolumeChangeEvent(event);
            break;
    ...
    const AppExecFwk::InnerEvent::Pointer &event)
{
    switch (eventId) {
        ...
        case EventAudioServerCmd::VOLUME_DEGREE_EVENT:
            ...
            break;
        case EventAudioServerCmd::PERSONALIZED_SPATIALIZATION_ENABLED_CHANGE_FOR_ANY_DEVICE:
            HandlePersonalizedSpatializationEnabledChangeForAnyDeviceEvent(event);
            break;
        case EventAudioServerCmd::AUDIO_SEPARATION_EFFECT_ENABLED_CHANGE:
            HandleAudioSeparationEffectEnabledChangeEvent(event);
            break;
        default:
            break;
    }
...
void AudioPolicyServerHandler::ProcessEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    ...
    switch (eventId) {
        ...
        case EventAudioServerCmd::ADAPTIVE_SPATIAL_RENDERING_ENABLED_CHANGE_FOR_ANY_DEVICE:
            ...
            break;
        case EventAudioServerCmd::BYPASS_SPATIAL_FOR_STEREO_CHANGE_FOR_CURRENT_DEVICE:
            HandleSpatializationEnabledChangeForBT(event);
            break;
        case EventAudioServerCmd::EFFECT_MODE_CHANGE_FOR_ANY_DEVICE:
            HandleSpatialAudioSourceTypeChange(event);
            break;
        default:
            break;
    }
...
{
    if (IPCSkeleton::GetCallingUid() != uid) {
        CHECK_AND_RETURN_RET_LOG(PermissionUtil::VerifySystemPermission(), ERR_PERMISSION_DENIED,
            "Permission denied for setting callback renderer info with different uid");
    }
...
int32_t AudioPolicyServerHandler::SetCallbackCapturerInfo(const AudioCapturerInfo &capturerInfo, const int32_t uid)
{
    if (IPCSkeleton::GetCallingUid() != uid) {
        CHECK_AND_RETURN_RET_LOG(PermissionUtil::VerifySystemPermission(), ERR_PERMISSION_DENIED,
            "Permission denied for setting callback capturer info with different uid");
    }
    int32_t clientPid = IPCSkeleton::GetCallingPid();
    lock_guard<mutex> lock(clientCbCapturerFilterMapMutex_);
    auto &capturerFilterList = clientCbCapturerFilterMap_[clientPid];
    auto it = std::find_if(capturerFilterList.begin(), capturerFilterList.end(),
        [&capturerInfo, uid](const AudioCapturerFilter &existingFilter) {
            return existingFilter.uid == uid &&
                existingFilter.capturerInfo.sourceType == capturerInfo.sourceType;
        });
    if (it == capturerFilterList.end()) {
        AudioCapturerFilter newFilter;
        newFilter.uid = uid;
        newFilter.capturerInfo = capturerInfo;
        capturerFilterList.push_back(newFilter);
    }
    return AUDIO_OK;
}

std::vector<AudioCapturerFilter> AudioPolicyServerHandler::GetCallbackCapturerInfoList(int32_t clientPid)
{
    lock_guard<mutex> lock(clientCbCapturerFilterMapMutex_);
    auto it = clientCbCapturerFilterMap_.find(clientPid);
    if (it == clientCbCapturerFilterMap_.end()) {
        return {};
    }
    return it->second;
}
...
void AudioPolicyServerHandler::HandlePreferredDeviceSetEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    ...
    for (auto it = audioPolicyClientProxyAPSCbsMap_.begin(); it != audioPolicyClientProxyAPSCbsMap_.end(); ++it) {
        if (clientCallbacksMap_.count(it->first) > 0 &&
            ...
            CHECK_AND_CONTINUE(preferredDeviceSetCb != nullptr && preferredDeviceSetCb->hasSystemPermission_);
...
void AudioPolicyServerHandler::PublishCtrlCmdEvent(int32_t hintType, int32_t uid, int32_t streamId)
{
...
std::shared_ptr<AudioPolicyClientHolder> AudioPolicyServerHandler::GetAudioPolicyClientHolder(int32_t clientId)
{
    lock_guard<mutex> lock(handleMapMutex_);
    auto it = audioPolicyClientProxyAPSCbsMap_.find(clientId);
    if (it != audioPolicyClientProxyAPSCbsMap_.end()) {
        return it->second;
    }
    return nullptr;
