...
/**
 * @tc.name: GetPipeType_014
 * @tc.desc: Test GetPipeType when audioMode is AUDIO_MODE_RECORD and flag is live.
 * @tc.type: FUNC
 * @tc.require: #I5Y4MZ
 */
HWTEST_F(AudioPipeSelectorUnitTest, GetPipeType_014, TestSize.Level1)
{
...
/**
 * @tc.name: GetPipeType_015
 * @tc.desc: Test GetPipeType when audioMode is AUDIO_MODE_RECORD and flag contains AUDIO_INPUT_FLAG_CAMCORDER.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, GetPipeType_015, TestSize.Level1)
{
    uint32_t flag = AUDIO_INPUT_FLAG_CAMCORDER;
    AudioMode audioMode = AUDIO_MODE_RECORD;
    AudioPipeType result = AudioPipeSelector::GetPipeSelector()->GetPipeType(flag, audioMode);
    EXPECT_EQ(result, PIPE_TYPE_IN_CAMCORDER);
}
...
HWTEST_F(AudioPipeSelectorUnitTest, GetAdapterNameByStreamDesc_001, TestSize.Level1)
{
    ...
    std::string pipeName;
    audioPipeSelector->GetAdapterNameByStreamDesc(streamDesc, adapterName, pipeName);
    EXPECT_EQ(adapterName, "");
    EXPECT_EQ(pipeName, "");
}
...
/**
 * @tc.name: JudgeStreamAction_008
 * @tc.desc: Test JudgeStreamAction when newPipe and oldPipe have the same adapterName, routeFlag and name_.
 * @tc.type: FUNC
 * @tc.require: #I5Y4MZ
 */
HWTEST_F(AudioPipeSelectorUnitTest, JudgeStreamAction_008, TestSize.Level1)
{
    std::shared_ptr<AudioPipeInfo> newPipe = std::make_shared<AudioPipeInfo>();
    newPipe->adapterName_ = "test_adapter";
    newPipe->routeFlag_ = 1;
    newPipe->name_ = "pipe_name";

    std::shared_ptr<AudioPipeInfo> oldPipe = std::make_shared<AudioPipeInfo>();
    oldPipe->adapterName_ = "test_adapter";
    oldPipe->routeFlag_ = 1;
    oldPipe->name_ = "pipe_name";
...
/**
 * @tc.name: JudgeStreamAction_009
 * @tc.desc: Test JudgeStreamAction when adapterName and routeFlag are the same but name_ is different.
 * @tc.type: FUNC
 * @tc.require: #I5Y4MZ
 */
HWTEST_F(AudioPipeSelectorUnitTest, JudgeStreamAction_009, TestSize.Level1)
{
    std::shared_ptr<AudioPipeInfo> newPipe = std::make_shared<AudioPipeInfo>();
    newPipe->adapterName_ = "test_adapter";
    newPipe->routeFlag_ = AUDIO_OUTPUT_FLAG_NORMAL;
    newPipe->name_ = "pipe_name_new";

    std::shared_ptr<AudioPipeInfo> oldPipe = std::make_shared<AudioPipeInfo>();
    oldPipe->adapterName_ = "test_adapter";
    oldPipe->routeFlag_ = AUDIO_OUTPUT_FLAG_NORMAL;
    oldPipe->name_ = "pipe_name_old";
...
/**
 * @tc.name: ConvertStreamDescToPipeInfo_009
 * @tc.desc: Test remote input stream should not carry voice recognition micIn/ec module info.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, ConvertStreamDescToPipeInfo_009, TestSize.Level1)
{
    std::shared_ptr<AudioStreamDescriptor> streamDesc = std::make_shared<AudioStreamDescriptor>();
    streamDesc->routeFlag_ = AUDIO_INPUT_FLAG_VOICE_RECOGNITION;
    streamDesc->sessionId_ = 102;
    streamDesc->newDeviceDescs_.push_back(std::make_shared<AudioDeviceDescriptor>());
    streamDesc->newDeviceDescs_.front()->deviceType_ = DEVICE_TYPE_MIC;
    streamDesc->newDeviceDescs_.front()->networkId_ = "RemoteDevice";
    streamDesc->capturerInfo_.sourceType = SourceType::SOURCE_TYPE_VOICE_RECOGNITION;
    ...
    streamDesc->ecStreamInfo_.channels = STEREO;
    streamDesc->ecStreamInfo_.format = SAMPLE_S16LE;

    std::shared_ptr<PipeStreamPropInfo> streamPropInfo = std::make_shared<PipeStreamPropInfo>();
    streamPropInfo->format_ = AudioSampleFormat::SAMPLE_S16LE;
    streamPropInfo->sampleRate_ = 48000;
    streamPropInfo->channelLayout_ = AudioChannelLayout::CH_LAYOUT_MONO;
    streamPropInfo->bufferSize_ = 1024;
    ...
    pipeInfoPtr->paProp_.moduleName_ = "test_module";
    pipeInfoPtr->name_ = "test_name";
    pipeInfoPtr->role_ = PIPE_ROLE_INPUT;
    ...
    EXPECT_EQ(info.moduleInfo_.ecType, "");
    EXPECT_EQ(info.moduleInfo_.ecSamplingRate, "");
    EXPECT_EQ(info.moduleInfo_.ecChannels, "");
    EXPECT_EQ(info.moduleInfo_.ecFormat, "");
    EXPECT_EQ(info.moduleInfo_.micInRate, "");
    EXPECT_EQ(info.moduleInfo_.micInChannels, "");
    EXPECT_EQ(info.moduleInfo_.micInFormat, "");
    EXPECT_EQ(info.moduleInfo_.micRefRate, "");
    EXPECT_EQ(info.moduleInfo_.micRefChannels, "");
    EXPECT_EQ(info.moduleInfo_.micRefFormat, "");
}

/**
 * @tc.name: ConvertStreamDescToPipeInfo_010
 * @tc.desc: Test old remote input device should also disable voice recognition micIn/ec module info.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, ConvertStreamDescToPipeInfo_010, TestSize.Level1)
{
    std::shared_ptr<AudioStreamDescriptor> streamDesc = std::make_shared<AudioStreamDescriptor>();
    streamDesc->routeFlag_ = AUDIO_INPUT_FLAG_VOICE_RECOGNITION;
    streamDesc->sessionId_ = 103;
    streamDesc->newDeviceDescs_.push_back(std::make_shared<AudioDeviceDescriptor>());
    streamDesc->newDeviceDescs_.front()->deviceType_ = DEVICE_TYPE_MIC;
    streamDesc->newDeviceDescs_.front()->networkId_ = LOCAL_NETWORK_ID;
    streamDesc->oldDeviceDescs_.push_back(std::make_shared<AudioDeviceDescriptor>());
    streamDesc->oldDeviceDescs_.front()->deviceType_ = DEVICE_TYPE_MIC;
    streamDesc->oldDeviceDescs_.front()->networkId_ = "RemoteDevice";
    streamDesc->capturerInfo_.sourceType = SourceType::SOURCE_TYPE_VOICE_RECOGNITION;
    streamDesc->micInStreamInfo_.samplingRate = SAMPLE_RATE_48000;
    streamDesc->micInStreamInfo_.channels = CHANNEL_4;
    streamDesc->micInStreamInfo_.format = SAMPLE_S16LE;
...
/**
 * @tc.name: ConvertStreamDescToPipeInfo_011
 * @tc.desc: Test local non-primary input device should also disable voice recognition micIn/ec module info.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, ConvertStreamDescToPipeInfo_011, TestSize.Level1)
{
    std::shared_ptr<AudioStreamDescriptor> streamDesc = std::make_shared<AudioStreamDescriptor>();
    streamDesc->routeFlag_ = AUDIO_INPUT_FLAG_VOICE_RECOGNITION;
    streamDesc->sessionId_ = 104;
    streamDesc->newDeviceDescs_.push_back(std::make_shared<AudioDeviceDescriptor>());
    streamDesc->newDeviceDescs_.front()->deviceType_ = DEVICE_TYPE_ACCESSORY;
    streamDesc->newDeviceDescs_.front()->networkId_ = LOCAL_NETWORK_ID;
    streamDesc->capturerInfo_.sourceType = SourceType::SOURCE_TYPE_VOICE_RECOGNITION;
    streamDesc->micInStreamInfo_.samplingRate = SAMPLE_RATE_48000;
    streamDesc->micInStreamInfo_.channels = CHANNEL_4;
...
/**
 * @tc.name: ConvertStreamDescToPipeInfo_012
 * @tc.desc: Test camcorder mic-in stream should carry micIn and micRef module info.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, ConvertStreamDescToPipeInfo_012, TestSize.Level1)
{
    std::shared_ptr<AudioStreamDescriptor> streamDesc = std::make_shared<AudioStreamDescriptor>();
    streamDesc->routeFlag_ = AUDIO_INPUT_FLAG_CAMCORDER;
    streamDesc->sessionId_ = 105;
    streamDesc->newDeviceDescs_.push_back(std::make_shared<AudioDeviceDescriptor>());
    streamDesc->newDeviceDescs_.front()->deviceType_ = DEVICE_TYPE_MIC;
    streamDesc->newDeviceDescs_.front()->networkId_ = LOCAL_NETWORK_ID;
    streamDesc->capturerInfo_.sourceType = SourceType::SOURCE_TYPE_CAMCORDER;
    streamDesc->micInStreamInfo_.samplingRate = SAMPLE_RATE_48000;
    streamDesc->micInStreamInfo_.channels = CHANNEL_4;
    ...
    EXPECT_EQ(info.moduleInfo_.micInRate, "48000");
    EXPECT_EQ(info.moduleInfo_.micInChannels, "4");
    EXPECT_EQ(info.moduleInfo_.micInFormat, "s24");
    EXPECT_EQ(info.moduleInfo_.micRefRate, "48000");
    EXPECT_EQ(info.moduleInfo_.micRefChannels, "4");
    EXPECT_EQ(info.moduleInfo_.micRefFormat, "s24");
    EXPECT_EQ(info.moduleInfo_.ecType, "");
    EXPECT_EQ(info.moduleInfo_.ecSamplingRate, "");
}
...
/**
 * @tc.name: HandleFindBusPipe_003
 * @tc.desc: Test that when streamDesc is nullptr and busAddresses size is greater than 1,
 *           the function returns safely without dereferencing nullptr.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, HandleFindBusPipe_003, TestSize.Level1)
{
    AudioModuleInfo moduleInfo;
    moduleInfo.name = "Speaker";
    ...
    auto busPipeIter = newPipeInfoList.end();
    AudioPipeSelector::GetPipeSelector()->HandleFindBusPipe({"Speaker", "MCH_Speaker"}, newPipeInfoList, nullptr,
                                                            busPipeIter);
    EXPECT_EQ(busPipeIter, newPipeInfoList.end());
}

/**
 * @tc.name: HandleFindBusPipe_004
 * @tc.desc: Test that for recording stream, bus pipe matching uses moduleInfo_.busAddress.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, HandleFindBusPipe_004, TestSize.Level1)
{
    AudioModuleInfo moduleInfo;
    moduleInfo.name = "MicModule";
    moduleInfo.busAddress = "record_bus1";
    ...
    streamDesc->newDeviceDescs_.front()->networkId_ = LOCAL_NETWORK_ID;
    streamDesc->newDeviceDescs_.front()->macAddress_ = "record_bus1";

    auto busPipeIter = newPipeInfoList.end();
    AudioPipeSelector::GetPipeSelector()->HandleFindBusPipe({"record_bus1"}, newPipeInfoList, streamDesc, busPipeIter);
    EXPECT_NE(busPipeIter, newPipeInfoList.end());
    EXPECT_EQ((*busPipeIter)->moduleInfo_.busAddress, "record_bus1");
}
#endif
...
HWTEST_F(AudioPipeSelectorUnitTest, FindExistingPipe_005, TestSize.Level1)
{
    ...
    fastInputPipeInfo->name_ = "test_in";
...
/**
 * @tc.name: SortStreamByStatus_001
 * @tc.desc: Test SortStreamByStatus when all streams are STARTED - should sort by sessionId ascending.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, SortStreamByStatus_001, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);

    std::vector<std::pair<std::shared_ptr<AudioStreamDescriptor>, std::shared_ptr<AudioPipeInfo>>> streamList;

    auto pipe1 = std::make_shared<AudioPipeInfo>();
    auto stream1 = std::make_shared<AudioStreamDescriptor>();
    stream1->sessionId_ = 100003;
    stream1->streamStatus_ = STREAM_STATUS_STARTED;
    streamList.emplace_back(stream1, pipe1);

    auto pipe2 = std::make_shared<AudioPipeInfo>();
    auto stream2 = std::make_shared<AudioStreamDescriptor>();
    stream2->sessionId_ = 100001;
    stream2->streamStatus_ = STREAM_STATUS_STARTED;
    streamList.emplace_back(stream2, pipe2);

    auto pipe3 = std::make_shared<AudioPipeInfo>();
    auto stream3 = std::make_shared<AudioStreamDescriptor>();
    stream3->sessionId_ = 100002;
    stream3->streamStatus_ = STREAM_STATUS_STARTED;
    streamList.emplace_back(stream3, pipe3);

    audioPipeSelector->SortStreamByStatus(streamList);

    EXPECT_EQ(streamList[0].first->sessionId_, 100001);
    EXPECT_EQ(streamList[1].first->sessionId_, 100002);
    EXPECT_EQ(streamList[2].first->sessionId_, 100003);
}

/**
 * @tc.name: SortStreamByStatus_002
 * @tc.desc: Test SortStreamByStatus when all streams are not STARTED - should sort by sessionId ascending.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, SortStreamByStatus_002, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    auto stream1 = std::make_shared<AudioStreamDescriptor>();
    stream1->sessionId_ = 100003;
    stream1->streamStatus_ = STREAM_STATUS_STOPPED;
    streamList.emplace_back(stream1, pipe1);

    auto pipe2 = std::make_shared<AudioPipeInfo>();
    auto stream2 = std::make_shared<AudioStreamDescriptor>();
    stream2->sessionId_ = 100001;
    stream2->streamStatus_ = STREAM_STATUS_PAUSED;
    streamList.emplace_back(stream2, pipe2);

    auto pipe3 = std::make_shared<AudioPipeInfo>();
    auto stream3 = std::make_shared<AudioStreamDescriptor>();
    stream3->sessionId_ = 100002;
    stream3->streamStatus_ = STREAM_STATUS_RELEASED;
    streamList.emplace_back(stream3, pipe3);
...
/**
 * @tc.name: SortStreamByStatus_003
 * @tc.desc: Test SortStreamByStatus with mixed statuses - STARTED streams first, then others, all by sessionId.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, SortStreamByStatus_003, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    auto stream2 = std::make_shared<AudioStreamDescriptor>();
    stream2->sessionId_ = 100001;
    stream2->streamStatus_ = STREAM_STATUS_STOPPED;
    streamList.emplace_back(stream2, pipe2);

    auto pipe3 = std::make_shared<AudioPipeInfo>();
    auto stream3 = std::make_shared<AudioStreamDescriptor>();
    stream3->sessionId_ = 100005;
    stream3->streamStatus_ = STREAM_STATUS_STARTED;
    streamList.emplace_back(stream3, pipe3);

    auto pipe4 = std::make_shared<AudioPipeInfo>();
    auto stream4 = std::make_shared<AudioStreamDescriptor>();
    stream4->sessionId_ = 100002;
    stream4->streamStatus_ = STREAM_STATUS_PAUSED;
    streamList.emplace_back(stream4, pipe4);

    auto pipe5 = std::make_shared<AudioPipeInfo>();
    auto stream5 = std::make_shared<AudioStreamDescriptor>();
    stream5->sessionId_ = 100004;
    stream5->streamStatus_ = STREAM_STATUS_RELEASED;
    streamList.emplace_back(stream5, pipe5);

    auto pipe6 = std::make_shared<AudioPipeInfo>();
    auto stream6 = std::make_shared<AudioStreamDescriptor>();
    stream6->sessionId_ = 100006;
    stream6->streamStatus_ = STREAM_STATUS_STARTED;
    streamList.emplace_back(stream6, pipe6);

    audioPipeSelector->SortStreamByStatus(streamList);

    EXPECT_EQ(streamList[0].first->sessionId_, 100003);
    EXPECT_EQ(streamList[1].first->sessionId_, 100005);
    EXPECT_EQ(streamList[2].first->sessionId_, 100006);
    EXPECT_EQ(streamList[3].first->sessionId_, 100001);
    EXPECT_EQ(streamList[4].first->sessionId_, 100002);
    EXPECT_EQ(streamList[5].first->sessionId_, 100004);
}

/**
 * @tc.name: SortStreamByStatus_004
 * @tc.desc: Test SortStreamByStatus with empty list - should not crash and return empty.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, SortStreamByStatus_004, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
...
/**
 * @tc.name: SortStreamByStatus_005
 * @tc.desc: Test SortStreamByStatus with single STARTED stream.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, SortStreamByStatus_005, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    auto stream = std::make_shared<AudioStreamDescriptor>();
    stream->sessionId_ = 100001;
    stream->streamStatus_ = STREAM_STATUS_STARTED;
    streamList.emplace_back(stream, pipe);

    audioPipeSelector->SortStreamByStatus(streamList);

    EXPECT_EQ(streamList.size(), 1);
    EXPECT_EQ(streamList[0].first->sessionId_, 100001);
    EXPECT_EQ(streamList[0].first->streamStatus_, STREAM_STATUS_STARTED);
}

/**
 * @tc.name: SortStreamByStatus_006
 * @tc.desc: Test SortStreamByStatus with single non-STARTED stream.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, SortStreamByStatus_006, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    auto stream = std::make_shared<AudioStreamDescriptor>();
    stream->sessionId_ = 100001;
    stream->streamStatus_ = STREAM_STATUS_PAUSED;
    streamList.emplace_back(stream, pipe);
    ...
    EXPECT_EQ(streamList.size(), 1);
    EXPECT_EQ(streamList[0].first->sessionId_, 100001);
    EXPECT_EQ(streamList[0].first->streamStatus_, STREAM_STATUS_PAUSED);
}

/**
 * @tc.name: AdjustConcedeActionByStatus_001
 * @tc.desc: Test when target stream is not STARTED and not in switch state, action should be flipped
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, AdjustConcedeActionByStatus_001, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);

    auto existingStream = std::make_shared<AudioStreamDescriptor>();
    existingStream->sessionId_ = 100001;
    existingStream->streamStatus_ = STREAM_STATUS_PAUSED;
    
    auto incomingStream = std::make_shared<AudioStreamDescriptor>();
    incomingStream->sessionId_ = 100002;
    incomingStream->streamStatus_ = STREAM_STATUS_STARTED;

    // Mock IsSwitchStreamExist to return false
    AudioPipeManager::GetPipeManager()->switchStreamMap_.clear();

    ConcurrencyAction action = CONCEDE_INCOMING;
    audioPipeSelector->AdjustConcedeActionByStatus(existingStream, incomingStream, action);
    
    // Expect action to flip from CONCEDE_INCOMING to CONCEDE_EXISTING
    EXPECT_EQ(action, CONCEDE_EXISTING);
}

/**
 * @tc.name: AdjustConcedeActionByStatus_002
 * @tc.desc: Test when target stream is not STARTED and not in switch state, action should be flipped
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, AdjustConcedeActionByStatus_002, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    auto existingStream = std::make_shared<AudioStreamDescriptor>();
    existingStream->sessionId_ = 100001;
    existingStream->streamStatus_ = STREAM_STATUS_STOPPED;
...
/**
 * @tc.name: AdjustConcedeActionByStatus_003
 * @tc.desc: Test when target stream is STARTED and not in switch state, action should remain unchanged
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, AdjustConcedeActionByStatus_003, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    auto existingStream = std::make_shared<AudioStreamDescriptor>();
    existingStream->sessionId_ = 100001;
    existingStream->streamStatus_ = STREAM_STATUS_STARTED;
    
    auto incomingStream = std::make_shared<AudioStreamDescriptor>();
    incomingStream->sessionId_ = 100002;
    incomingStream->streamStatus_ = STREAM_STATUS_PAUSED;
    ...
    // Expect action to remain CONCEDE_INCOMING
    EXPECT_EQ(action, CONCEDE_INCOMING);
}

/**
 * @tc.name: AdjustConcedeActionByStatus_004
 * @tc.desc: Test when target stream is in switch state and requires normal route, action should be flipped
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, AdjustConcedeActionByStatus_004, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    existingStream->sessionId_ = 100001;
    existingStream->streamStatus_ = STREAM_STATUS_PAUSED;
    existingStream->SetFutureRoute(AUDIO_OUTPUT_FLAG_NORMAL); // Mock IsFutureRouteNormal to return true
    
    auto incomingStream = std::make_shared<AudioStreamDescriptor>();
    incomingStream->sessionId_ = 100002;
    incomingStream->streamStatus_ = STREAM_STATUS_STARTED;

    // Mock IsSwitchStreamExist to return true
    AudioPipeManager::GetPipeManager()->AddSingleSwitchStream(100001, existingStream);
...
/**
 * @tc.name: AdjustConcedeActionByStatus_005
 * @tc.desc: Test when target stream is in switch state but doesn't require normal route, action should remain unchanged
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, AdjustConcedeActionByStatus_005, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    existingStream->sessionId_ = 100001;
    existingStream->streamStatus_ = STREAM_STATUS_PAUSED;
    existingStream->SetFutureRoute(AUDIO_OUTPUT_FLAG_FAST); // Mock IsFutureRouteNormal to return false
    ...
    ConcurrencyAction action = CONCEDE_EXISTING;
    audioPipeSelector->AdjustConcedeActionByStatus(existingStream, incomingStream, action);
    
    // Expect action to remain CONCEDE_EXISTING
    EXPECT_EQ(action, CONCEDE_EXISTING);
}

/**
 * @tc.name: AdjustConcedeActionByStatus_006
 * @tc.desc: Test with CONCEDE_INCOMING action and target stream in switch state requiring normal route
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, AdjustConcedeActionByStatus_006, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    incomingStream->sessionId_ = 100002;
    incomingStream->streamStatus_ = STREAM_STATUS_PAUSED;
    incomingStream->SetFutureRoute(AUDIO_OUTPUT_FLAG_NORMAL); // Mock IsFutureRouteNormal to return true

    // Mock IsSwitchStreamExist to return true
    AudioPipeManager::GetPipeManager()->AddSingleSwitchStream(100002, incomingStream);

    ConcurrencyAction action = CONCEDE_EXISTING;
    audioPipeSelector->AdjustConcedeActionByStatus(existingStream, incomingStream, action);
    
    // Expect action to flip from CONCEDE_EXISTING to CONCEDE_INCOMING
    EXPECT_EQ(action, CONCEDE_INCOMING);
}

/**
 * @tc.name: AdjustConcedeActionByStatus_007
 * @tc.desc: Test when both streams are in STARTED state and not in switch state, action should remain unchanged
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, AdjustConcedeActionByStatus_007, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
...
/**
 * @tc.name: AdjustConcedeActionByStatus_008
 * @tc.desc: Test with invalid initial action (should not happen in practice but testing robustness)
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, AdjustConcedeActionByStatus_008, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    ConcurrencyAction action = PLAY_BOTH; // Invalid initial action for this function
    audioPipeSelector->AdjustConcedeActionByStatus(existingStream, incomingStream, action);
    
    // Expect action to remain PLAY_BOTH (though this should not be passed to the function in practice)
    EXPECT_EQ(action, PLAY_BOTH);
}

/**
 * @tc.name: AdjustConcedeActionByStatus_009
 * @tc.desc: Test when existing stream is in switch state requiring normal route but incoming stream is not STARTED
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, AdjustConcedeActionByStatus_009, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    incomingStream->sessionId_ = 100002;
    incomingStream->streamStatus_ = STREAM_STATUS_PAUSED;
    incomingStream->SetFutureRoute(AUDIO_OUTPUT_FLAG_NORMAL);

    // Mock IsSwitchStreamExist to return true for incoming stream
    AudioPipeManager::GetPipeManager()->AddSingleSwitchStream(100002, incomingStream);
...
/**
 * @tc.name: AdjustConcedeActionByStatus_010
 * @tc.desc: Test when existing stream is in switch state requiring normal route and is the target
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, AdjustConcedeActionByStatus_010, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    existingStream->sessionId_ = 100001;
    existingStream->streamStatus_ = STREAM_STATUS_PAUSED;
    existingStream->SetFutureRoute(AUDIO_OUTPUT_FLAG_NORMAL);
    
    auto incomingStream = std::make_shared<AudioStreamDescriptor>();
    incomingStream->sessionId_ = 100002;
    incomingStream->streamStatus_ = STREAM_STATUS_STARTED;

    // Mock IsSwitchStreamExist to return true for existing stream
    AudioPipeManager::GetPipeManager()->AddSingleSwitchStream(100001, existingStream);
...
/**
 * @tc.name: GetIncomingStreamPriority_001
 * @tc.desc: Test GetIncomingStreamPriority when existingStream played earlier than incomingStream.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, GetIncomingStreamPriority_001, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();

    std::shared_ptr<AudioStreamDescriptor> existingStream = std::make_shared<AudioStreamDescriptor>();
    existingStream->newDeviceDescs_.push_back(std::make_shared<AudioDeviceDescriptor>());
    existingStream->newDeviceDescs_.front()->networkId_ = "test_networkId";
    existingStream->streamInfo_.channels = AudioChannel::CHANNEL_6;
    existingStream->streamInfo_.samplingRate = AudioSamplingRate::SAMPLE_RATE_48000;
    existingStream->routeFlag_ = AUDIO_OUTPUT_FLAG_LOWPOWER;
    existingStream->streamStatus_ = STREAM_STATUS_STARTED;
    existingStream->startTimeStamp_ = 1;

    std::shared_ptr<AudioStreamDescriptor> incomingStream = std::make_shared<AudioStreamDescriptor>();
    incomingStream->newDeviceDescs_.push_back(std::make_shared<AudioDeviceDescriptor>());
    incomingStream->newDeviceDescs_.front()->networkId_ = "test_networkId";
    incomingStream->streamInfo_.channels = AudioChannel::CHANNEL_6;
    incomingStream->streamInfo_.samplingRate = AudioSamplingRate::SAMPLE_RATE_192000;
    incomingStream->routeFlag_ = AUDIO_OUTPUT_FLAG_LOWPOWER;
    incomingStream->streamStatus_ = STREAM_STATUS_STARTED;
    incomingStream->startTimeStamp_ = 2;

    bool isConcedeExisting = false;
    isConcedeExisting = audioPipeSelector->GetIncomingStreamPriority(existingStream, incomingStream);
    EXPECT_EQ(isConcedeExisting, false);

    existingStream->streamInfo_.channels = AudioChannel::STEREO;
    isConcedeExisting = audioPipeSelector->GetIncomingStreamPriority(existingStream, incomingStream);
    EXPECT_EQ(isConcedeExisting, true);

    incomingStream->streamInfo_.channels = AudioChannel::STEREO;
    isConcedeExisting = audioPipeSelector->GetIncomingStreamPriority(existingStream, incomingStream);
    EXPECT_EQ(isConcedeExisting, true);

    incomingStream->streamInfo_.samplingRate = AudioSamplingRate::SAMPLE_RATE_48000;
    isConcedeExisting = audioPipeSelector->GetIncomingStreamPriority(existingStream, incomingStream);
    EXPECT_EQ(isConcedeExisting, false);
}

/**
 * @tc.name: GetIncomingStreamPriority_002
 * @tc.desc: Test GetIncomingStreamPriority when incomingStream played earlier than existingStream.
 * @tc.type: FUNC
 */
HWTEST_F(AudioPipeSelectorUnitTest, GetIncomingStreamPriority_002, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ...
    existingStream->routeFlag_ = AUDIO_OUTPUT_FLAG_LOWPOWER;
    existingStream->streamStatus_ = STREAM_STATUS_STARTED;
    existingStream->startTimeStamp_ = 2;
    ...
    incomingStream->routeFlag_ = AUDIO_OUTPUT_FLAG_LOWPOWER;
    incomingStream->streamStatus_ = STREAM_STATUS_STARTED;
    incomingStream->startTimeStamp_ = 1;
    ...
    incomingStream->streamInfo_.samplingRate = AudioSamplingRate::SAMPLE_RATE_44100;
    isConcedeExisting = audioPipeSelector->GetIncomingStreamPriority(existingStream, incomingStream);
    EXPECT_EQ(isConcedeExisting, false);
}

/**
 * @tc.name: IsPipeMatch_001
 * @tc.desc: Test IsPipeMatch with pipeName matching.
 * @tc.type: FUNC
 * @tc.require: #I5Y4MZ
 */
HWTEST_F(AudioPipeSelectorUnitTest, IsPipeMatch_001, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ASSERT_NE(audioPipeSelector, nullptr);
    ...
    std::string pipeName = "primary_output";

    bool result = audioPipeSelector->IsPipeMatch(streamDesc, pipeInfo, adapterName, pipeName);
    EXPECT_TRUE(result);
}

/**
 * @tc.name  : UpdateActionForHdPlay_001
 * @tc.number: UpdateActionForHdPlay_001
 * @tc.desc  : Test UpdateActionForHdPlay when existingStream is Offload and action is CONCEDE_INCOMING.
 */
HWTEST_F(AudioPipeSelectorUnitTest, UpdateActionForHdPlay_001, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();

    auto existingStream = std::make_shared<AudioStreamDescriptor>();
    existingStream->newDeviceDescs_.push_back(std::make_shared<AudioDeviceDescriptor>());
    existingStream->newDeviceDescs_[0]->hdPlaybackMode_ = HdPlaybackMode::APP_LEVEL;
    existingStream->routeFlag_ = AUDIO_OUTPUT_FLAG_LOWPOWER;
    existingStream->streamStatus_ = STREAM_STATUS_STARTED;

    auto incomingStream = std::make_shared<AudioStreamDescriptor>();
    incomingStream->newDeviceDescs_.push_back(std::make_shared<AudioDeviceDescriptor>());
    incomingStream->newDeviceDescs_[0]->hdPlaybackMode_ = HdPlaybackMode::APP_LEVEL;
    incomingStream->routeFlag_ = AUDIO_OUTPUT_FLAG_NORMAL;
    incomingStream->streamStatus_ = STREAM_STATUS_STARTED;

    ConcurrencyAction action = CONCEDE_INCOMING;
    audioPipeSelector->UpdateActionForHdPlay(existingStream, incomingStream, action);
    EXPECT_EQ(action, CONCEDE_BOTH);
}

/**
 * @tc.name  : UpdateActionForHdPlay_002
 * @tc.number: UpdateActionForHdPlay_002
 * @tc.desc  : Test UpdateActionForHdPlay when existingStream is Offload and action is PLAY_BOTH.
 */
HWTEST_F(AudioPipeSelectorUnitTest, UpdateActionForHdPlay_002, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ...
    ConcurrencyAction action = PLAY_BOTH;
    audioPipeSelector->UpdateActionForHdPlay(existingStream, incomingStream, action);
    EXPECT_EQ(action, CONCEDE_EXISTING);
}

/**
 * @tc.name  : UpdateActionForHdPlay_003
 * @tc.number: UpdateActionForHdPlay_003
 * @tc.desc  : Test UpdateActionForHdPlay when existingStream is Offload and action is CONCEDE_EXISTING.
 */
HWTEST_F(AudioPipeSelectorUnitTest, UpdateActionForHdPlay_003, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
...
/**
 * @tc.name  : UpdateActionForHdPlay_004
 * @tc.number: UpdateActionForHdPlay_004
 * @tc.desc  : Test UpdateActionForHdPlay when incomingStream is Offload and action is PLAY_BOTH.
 */
HWTEST_F(AudioPipeSelectorUnitTest, UpdateActionForHdPlay_004, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
    ...
    existingStream->newDeviceDescs_.push_back(std::make_shared<AudioDeviceDescriptor>());
    existingStream->newDeviceDescs_[0]->hdPlaybackMode_ = HdPlaybackMode::APP_LEVEL;
    existingStream->routeFlag_ = AUDIO_OUTPUT_FLAG_NORMAL;
    existingStream->streamStatus_ = STREAM_STATUS_STARTED;
...
/**
 * @tc.name  : UpdateActionForHdPlay_005
 * @tc.number: UpdateActionForHdPlay_005
 * @tc.desc  : Test UpdateActionForHdPlay when incomingStream is Offload and action is CONCEDE_INCOMING.
 */
HWTEST_F(AudioPipeSelectorUnitTest, UpdateActionForHdPlay_005, TestSize.Level1)
{
    auto audioPipeSelector = AudioPipeSelector::GetPipeSelector();
