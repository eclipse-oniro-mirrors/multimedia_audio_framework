...
*/
HWTEST(AudioPolicyManager, IsCollaborativePlaybackSupportedForDevice_001, TestSize.Level1)
{
    auto audioPolicyManager_ = std::make_shared<AudioPolicyManager>();
    ASSERT_TRUE(audioPolicyManager_ != nullptr);

    bool isSupported = audioPolicyManager_->IsCollaborativePlaybackSupportedForDevice(nullptr);
...
* @tc.desc  : Test IsCollaborativePlaybackSupportedForDevice.
*/
HWTEST(AudioPolicyManager, IsCollaborativePlaybackSupportedForDevice_002, TestSize.Level1)
{
    auto audioPolicyManager_ = std::make_shared<AudioPolicyManager>();
    ASSERT_TRUE(audioPolicyManager_ != nullptr);
    std::shared_ptr<AudioDeviceDescriptor> device = std::make_shared<AudioDeviceDescriptor>();
    ASSERT_TRUE(device != nullptr);
    device->macAddress_ = "testAddr";
    bool isSupported = audioPolicyManager_->IsCollaborativePlaybackSupportedForDevice(device);
    EXPECT_FALSE(isSupported);
}
 ...
 * @tc.desc  : Call SetAudioZoneVolumeConfig with empty/typical params, expect non-ERROR return.
 */
HWTEST(AudioPolicyManager, SetAudioZoneVolumeConfig_001, TestSize.Level1)
{
    auto audioPolicyManager_ = std::make_shared<AudioPolicyManager>();
    ASSERT_NE(audioPolicyManager_, nullptr);

    std::string zoneName = std::string(PRIMARY_ZONE_NAME);
    std::vector<AudioZoneVolumeConfig> configs; // empty list

    int32_t ret = audioPolicyManager_->SetAudioZoneVolumeConfig(zoneName, configs);
    EXPECT_NE(ret, ERROR);
}
 ...
 * @tc.desc  : Call TriggerAudioZoneVolumeSync and expect non-ERROR return.
 */
HWTEST(AudioPolicyManager, TriggerAudioZoneVolumeSync_001, TestSize.Level1)
{
    auto audioPolicyManager_ = std::make_shared<AudioPolicyManager>();
    ASSERT_NE(audioPolicyManager_, nullptr);

    int32_t ret = audioPolicyManager_->TriggerAudioZoneVolumeSync();
    EXPECT_NE(ret, ERROR);
}
 ...
 * @tc.desc  : Call EnableSystemVolumeProxy with invalid zoneId (0) and expect ERROR return.
 */
HWTEST(AudioPolicyManager, EnableSystemVolumeProxy_001, TestSize.Level1)
{
    auto audioPolicyManager_ = std::make_shared<AudioPolicyManager>();
    ASSERT_NE(audioPolicyManager_, nullptr);
    ...
    int32_t ret = audioPolicyManager_->EnableSystemVolumeProxy(zoneId, deviceType, enable);
    EXPECT_NE(ret, ERROR);
}

/**
 * @tc.name  : Test AudioPolicyManager GetLoopbackSupportedDevicePairs.
 * @tc.number: GetLoopbackSupportedDevicePairs_001
 * @tc.desc  : Test GetLoopbackSupportedDevicePairs with normal case.
 */
HWTEST(AudioPolicyManager, GetLoopbackSupportedDevicePairs_001, TestSize.Level1)
{
    auto audioPolicyManager_ = std::make_shared<AudioPolicyManager>();
    ASSERT_NE(audioPolicyManager_, nullptr);

    auto devicePairs = audioPolicyManager_->GetLoopbackSupportedDevicePairs();
    EXPECT_TRUE(devicePairs.empty() || !devicePairs.empty());
}

/**
 * @tc.name  : Test AudioPolicyManager GetLoopbackPreferredDevicePair.
 * @tc.number: GetLoopbackPreferredDevicePair_002
 * @tc.desc  : Test GetLoopbackPreferredDevicePair with proxy null case.
 */
HWTEST(AudioPolicyManager, GetLoopbackPreferredDevicePair_002, TestSize.Level1)
{
    auto audioPolicyManager_ = std::make_shared<AudioPolicyManager>();
    ASSERT_NE(audioPolicyManager_, nullptr);

    auto devicePair = audioPolicyManager_->GetLoopbackPreferredDevicePair();
    EXPECT_TRUE(devicePair.first == nullptr || devicePair.second == nullptr);
}
} // namespace AudioStandard
} // namespace OHOS
