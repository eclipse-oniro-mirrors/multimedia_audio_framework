...
HWTEST_F(AudioPolicyServiceExtUnitTest, IsWiredHeadSet_001, TestSize.Level1)
{
    ...
    bool ret = server->audioPolicyService_.audioVolumeManager_.safeVolumeManager_.IsWiredHeadSet(deviceType);
    EXPECT_EQ(ret, false);

    deviceType = DeviceType::DEVICE_TYPE_WIRED_HEADSET;
    ret = server->audioPolicyService_.audioVolumeManager_.safeVolumeManager_.IsWiredHeadSet(deviceType);
    EXPECT_EQ(ret, true);
}
...
HWTEST_F(AudioPolicyServiceExtUnitTest, IsBlueTooth_001, TestSize.Level1)
{
    ...
    bool ret = server->audioPolicyService_.audioVolumeManager_.safeVolumeManager_.IsBlueTooth(deviceType);
    EXPECT_EQ(ret, false);

    deviceType = DeviceType::DEVICE_TYPE_BLUETOOTH_A2DP;
    ret = server->audioPolicyService_.audioVolumeManager_.safeVolumeManager_.IsBlueTooth(deviceType);
    EXPECT_EQ(ret, true);
}
...
HWTEST_F(AudioPolicyServiceExtUnitTest, SetA2dpDeviceVolume_001, TestSize.Level1)
{
    ...
    int32_t ret = server->audioVolumeManager_.SetA2dpDeviceVolume(macAddress, volume, internalCall);
    EXPECT_EQ(ret, ERROR);
}
...
HWTEST_F(AudioPolicyServiceExtUnitTest, GetVoipRendererFlag_001, TestSize.Level1)
{
    ...
    AudioCoreConfigManager::GetInstance().normalVoipFlag_ = false;
    AudioCoreConfigManager::GetInstance().OnVoipConfigParsed(false);
    ret = AudioCoreConfigManager::GetInstance().GetVoipRendererFlag(sinkPortName, networkId, samplingRate);
    EXPECT_EQ(ret, AUDIO_FLAG_NORMAL);

    networkId = LOCAL_NETWORK_ID;
    AudioCoreConfigManager::GetInstance().normalVoipFlag_ = true;
    AudioCoreConfigManager::GetInstance().OnVoipConfigParsed(true);
    ret = AudioCoreConfigManager::GetInstance().GetVoipRendererFlag(sinkPortName, networkId, samplingRate);
    EXPECT_EQ(ret, AUDIO_FLAG_NORMAL);
...
HWTEST_F(AudioPolicyServiceExtUnitTest, GetTargetSourceTypeAndMatchingFlag_001, TestSize.Level1)
{
    ...
    AudioCoreService::GetCoreService()->GetTargetSourceTypeAndMatchingFlag(source, targetSource,
        useMatchingPropInfo);
...
HWTEST_F(AudioPolicyServiceExtUnitTest, GetEcType_001, TestSize.Level1)
{
    ...
    EcType ecType = AudioCoreService::GetCoreService()->GetEcType(inputDevice, outputDevice);
    ...
    ecType = AudioCoreService::GetCoreService()->GetEcType(inputDevice, outputDevice);
...
HWTEST_F(AudioPolicyServiceExtUnitTest, GetHalNameForDevice_001, TestSize.Level1)
{
    ...
    DeviceType deviceType = DeviceType::DEVICE_TYPE_SPEAKER;
    std::string halNameForDevice = AudioCoreService::GetCoreService()->GetHalNameForDevice(role, deviceType);
    EXPECT_EQ(halNameForDevice, "primary");

    role = ROLE_SOURCE;
    halNameForDevice = AudioCoreService::GetCoreService()->GetHalNameForDevice(role, deviceType);
    EXPECT_EQ(halNameForDevice, "primary");
}
...
HWTEST_F(AudioPolicyServiceExtUnitTest, GetPipeNameByDeviceForEc_001, TestSize.Level1)
{
    ...
    pipeNameByDeviceForEc = AudioCoreService::GetCoreService()->GetPipeNameByDeviceForEc(role, deviceType);
...
HWTEST_F(AudioPolicyServiceExtUnitTest, GetPipeInfoByDeviceTypeForEc_001, TestSize.Level1)
{
    ...
    int32_t ret = AudioCoreService::GetCoreService()->GetPipeInfoByDeviceTypeForEc(role, deviceType, pipeInfo);
    EXPECT_EQ(ret, SUCCESS);

    role = ROLE_SINK;
    ret = AudioCoreService::GetCoreService()->GetPipeInfoByDeviceTypeForEc(role, deviceType, pipeInfo);
    EXPECT_EQ(ret, SUCCESS);
}
...
HWTEST_F(AudioPolicyServiceExtUnitTest, GetEcSamplingRate_001, TestSize.Level1)
{
    ...
    halName = DP_CLASS;
    AudioCoreService::GetCoreService()->dpSinkModuleInfo_.rate = "888";
    ecSamplingRate = AudioCoreService::GetCoreService()->GetEcSamplingRate(halName, outModuleInfo);
    EXPECT_EQ(ecSamplingRate, "888");

    AudioCoreService::GetCoreService()->dpSinkModuleInfo_.rate = "";
    ecSamplingRate = AudioCoreService::GetCoreService()->GetEcSamplingRate(halName, outModuleInfo);
    EXPECT_EQ(ecSamplingRate, "0");

    halName = USB_CLASS;
    AudioCoreService::GetCoreService()->usbSinkModuleInfo_.rate = "666";
    ecSamplingRate = AudioCoreService::GetCoreService()->GetEcSamplingRate(halName, outModuleInfo);
    EXPECT_EQ(ecSamplingRate, "666");

    AudioCoreService::GetCoreService()->usbSinkModuleInfo_.rate = "";
    ecSamplingRate = AudioCoreService::GetCoreService()->GetEcSamplingRate(halName, outModuleInfo);
    EXPECT_EQ(ecSamplingRate, "0");
...
HWTEST_F(AudioPolicyServiceExtUnitTest, GetEcFormat_001, TestSize.Level1)
{
    ...
    halName = DP_CLASS;
    AudioCoreService::GetCoreService()->dpSinkModuleInfo_.format = "666";
    ecFormat = AudioCoreService::GetCoreService()->GetEcFormat(halName, outModuleInfo);
    EXPECT_EQ(ecFormat, "666");

    AudioCoreService::GetCoreService()->dpSinkModuleInfo_.format = "";
    ecFormat = AudioCoreService::GetCoreService()->GetEcFormat(halName, outModuleInfo);
    EXPECT_EQ(ecFormat, "");

    halName = USB_CLASS;
    AudioCoreService::GetCoreService()->usbSinkModuleInfo_.format = "444";
    ecFormat = AudioCoreService::GetCoreService()->GetEcFormat(halName, outModuleInfo);
    EXPECT_EQ(ecFormat, "444");

    AudioCoreService::GetCoreService()->usbSinkModuleInfo_.format = "";
    ecFormat = AudioCoreService::GetCoreService()->GetEcFormat(halName, outModuleInfo);
    EXPECT_EQ(ecFormat, "");
...
HWTEST_F(AudioPolicyServiceExtUnitTest, GetEcChannels_001, TestSize.Level1)
{
    ...
    halName = DP_CLASS;
    AudioCoreService::GetCoreService()->dpSinkModuleInfo_.channels = "666";
    ecChannels = AudioCoreService::GetCoreService()->GetEcChannels(halName, outModuleInfo);
    EXPECT_EQ(ecChannels, "666");

    AudioCoreService::GetCoreService()->dpSinkModuleInfo_.channels = "";
    ecChannels = AudioCoreService::GetCoreService()->GetEcChannels(halName, outModuleInfo);
    EXPECT_EQ(ecChannels, "0");

    halName = USB_CLASS;
    AudioCoreService::GetCoreService()->usbSinkModuleInfo_.channels = "555";
    ecChannels = AudioCoreService::GetCoreService()->GetEcChannels(halName, outModuleInfo);
    EXPECT_EQ(ecChannels, "555");

    AudioCoreService::GetCoreService()->usbSinkModuleInfo_.channels = "";
    ecChannels = AudioCoreService::GetCoreService()->GetEcChannels(halName, outModuleInfo);
    EXPECT_EQ(ecChannels, "0");

    halName = INVALID_CLASS;
    AudioCoreService::GetCoreService()->audioEcInfo_.inputDevice.deviceType_ = DEVICE_TYPE_MIC;
    ecChannels = AudioCoreService::GetCoreService()->GetEcChannels(halName, outModuleInfo);
    EXPECT_EQ(ecChannels, "2");

    AudioCoreService::GetCoreService()->audioEcInfo_.inputDevice.deviceType_ = DEVICE_TYPE_MAX;
    ecChannels = AudioCoreService::GetCoreService()->GetEcChannels(halName, outModuleInfo);
    EXPECT_EQ(ecChannels, "2");
}
...
HWTEST_F(AudioPolicyServiceExtUnitTest, UpdateAudioEcInfo_001, TestSize.Level1)
{
    ...
    server->audioPolicyService_.ecEnableState_ = false;
    AudioCoreService::GetCoreService()->UpdateAudioEcInfo(inputDevice, outputDevice);

    inputDevice = AudioCoreService::GetCoreService()->audioEcInfo_.inputDevice;
    outputDevice = AudioCoreService::GetCoreService()->audioEcInfo_.outputDevice;
    server->audioPolicyService_.ecEnableState_ = true;
    AudioCoreService::GetCoreService()->UpdateAudioEcInfo(inputDevice, outputDevice);
...
HWTEST_F(AudioPolicyServiceExtUnitTest, UpdateStreamCommonInfo_001, TestSize.Level1)
{
    ...
    server->audioPolicyService_.ecEnableState_ = true;
    AudioCoreService::GetCoreService()->UpdateStreamCommonInfo(moduleInfo, targetInfo, sourceType);
...
HWTEST_F(AudioPolicyServiceExtUnitTest, UpdateStreamMicRefInfo_001, TestSize.Level1)
{
    ...
    AudioCoreService::GetCoreService()->UpdateStreamMicRefInfo(moduleInfo, sourceType);
...
HWTEST_F(AudioPolicyServiceExtUnitTest, GetA2dpOffloadCodecAndSendToDsp_001, TestSize.Level1)
{
    ...
    CurrentActiveDevice().deviceType_ = DeviceType::DEVICE_TYPE_SPEAKER;
    server->audioPolicyService_.audioA2dpOffloadManager_->GetA2dpOffloadCodecAndSendToDsp();
}
...
HWTEST_F(AudioPolicyServiceExtUnitTest, OnPreferredStateUpdated_001, TestSize.Level1)
{
    ...
    desc.macAddress_ = CurrentActiveDevice().macAddress_;
