...
HWTEST_F(AudioPolicyServiceUnitTest, AudioPolicyServiceTest_004, TestSize.Level1)
{
    ...
    for (const auto& deviceType : deviceTypes) {
        ...
        CurrentActiveDevice().deviceType_ = deviceType;
...
void debugPrintMemoryVariable()
{
    ...
    AUDIO_INFO_LOG("debugPrintMemoryVariable() currentActiveDevice_:%{public}d, addr:%{private}p",
        static_cast<std::uint32_t>(CurrentActiveDevice().deviceType_), &CurrentActiveDevice().deviceType_);
...
HWTEST_F(AudioPolicyServiceUnitTest, SetWakeUpAudioCapturer_001, TestSize.Level1)
{
    ...
    AudioCoreConfigManager::GetInstance().isAdapterInfoMap_.store(false);
    AudioCoreConfigManager::GetInstance().OnUpdateRouteSupport(false);
    int32_t ret = AudioCoreService::GetCoreService()->SetWakeUpAudioCapturer(capturerOptions);
    EXPECT_EQ(ERROR, ret);

    AudioCoreConfigManager::GetInstance().isAdapterInfoMap_.store(true);
    AudioCoreConfigManager::GetInstance().OnUpdateRouteSupport(true);
    ret = AudioCoreService::GetCoreService()->SetWakeUpAudioCapturer(capturerOptions);
    EXPECT_EQ(SUCCESS, ret);
}
...
HWTEST_F(AudioPolicyServiceUnitTest, OnCapturerSessionAdded_001, TestSize.Level1)
{
    ...
    AudioCoreService::GetCoreService()->normalSourceOpened_ = SOURCE_TYPE_INVALID;
    ...
    int32_t ret = SUCCESS;
    AudioCoreService::GetCoreService()->normalSourceOpened_ = SOURCE_TYPE_VOICE_CALL;
    ret = AudioCoreService::GetCoreService()->OnCapturerSessionAdded(TEST_SESSIONID,
        sessionInfo, streamInfo);
    EXPECT_EQ(SUCCESS, ret);

    GetServerPtr()->audioPolicyService_.audioConnectedDevice_.connectedDevices_.clear();
    sessionInfo.sourceType = SOURCE_TYPE_REMOTE_CAST;
    AudioCoreService::GetCoreService()->OnCapturerSessionAdded(TEST_SESSIONID,
        sessionInfo, streamInfo);
    EXPECT_EQ(SUCCESS, ret);
    ...
    AudioCoreService::GetCoreService()->capturerSessionIdisRemovedSet_.insert(TEST_SESSIONID);
    AudioCoreService::GetCoreService()->OnCapturerSessionAdded(TEST_SESSIONID,
        sessionInfo, streamInfo);
    EXPECT_EQ(SUCCESS, ret);
}
...
HWTEST_F(AudioPolicyServiceUnitTest, OnCapturerSessionRemoved_001, TestSize.Level1)
{
    ...
    ASSERT_NE(nullptr, GetServerPtr());
    AudioCoreService::GetCoreService()->OnCapturerSessionRemoved(TEST_SESSIONID);
}
...
HWTEST_F(AudioPolicyServiceUnitTest, LoadInnerCapturerSink_001, TestSize.Level1)
{
    ...
    for (const auto& audioSampleFormat : audioSampleFormats) {
        ...
        AudioCoreService::GetCoreService()->LoadInnerCapturerSink(moduleName, streamInfo);
        AudioCoreService::GetCoreService()->UnloadInnerCapturerSink(moduleName);
    }
}
 ...
 * @tc.desc  : Test GetRealUid interfaces.
 */
HWTEST_F(AudioPolicyServiceUnitTest, GetRealUid_001, TestSize.Level1)
{
    auto server = GetServerPtr();
    ASSERT_NE(nullptr, server);

    AudioProcessConfig config;
    config.callerUid = 456;
    int32_t uid = server->audioPolicyService_.GetRealUid(config);
    EXPECT_EQ(uid, 456);
    config.callerUid = MEDIA_SERVICE_UID;
    config.appInfo.appUid = 456;
    uid = server->audioPolicyService_.GetRealUid(config);
    EXPECT_EQ(uid, 456);
}
} // namespace AudioStandard
} // namespace OHOS
