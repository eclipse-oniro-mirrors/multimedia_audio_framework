...
HWTEST(AudioRouterCenterUnitTest, AudioRouterCenter_003, TestSize.Level1)
{
    AudioRouterCenter audioRouterCenter;
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> descs;
    FetchDeviceInfo info = { STREAM_USAGE_ALARM, 0, ""};
    descs = audioRouterCenter.FetchOutputDevices(info);
...
public:
    ...
    MOCK_METHOD(int32_t, SetSplitModeReadyRefined, (), (override));
};
...
public:
    ...
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> ringRenderRet_;
    ...
    MockRouter(RouterType type = ROUTER_TYPE_DEFAULT,
        ...
        std::vector<std::shared_ptr<AudioDeviceDescriptor>> ringRenderRet = {})
        : routerType_(type),
          ...
          ringRenderRet_(std::move(ringRenderRet)) {}

    std::shared_ptr<AudioDeviceDescriptor> GetMediaRenderDevice(StreamUsage, int32_t, const uint32_t) override
    {
        return mediaRenderRet_;
    }

    std::shared_ptr<AudioDeviceDescriptor> GetCallRenderDevice(StreamUsage, int32_t, const uint32_t) override
    {
    ...
    {
        if (!ringRenderRet_.empty()) {
            return ringRenderRet_;
        }
        if (mediaRenderRet_) {
            return {mediaRenderRet_};
        }
...
/**
 * @tc.name  : Test MediaFollowRingStrategy.
 * @tc.number: AudioRouterCenter_011
 * @tc.desc  : Test MEDIA stream follows RINGTONE device, audio scene is RINGING.
 */
HWTEST(AudioRouterCenterUnitTest, AudioRouterCenter_011, TestSize.Level1)
{
    std::shared_ptr<AudioDeviceDescriptor> speakerDevice = std::make_shared<AudioDeviceDescriptor>();
    speakerDevice->deviceType_ = DEVICE_TYPE_SPEAKER;
    speakerDevice->deviceId_ = 100;
    
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> ringDevices;
    ringDevices.push_back(speakerDevice);

    auto mockRingRouter = std::make_unique<MockRouter>(
        ROUTER_TYPE_DEFAULT,
        nullptr,
        nullptr,
        nullptr,
    ...
    AudioRouterCenter &audioRouterCenter = AudioRouterCenter::GetAudioRouterCenter();
    audioRouterCenter.ringRenderRouters_.clear();
    audioRouterCenter.ringRenderRouters_.emplace_back(std::move(mockRingRouter));

    AudioSceneManager &audioSceneManager = AudioSceneManager::GetInstance();
    audioSceneManager.audioScene_ = AUDIO_SCENE_RINGING;
    audioSceneManager.lastAudioScene_ = AUDIO_SCENE_DEFAULT;

    FetchDeviceInfo info;
    info.streamUsage = STREAM_USAGE_MEDIA;
    info.clientUID = 1000;
    info.routerType = ROUTER_TYPE_DEFAULT;
    info.caller = "test_caller";
    info.privacyType = PRIVACY_TYPE_PUBLIC;

    std::vector<std::shared_ptr<AudioDeviceDescriptor>> result =
        audioRouterCenter.FetchOutputDevicesInner(info);
    EXPECT_EQ(result.size(), 1);
    ASSERT_NE(result[0], nullptr);
    EXPECT_EQ(result[0]->deviceType_, DEVICE_TYPE_SPEAKER);
    EXPECT_EQ(result[0]->deviceId_, 100);
}

/**
 * @tc.name  : Test MediaFollowRingStrategy.
 * @tc.number: AudioRouterCenter_012
 * @tc.desc  : Test MEDIA stream falls back to dafault media device when device is invalid.
 */
HWTEST(AudioRouterCenterUnitTest, AudioRouterCenter_012, TestSize.Level1)
{
    auto mediaDevice = std::make_shared<AudioDeviceDescriptor>();
    mediaDevice->deviceType_ = DEVICE_TYPE_SPEAKER;
    mediaDevice->deviceId_ = 200;
    
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> emptyRingDevices;
    ...
    ASSERT_NE(result[0], nullptr);
    EXPECT_EQ(result[0]->deviceType_, DEVICE_TYPE_SPEAKER);
    EXPECT_EQ(result[0]->deviceId_, 200);
}

/**
 * @tc.name  : Test MediaFollowRingStrategy.
 * @tc.number: AudioRouterCenter_013
 * @tc.desc  : Test MEDIA stream follows VOICE_RINGTONE device, audio scene is VOICE_RINGING.
 */
HWTEST(AudioRouterCenterUnitTest, AudioRouterCenter_013, TestSize.Level1)
{
    std::shared_ptr<AudioDeviceDescriptor> speakerDevice = std::make_shared<AudioDeviceDescriptor>();
    speakerDevice->deviceType_ = DEVICE_TYPE_SPEAKER;
    speakerDevice->deviceId_ = 101;
    ...
    AudioSceneManager &audioSceneManager = AudioSceneManager::GetInstance();
    audioSceneManager.audioScene_ = AUDIO_SCENE_VOICE_RINGING;
    audioSceneManager.lastAudioScene_ = AUDIO_SCENE_DEFAULT;
    ...
    ASSERT_NE(result[0], nullptr);
    EXPECT_EQ(result[0]->deviceType_, DEVICE_TYPE_SPEAKER);
    EXPECT_EQ(result[0]->deviceId_, 101);
}

/**
 * @tc.name  : Test MediaFollowRingStrategy.
 * @tc.number: AudioRouterCenter_014
 * @tc.desc  : Test MEDIA stream falls back to dafault media device when device is invalid.
 */
HWTEST(AudioRouterCenterUnitTest, AudioRouterCenter_014, TestSize.Level1)
{
    auto mediaDevice = std::make_shared<AudioDeviceDescriptor>();
    mediaDevice->deviceType_ = DEVICE_TYPE_SPEAKER;
    mediaDevice->deviceId_ = 300;
    ...
    ASSERT_NE(result[0], nullptr);
    EXPECT_EQ(result[0]->deviceType_, DEVICE_TYPE_SPEAKER);
    EXPECT_EQ(result[0]->deviceId_, 300);
}

/**
 * @tc.name  : Test MediaFollowRingStrategy.
 * @tc.number: AudioRouterCenter_015
 * @tc.desc  : Verify that Ringing scene not tigger media follow call strategy.
 */
HWTEST(AudioRouterCenterUnitTest, AudioRouterCenter_015, TestSize.Level1)
{
    AudioRouterCenter audioRouterCenter;
...
/**
 * @tc.name  : Test IsWirelessPrivacyDevice.
 * @tc.number: IsWirelessPrivacyDevice_001
 * @tc.desc  : Verify that IsWirelessPrivacyDevice returns correct value for different device types and categories.
 */
HWTEST(AudioRouterCenterUnitTest, IsWirelessPrivacyDevice_001, TestSize.Level1)
{
    AudioRouterCenter audioRouterCenter;

    auto desc1 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_BLUETOOTH_A2DP, OUTPUT_DEVICE);
    desc1->deviceCategory_ = BT_HEADPHONE;
    EXPECT_TRUE(audioRouterCenter.IsWirelessPrivacyDevice(desc1));

    auto desc2 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_BLUETOOTH_A2DP, OUTPUT_DEVICE);
    desc2->deviceCategory_ = BT_GLASSES;
    EXPECT_TRUE(audioRouterCenter.IsWirelessPrivacyDevice(desc2));

    auto desc3 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_BLUETOOTH_A2DP, OUTPUT_DEVICE);
    desc3->deviceCategory_ = CATEGORY_DEFAULT;
    EXPECT_FALSE(audioRouterCenter.IsWirelessPrivacyDevice(desc3));

    auto desc4 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_NEARLINK, OUTPUT_DEVICE);
    desc4->deviceCategory_ = BT_HEADPHONE;
    EXPECT_TRUE(audioRouterCenter.IsWirelessPrivacyDevice(desc4));

    auto desc5 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_NEARLINK, OUTPUT_DEVICE);
    desc5->deviceCategory_ = BT_GLASSES;
    EXPECT_TRUE(audioRouterCenter.IsWirelessPrivacyDevice(desc5));

    auto desc6 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_NEARLINK, OUTPUT_DEVICE);
    desc6->deviceCategory_ = CATEGORY_DEFAULT;
    EXPECT_FALSE(audioRouterCenter.IsWirelessPrivacyDevice(desc6));

    auto desc7 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_SPEAKER, OUTPUT_DEVICE);
    desc7->deviceCategory_ = BT_HEADPHONE;
    EXPECT_FALSE(audioRouterCenter.IsWirelessPrivacyDevice(desc7));
}

/**
 * @tc.name  : Test IsVehicleDevice.
 * @tc.number: IsVehicleDevice_001
 * @tc.desc  : Verify that IsVehicleDevice returns correct value for different device types and categories.
 */
HWTEST(AudioRouterCenterUnitTest, IsVehicleDevice_001, TestSize.Level1)
{
    AudioRouterCenter audioRouterCenter;

    auto desc1 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_BLUETOOTH_A2DP, OUTPUT_DEVICE);
    desc1->deviceCategory_ = BT_CAR;
    EXPECT_TRUE(audioRouterCenter.IsVehicleDevice(desc1));

    auto desc2 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_BLUETOOTH_A2DP, OUTPUT_DEVICE);
    desc2->deviceCategory_ = BT_HEADPHONE;
    EXPECT_FALSE(audioRouterCenter.IsVehicleDevice(desc2));

    auto desc3 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_NEARLINK, OUTPUT_DEVICE);
    desc3->deviceCategory_ = BT_CAR;
    EXPECT_TRUE(audioRouterCenter.IsVehicleDevice(desc3));

    auto desc4 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_NEARLINK, OUTPUT_DEVICE);
    desc4->deviceCategory_ = BT_HEADPHONE;
    EXPECT_FALSE(audioRouterCenter.IsVehicleDevice(desc4));

    auto desc5 = std::make_shared<AudioDeviceDescriptor>(DEVICE_TYPE_SPEAKER, OUTPUT_DEVICE);
    desc5->deviceCategory_ = BT_CAR;
    EXPECT_FALSE(audioRouterCenter.IsVehicleDevice(desc5));
}
} // namespace AudioStandard
} // namespace OHOS
