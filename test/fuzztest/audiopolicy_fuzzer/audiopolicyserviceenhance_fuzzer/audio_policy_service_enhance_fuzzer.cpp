...
void AudioPolicyServiceEnhanceOneFuzzTest(FuzzedDataProvider& fdp)
{
    ...
    AudioRouterSelectStrategy::GetInstance().UpdateCurrentInputDevice(-1,
        {make_shared<AudioDeviceDescriptor>(deviceDescriptor)});
}
...
void AudioPolicyServiceEnhanceSevenFuzzTest(FuzzedDataProvider& fdp)
{
    ...
    GetServerPtr()->audioPolicyService_.audioVolumeManager_.SetRingerModeMute(false);
...
void AudioPolicyServiceEnhanceNineFuzzTest()
{
    ...
    AudioCoreConfigManager::GetInstance().GetAudioAdapterInfos(adapterInfoMap);
    std::unordered_map<std::string, std::string> volumeGroupData;
    AudioCoreConfigManager::GetInstance().GetVolumeGroupData(volumeGroupData);
    AudioCoreConfigManager::GetInstance().GetInterruptGroupData(volumeGroupData);
    ...
    CurrentActiveDevice().deviceType_ = DEVICE_TYPE_FILE_SINK;
}
