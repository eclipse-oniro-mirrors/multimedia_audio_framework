...
void AudioCoreServicePrivateUidGatedOffloadFuzzTest()
{
    auto audioCoreService = std::make_shared<AudioCoreService>();
    if (audioCoreService == nullptr) {
        return;
    ...
    bool allowed = GetData<bool>();
    audioCoreService->SetOffloadAllowedForUid(uid, allowed);
    audioCoreService->IsOffloadAllowedForUid(uid);
    ...
    streamDesc->callerUid_ = static_cast<int32_t>(uid);
    ...
    audioCoreService->IsStreamSupportLowpower(streamDesc);
}
...
void AudioCoreServicePrivateUpdateOutputRouteFuzzTest()
{
    ...
    uint32_t pipeId = GetData<uint32_t>();
...
void AudioCoreServicePrivateWriteInputRouteChangeEventFuzzTest()
{
    ...
    AudioStreamDeviceChangeReason reason = AudioStreamDeviceChangeReason::NEW_DEVICE_AVAILABLE;
    audioCoreService->WriteInputRouteChangeEvent(desc, reason, -1);
}
...
void AudioCoreServicePrivateCheckAndUpdateOffloadEnableForStream()
{
    ...
    uint32_t ioHandle = GetData<uint32_t>();
    audioCoreService->CheckAndUpdateOffloadEnableForStream(OFFLOAD_NEW, audioStreamDescriptor, ioHandle);
    audioCoreService->CheckAndUpdateOffloadEnableForStream(OFFLOAD_MOVE_IN, audioStreamDescriptor, ioHandle);
    audioCoreService->CheckAndUpdateOffloadEnableForStream(OFFLOAD_MOVE_OUT, audioStreamDescriptor, ioHandle);
}
...
TestFuncs g_testFuncs[] = {
    ...
    AudioCoreServicePrivateUidGatedOffloadFuzzTest,
};
