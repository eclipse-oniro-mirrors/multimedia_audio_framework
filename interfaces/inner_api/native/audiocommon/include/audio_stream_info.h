...
enum StreamUsage {
    ...
    STREAM_USAGE_VOICE_TRANSFER_COMMUNICATION,
    STREAM_USAGE_VOICE_TRANSFER_RINGTONE,
    STREAM_USAGE_MAX = STREAM_USAGE_VOICE_TRANSFER_RINGTONE
};
...
enum AudioPipeType {
    ...
    PIPE_TYPE_OUT_INTERPHONE = 19,
    PIPE_TYPE_IN_INTERPHONE = 20,
    PIPE_TYPE_IN_NORMAL_LIVE = 21,
    PIPE_TYPE_IN_CAMCORDER = 23,
};
...
struct AudioStreamParams {
    ...
    uint32_t micInSamplingRate = 0;
    uint8_t micInEncoding = 0;
    uint8_t micInFormat = 0;
    uint8_t micInChannels = 0;
    uint64_t micInChannelLayout = 0ULL;
    bool isVoiceRecognitionMicInEcDegrade = false;
    uint8_t ultraFastFlag = 0;
};
...
inline const std::vector<AudioChannelLayout> RENDERER_SUPPORTED_CHANNELLAYOUTS {
    CH_LAYOUT_UNKNOWN,
    CH_LAYOUT_MONO,
...
inline const std::vector<AudioSampleFormat> AUDIO_SUPPORTED_FORMATS {
    SAMPLE_U8,
    SAMPLE_S16LE,
    SAMPLE_S24LE,
    SAMPLE_S32LE,
...
inline const std::vector<AudioChannel> RENDERER_SUPPORTED_CHANNELS {
    MONO,
    STEREO,
    CHANNEL_3,
    CHANNEL_4,
...
inline const std::vector<AudioChannel> CAPTURER_SUPPORTED_CHANNELS {
    MONO,
    STEREO,
    CHANNEL_3,
    CHANNEL_4,
...
inline const std::vector<AudioEncodingType> AUDIO_SUPPORTED_ENCODING_TYPES {
    ENCODING_PCM,
    ENCODING_AUDIOVIVID,
    ENCODING_EAC3,
    ENCODING_AC3,
...
inline const std::set<AudioEncodingType> HWDECODING_TYPES {
    ENCODING_EAC3,
    ENCODING_AC3,
    ENCODING_TRUE_HD,
    ENCODING_DTS_HD,
...
inline const std::vector<AudioSamplingRate> AUDIO_SUPPORTED_SAMPLING_RATES {
    SAMPLE_RATE_8000,
    SAMPLE_RATE_11025,
    SAMPLE_RATE_12000,
    SAMPLE_RATE_16000,
...
inline const std::vector<StreamUsage> AUDIO_SUPPORTED_STREAM_USAGES {
...
public:
    ...
    bool operator==(const AudioStreamInfo &info) const
    {
        return encoding == info.encoding && format == info.format && channels == info.channels &&
            ...
            customSampleRate == info.customSampleRate;
    }

    uint32_t GetEffectiveSampleRate() const
    {
        return customSampleRate != 0 ? customSampleRate : static_cast<uint32_t>(samplingRate);
    }
...
enum SpatialAudioSourceType : int32_t {
