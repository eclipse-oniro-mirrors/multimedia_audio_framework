...
public:
    ...
    /**
     * @brief Set the system-controlled app volume percentage for the specified UID.
     *
     * @param appUid app uid.
     * @param volumePercentage The volume percentage (0-100) to be set for the current uid app.
     * @return Returns {@link SUCCESS} if volume is successfully set; returns an error code
     * defined in {@link audio_errors.h} otherwise.
     */
    int32_t SetSystemAppVolumePercentageForUid(const int32_t appUid, const int32_t volumePercentage);
    ...
    /**
     * @brief Get the system-controlled app volume percentage of the specified application.
     *
     * @param appUid App uid.
     * @param volumePercent Output parameter, used to return the queried volume percentage value (0-100).
     * @return Returns {@link SUCCESS} if volume is successfully set; returns an error code
     * defined in {@link audio_errors.h} otherwise.
     */
    int32_t GetSystemAppVolumePercentageForUid(int32_t appUid, int32_t &volumePercentage) const;
    ...
    /**
     * @brief Restore the audio output device selection according to the filter conditions.
     *
     * @param audioRendererFilter filter conditions.
     * @return Returns {@link SUCCESS} if the restore is successful; returns an error code defined
     * in {@link audio_errors.h} otherwise.
     * @since 16
     */
    ...
    /**
     * @brief Set split mode ready to ensure that device can be selected.
     *
     * @return Returns {@link SUCCESS} if the device is set successfully; otherwise, returns an error code defined
    ...
    /**
     * @brief update safe volume by str mode.
     *
     * @return Returns {@link SUCCESS} if the operation is successfully.
     */
    ...
    /**
     * @brief Get volume information of all currently active streams
     *
     * @return Active streams volume information
     * @since 24
     */
    ...
    /**
     * @brief Report playback capture user choice.
     *
     * @param sessionId Specifies which playback capture session to report.
     * @param choice The user choice
     * @since 24
     */
    int32_t ReportPlaybackCaptureUserChoice(uint32_t sessionId, bool choice);

    /**
     * @brief Report wakeup stage, result and errCode.
     *
     * @param stage Wakeup stage.
     * @param wakeupResult Wakeup result.
     * @param wakeupErrCode Wakeup error code.
     * @since 26
     */
    int32_t ReportWakeupEvent(const AudioWakeupTrackInfo &audioWakeupTrackInfo);
private:
    AudioSystemManager();
