...
public:
    ...
    /**
     * @brief Set the output device for audio session scene.
     *
     * @param deviceType. The available deviceTypes are SPEAKER/DEFAULT.
     * @return Returns {@link SUCCESS} if the operation is successful; returns an error code
     * defined in {@link audio_errors.h} otherwise.
     * @since 26
     */
    ...
    int32_t SetCapturerMuteHint(bool mute);

    void SetRegisterStateChangeCallbackFlag(const bool stateChangeCallbackFlag);
...
private:
    ...
    std::mutex setMediaOutputDeviceMutex_;
    bool setMediaOutputDevice_ = false;
    DeviceType setMediaDeviceType_ = DEVICE_TYPE_INVALID;
    ...
    bool stateChangeCallbackFlag_ = false;
    bool stateChangeCallbackByIpcFlag_ = false;
    bool audioSessionSceneFlag_ = false;
    bool isFirstActivateV2_ = true;

    static std::mutex callbackRegisteredMutex_;
    static bool callbackRegistered_;
};
} // namespace AudioStandard
} // namespace OHOS
