...
/**
 * @brief Sets mute hint for the capturer stream.
 * The hint only takes effect when the capturer stream is in running state.
 *
 * @param capturer Capturer generated by OH_AudioStreamBuilder_GenerateCapturer().
 * @param mute Indicates whether the stream should be considered muted.
 * @return result code for this function.
 *         {@link #AUDIOSTREAM_SUCCESS} succeeds in setting mute hint.
 *         {@link #AUDIOSTREAM_ERROR_INVALID_PARAM} the param of capturer is nullptr.
 *         {@link #AUDIOSTREAM_ERROR_ILLEGAL_STATE} function called when capturer is not in running state.
 *         {@link #AUDIOSTREAM_ERROR_SYSTEM} system internal error.
 * @since 23
 */
